...
def require_document_ui_access(request: Request, document: dict) -> None:
    ...
    for asset_id in document_asset_ids(document):
        if not catalog.asset_exists(asset_id):
            raise HTTPException(
                ...
                detail={'code': 'ASSET_MISSING', 'message': f'仪表盘引用的图片已不存在：{asset_id}'},
            )
...
def unique_slug(database: DatabaseSession, name: str) -> str:
    ...
    candidate = base
    suffix = 2
    if database.scalar(select(Project.id).where(Project.slug == candidate)):
        while True:
            candidate = f'{base}-{suffix}'
            suffix += 1
            if not database.scalar(select(Project.id).where(Project.slug == candidate)):
                break
    return candidate
...
def create_project(payload: ProjectCreateRequest, request: Request, database: DatabaseSession, user: LicensedUser) -> dict:
    ...
    project = Project(
        id=project_id,
        name=payload.name,
        slug=unique_slug(database, payload.name),
...
def update_project_draft(
    ...
    request.app.state.global_log.append(
        ...
        '仪表盘编辑器',
        '配置',
        f'仪表盘已保存：{document["name"]}（修订 {draft.revision}）',
    )
