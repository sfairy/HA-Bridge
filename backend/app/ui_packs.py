...
UI_PACKS = (
    UIPack(
        ...
        preview_url='/bridge-static/ui-packs/dwell-light-dashboard-cover.png',
        ...
        dashboard_templates=(
            DashboardTemplate(
                ...
                canvas_height=1293,
                preview_urls=(
                    '/bridge-static/ui-packs/qiguang-preview-01-overview.png',
                    '/bridge-static/ui-packs/qiguang-preview-02-lights.png',
                    '/bridge-static/ui-packs/qiguang-preview-03-devices.png',
                    '/bridge-static/ui-packs/qiguang-preview-04-vacuum.png',
                    '/bridge-static/ui-packs/qiguang-preview-05-climate-popup.png',
                    '/bridge-static/ui-packs/qiguang-preview-06-light-popup.png',
                    '/bridge-static/ui-packs/qiguang-preview-07-light-group-popup.png',
                    '/bridge-static/ui-packs/qiguang-preview-08-temperature-popup.png',
                    '/bridge-static/ui-packs/qiguang-preview-09-vacuum-popup.png',
                ),
                preview_labels=(
                    '家庭总览',
                    '所有灯光',
...
def load_dashboard_template(ui_pack_id: str, template_id: str) -> tuple[DashboardTemplate, dict]:
    ...
    if not path.is_relative_to(TEMPLATE_ROOT.resolve()) or not path.is_file():
...
def require_ui_pack_access(request: Request, ui_pack_id: str) -> UIPack:
    ui_pack = get_ui_pack(ui_pack_id)
    if ui_pack is None:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail='所选 UI 方案不存在。')
    if not request.app.state.license_service.allows(ui_pack.feature_code):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail={'code': 'UI_PACK_RESTRICTED', 'message': f'当前授权尚未解锁“{ui_pack.name}”。'},
        )
    return ui_pack
