...
function normalizeSelectedFiles(files) {
  ...
  return [...new Set(files.map(file => String(file || "")).filter(file => ALLOWED_EXPORT_FILES.has(file) || /^(?:group|screen|vehicle):[A-Za-z0-9_.:-]{1,180}$/.test(file)))].slice(0, 128);
}
function normalizeCamera(camera) {
  ...
  return {
    ...
    focalLength: mode === "perspective" ? clamp(finite(camera?.focalLength, 50), 18, 120) : null
  };
}
...
export function normalizeExportPresetSlots(slots) {
  ...
  const count = Array.isArray(slots) ? Math.max(1, Math.min(8, list.length || 1)) : 4;
  return Array.from({
    length: count
  }, (_slot, index) => normalizeExportPreset(list[index]));
}
...
export function exportPresetSummary(preset, floorNames = new Map()) {
  ...
  const floorLabel = normalized.floorMode === "all" ? "全楼合并" : floorNames.get(normalized.floorId) || "楼层已变更";
  const cameraLabel = normalized.camera.mode === "perspective" ? "透视" : "正交";
  return normalized.width + "×" + normalized.height + " · " + floorLabel + " · " + cameraLabel;
}
