...
import { GLTFLoader as value928 } from "/bridge-static/vendor/three/0.182.0/GLTFLoader.js?v=20260811-three-0182-glb2";
import { SameOriginDRACOLoader as value929 } from "./draco-loader.js?v=20260904-csp-static-worker-v1";
...
import { syncControlValue as value930 } from "./ui-controls.js?v=20260826-input-stability-v1";
import { initializeNumberInputs as value931, initializeStudioSelects as value932, syncStudioSelect } from "./studio-widgets.js?v=20260901-studio-widgets-v1";
...
import { createSpotShadowAtlasController } from "./studio-shadow-atlas.js?v=20260907-zero-light-v1-20260908-environment-v1-20260908-curtains-v1";
...
const renderCache2 = isStageEmbed ? createRenderCache({
  ...
  report: arg61 => {
    document.documentElement.dataset.lightRenderCache = JSON.stringify(arg61);
  }
}) : null;
...
function computeLightRenderCacheKey(width, height) {
  ...
  const roundMatrix = elements => elements.map(arg62 => Math.round(arg62 * 100000000) / 100000000);
  const visibility = [];
...
function projectHasItemModel(startsWith) {
  return !!projectDoc2?.floors?.some(floor => floor.scene?.items?.some(tvMountStyle => {
    if (startsWith.startsWith("tv_")) {
      ...
      return tvMountStyle.type === "tv" && startsWith === "tv_" + value;
    }
...
const dracoLoader = new value929("/bridge-static/3d-studio/draco-decoder-worker.js?v=20260904-csp-static-worker-v1");
...
function flushDeferredModelLoads(keys = []) {
  ...
  window.externalModelLoadsDeferred = false;
  const modelKeys = [...new Set(keys)].filter(key => ALL_ITEM_MODELS[key]);
  if (!modelKeys.length) {
    return [];
  }
...
const measureCtx = fn.getContext("2d");
const {
  drawMetricGrid,
...
function defaultFloorChineseName(arg0) {
  return ["一层", "二层", "三层", "四层", "五层", "六层", "七层", "八层", "九层", "十层"][arg0] || arg0 + 1 + "层";
}
...
function normalizeProjectDocument(projectDoc2) {
  ...
  const floors = list?.length ? list.map((floor, arg1) => {
    const scene = normalizeFloorScene(floor?.scene);
    const flag2 = floor?.originInitialized === true;
    const normalizeLabelText2 = normalizeLabelText(floor?.name, defaultFloorChineseName(arg1), 24);
    const name = normalizeLabelText2 === arg1 + 1 + "层" ? defaultFloorChineseName(arg1) : normalizeLabelText2;
    return {
      id: String(floor?.id || makeId("floor")),
      name,
      elevation: clamp(finite(floor?.elevation, arg1 * 3), -30, 120),
      ...
      aligned: arg1 === 0 || floor?.aligned === true || Math.abs(finite(floor?.offsetX, 0)) > 0.000001 || Math.abs(finite(floor?.offsetZ, 0)) > 0.000001,
...
async function executePendingFloorDelete() {
  ...
  projectDoc2.floors.forEach((elevation, arg1) => {
    elevation.elevation = arg1 * projectDoc2.defaultFloorHeight;
  });
...
function buildWallOpeningsIndex(wallsInput, windowsInput, doorsInput, openingWidthScale, railingsInput = []) {
  ...
  const remapAttachment = attachment => {
    ...
    const value549 = clamp(finite(attachment.t, 0.5), 0, 1);
    const piece = pieces.find(entry => value549 >= entry.startT - 1e-7 && value549 <= entry.endT + 1e-7) || pieces.at(-1);
    const span = Math.max(piece.endT - piece.startT, 1e-7);
    const remapped = {
      ...attachment,
      wallId: piece.wall.id,
      t: clamp((value549 - piece.startT) / span, 0, 1)
    };
...
function normalizeFloorScene(scene) {
  ...
  const entries = Array.isArray(scene.windows) ? scene.windows.map(attachment => ({
  ...
  const wallOpenings = buildWallOpeningsIndex(list, entries, entries2, pixelsPerMeter2, entries3);
  return {
    schemaVersion: 2,
    background,
    calibration,
...
function cloneFloorScene(arg0 = floorScene2) {
...
function ensureTvScreenLayerNames(arg0) {
  const idSet = new Set(ms().map(screenLayerName => screenLayerName.screenLayerName));
  let value = 1;
...
function duplicateLightGroup(lightGroup) {
  ...
  const list = floorScene2.items.filter(item => lightItemTypes2.has(item.type) && item.lightGroupId === lightGroup.id).map(arg0 => ({
...
function reorderLightGroups(arg0, arg1, flag) {
  const value = floorScene2.lightGroups.findIndex(item => item.id === arg0);
  const fromIndex = floorScene2.lightGroups.findIndex(item => item.id === arg1);
  if (value < 0 || fromIndex < 0 || value === fromIndex) {
    return;
  }
  const list = [...floorScene2.lightGroups];
  const [movedGroup] = list.splice(value, 1);
  const targetIndex = list.findIndex(item => item.id === arg1);
  list.splice(targetIndex + (flag ? 1 : 0), 0, movedGroup);
  if (!list.every((id, arg12) => id.id === floorScene2.lightGroups[arg12]?.id)) {
    pushHistory();
  ...
  if (!lightLayerPanel.hidden) {
    ...
    for (const id2 of floorScene2.lightGroups) {
      ...
      el2.addEventListener("drop", event => {
        ...
        draggingLightGroupId = "";
        clearLightGroupDropIndicators();
        reorderLightGroups(arg0, id2.id, value);
      });
      const button = document.createElement("button");
      button.type = "button";
    ...
    for (const [value, screenEnabled] of ms().entries()) {
      const el2 = document.createElement("div");
...
function setAllLightGroupsEnabled(arg0) {
  ...
  if (!(floorScene2.lightGroups || []).every(enabled => enabled.enabled === arg0) || !list.every(screenEnabled => screenEnabled.screenEnabled !== false === arg0) || !list2.every(chargingEnabled => chargingEnabled.chargingEnabled === true === arg0)) {
    pushHistory();
...
function isSelected(arg0, arg1) {
  return selection?.kind === arg0 && selection.id === arg1 || multiSelection.some(kind => kind.kind === arg0 && kind.id === arg1);
}
...
function activeSelectionAssetCategory() {
  return up(multiSelection.length ? multiSelection : selection ? [selection] : []);
}
...
function wallIdMap() {
  const map = new Map(floorScene2.walls.map(id => [id.id, id]));
...
function showToast(arg0, arg1 = "") {
  ...
  toast2.textContent = arg0;
  toast2.className = ("toast visible " + arg1).trim();
  toastTimer = window.setTimeout(() => {
    toast2.className = "toast";
  }, arg1 === "warning" ? 4400 : 2600);
}
function setSaveStateLabel(arg0, arg1 = "") {
  saveState2.className = ("save-state " + arg1).trim();
  saveState2.innerHTML = "<i></i>" + arg0;
}
...
async function loadProjectDocument(floor) {
  ...
  if (flag) {
    try {
      ...
      await Promise.race([settledResults, new Promise(arg0 => window.setTimeout(arg0, 3500))]);
...
function zoomPlanViewAt(arg0, planPoint3 = {
  x: planWidth / 2,
  y: planHeight / 2
}) {
...
function getUnclosedWallEndpoints(arg0) {
  const value = getWallAnalysis(arg0);
  value.unclosedEndpoints ||= unclosedWallEndpoints(floorScene2.walls, value.tolerance, getFloorPolygons(arg0));
...
function drawDoorPreview(size, preview = {}) {
  ...
  if (value === "sliding-glass") {
    ...
    const slidingDoorPanelCenters2 = slidingDoorPanelCenters(distance2, ddpV7);
  ...
  if (value === "roller-shutter") {
    ...
    for (let step = 1; step < maxValue2; step += 1) {
      ...
      const planPoint7 = {
        x: isWall.center.x + isWall.unit.x * ddpV9 + planPoint6.x * maxValue,
        y: isWall.center.y + isWall.unit.y * ddpV9 + planPoint6.y * maxValue
      };
      drawPlanLine({
...
function drawItemOnPlan(item) {
  ...
  const value759 = item.width * pixelsPerMeter() * planView.zoom;
  const value760 = item.depth * pixelsPerMeter() * planView.zoom;
  ...
  if (item.type === "flooropening") {
    ...
    planCtx.fillRect(-value759 / 2, -value760 / 2, value759, value760);
    planCtx.setLineDash([6, 4]);
    planCtx.strokeRect(-value759 / 2, -value760 / 2, value759, value760);
    planCtx.setLineDash([]);
    planCtx.beginPath();
    planCtx.moveTo(-value759 / 2, -value760 / 2);
    planCtx.lineTo(value759 / 2, value760 / 2);
    planCtx.moveTo(value759 / 2, -value760 / 2);
    planCtx.lineTo(-value759 / 2, value760 / 2);
    planCtx.stroke();
    ...
    const planLabelProjectionMetrics2 = Math.max(Math.min(value759, value760) * 0.44, item.type === "downlight" ? 10 : 8);
    ...
    if (item.type === "striplight") {
      planCtx.beginPath();
      const value450 = Math.min(7, value760 * 0.42);
      planCtx.roundRect(-value759 / 2, -value760 * 0.34, value759, value760 * 0.68, value450);
      planCtx.fill();
      planCtx.stroke();
      planCtx.strokeStyle = iconX ? "rgba(255, 238, 195, .95)" : "rgba(196, 207, 216, .42)";
      planCtx.lineWidth = Math.max(2, value760 * 0.12);
      planCtx.beginPath();
      planCtx.moveTo(-value759 * 0.42, 0);
      planCtx.lineTo(value759 * 0.42, 0);
      planCtx.stroke();
    } else if (item.type === "ceilinglight") {
      const value413 = Math.max(Math.min(value759, value760) * 0.82, 16);
      planCtx.beginPath();
      planCtx.rect(-value413 / 2, -value413 / 2, value413, value413);
      planCtx.fill();
      planCtx.stroke();
      const value414 = value413 * 0.58;
      planCtx.strokeRect(-value414 / 2, -value414 / 2, value414, value414);
    } else {
      planCtx.beginPath();
      ...
      planCtx.stroke();
      for (let value396 = 0; value396 < 4; value396 += 1) {
        const value366 = value396 * Math.PI / 2;
        planCtx.beginPath();
        planCtx.moveTo(Math.cos(value366) * planLabelProjectionMetrics2 * 0.68, Math.sin(value366) * planLabelProjectionMetrics2 * 0.68);
        planCtx.lineTo(Math.cos(value366) * planLabelProjectionMetrics2 * 1.12, Math.sin(value366) * planLabelProjectionMetrics2 * 1.12);
        planCtx.stroke();
      }
    }
  } else if (item.type === "planlabel") {
    const cornerRadius = planLabelProjectionMetrics(value759, value760, item.lineLength);
    planCtx.fillStyle = "#929baa";
    const value471 = cornerRadius.titleFontSize;
    planCtx.font = "700 " + value471 + "px sans-serif";
    drawTrackedText(planCtx, item.title || "家庭总览", cornerRadius.titleStartX, cornerRadius.titleY, value471 * clamp(finite(item.titleSpacing, 1.05), 0, 1.8), cornerRadius.titleMaxWidth);
    const value472 = cornerRadius.iconX;
    const value473 = cornerRadius.iconY;
    const value474 = cornerRadius.iconSize;
    planCtx.fillStyle = "#929baa";
    planCtx.beginPath();
    planCtx.moveTo(value472, value473 - value474 * 0.58);
    planCtx.lineTo(value472 + value474 * 0.56, value473 - value474 * 0.02);
    planCtx.lineTo(value472 + value474 * 0.38, value473 - value474 * 0.02);
    planCtx.lineTo(value472 + value474 * 0.38, value473 + value474 * 0.5);
    planCtx.lineTo(value472 - value474 * 0.38, value473 + value474 * 0.5);
    planCtx.lineTo(value472 - value474 * 0.38, value473 - value474 * 0.02);
    planCtx.lineTo(value472 - value474 * 0.56, value473 - value474 * 0.02);
    planCtx.lineTo(value472, value473 - value474 * 0.52);
    planCtx.closePath();
    planCtx.fill();
    planCtx.fillStyle = "#929baa";
    planCtx.textAlign = "left";
    const value475 = cornerRadius.subtitleFontSize;
    planCtx.font = "400 " + value475 + "px \"Arial Narrow\", Arial, sans-serif";
    drawTrackedText(planCtx, item.subtitle || "HOME PLAN", cornerRadius.subtitleStartX, cornerRadius.subtitleY, value475 * clamp(finite(item.subtitleSpacing, 0.08), 0, 0.6), cornerRadius.subtitleMaxWidth);
    planCtx.strokeStyle = "rgba(146, 155, 170, .72)";
    planCtx.lineWidth = cornerRadius.baselineLineWidth;
    const value476 = cornerRadius.baselineY;
    const value477 = cornerRadius.baselineStartX;
    const value478 = value477 + cornerRadius.baselineLength;
    planCtx.beginPath();
    planCtx.moveTo(value477, value476);
    planCtx.lineTo(value478, value476);
    planCtx.moveTo(value477, value476 - cornerRadius.baselineCapHalfHeight);
    planCtx.lineTo(value477, value476 + cornerRadius.baselineCapHalfHeight);
    planCtx.moveTo(value478, value476 - cornerRadius.baselineCapHalfHeight);
    planCtx.lineTo(value478, value476 + cornerRadius.baselineCapHalfHeight);
    planCtx.stroke();
  } else if (item.type === "smallcar") {
    const curtainPosition = Math.min(value759 * 0.22, value760 * 0.08);
    if (item.chargingEnabled === true) {
      planCtx.save();
      planCtx.scale(value759 * 0.76, value760 * 0.62);
      ...
      const value397 = Math.max(9, Math.min(value759, value760) * 0.07);
      for (let value367 = -value759 * 0.62; value367 <= value759 * 0.62; value367 += value397) {
        for (let value347 = -value760 * 0.54; value347 <= value760 * 0.54; value347 += value397) {
          const value331 = Math.hypot(value367 / (value759 * 0.62), value347 / (value760 * 0.54));
          if (!(value331 >= 1)) {
            planCtx.globalAlpha = (1 - value331) * 0.72;
            planCtx.beginPath();
            planCtx.arc(value367, value347, Math.max(0.7, value397 * 0.1), 0, Math.PI * 2);
            planCtx.fill();
          }
        }
      }
    ...
    planCtx.beginPath();
    planCtx.roundRect(-value759 * 0.48, -value760 * 0.49, value759 * 0.96, value760 * 0.98, curtainPosition);
    planCtx.fill();
    planCtx.stroke();
    planCtx.fillStyle = "rgba(38, 48, 57, .72)";
    planCtx.beginPath();
    planCtx.roundRect(-value759 * 0.38, -value760 * 0.2, value759 * 0.76, value760 * 0.42, curtainPosition * 0.7);
    planCtx.fill();
    for (const value415 of [-0.5, 0.5]) {
      for (const value368 of [-0.3, 0.3]) {
        planCtx.fillRect(value415 * value759 - value759 * 0.045, value368 * value760 - value760 * 0.085, value759 * 0.09, value760 * 0.17);
      }
    }
    if (item.chargingEnabled === true) {
      planCtx.fillStyle = "#7dffd0";
      planCtx.beginPath();
      planCtx.moveTo(value759 * 0.028, -value760 * 0.095);
      planCtx.lineTo(-value759 * 0.058, value760 * 0.008);
      planCtx.lineTo(value759 * 0.006, value760 * 0.008);
      planCtx.lineTo(-value759 * 0.028, value760 * 0.095);
      planCtx.lineTo(value759 * 0.07, -value760 * 0.02);
      planCtx.lineTo(value759 * 0.008, -value760 * 0.02);
      planCtx.closePath();
      planCtx.fill();
    }
  } else if (item.type === "curtain") {
    const value416 = ["left", "right", "split"].includes(item.curtainPosition) ? item.curtainPosition : "split";
    const value417 = (arg32, arg33) => {
      planCtx.beginPath();
      planCtx.roundRect(arg32, -value760 * 0.46, arg33, value760 * 0.92, Math.min(value760 * 0.32, arg33 * 0.18));
      planCtx.fill();
      planCtx.stroke();
      planCtx.strokeStyle = "rgba(25, 34, 43, .48)";
      planCtx.lineWidth = 1;
      for (let value332 = 1; value332 < 5; value332 += 1) {
        const value316 = arg32 + arg33 * value332 / 5;
        planCtx.beginPath();
        planCtx.moveTo(value316, -value760 * 0.34);
        planCtx.lineTo(value316, value760 * 0.34);
        planCtx.stroke();
      }
    };
    planCtx.strokeStyle = selected ? "#ff9d2e" : "rgba(234, 240, 244, .72)";
    ...
    planCtx.beginPath();
    planCtx.moveTo(-value759 * 0.5, 0);
    planCtx.lineTo(value759 * 0.5, 0);
    planCtx.stroke();
    if (value416 === "left") {
      value417(-value759 * 0.5, value759 * 0.24);
    } else if (value416 === "right") {
      value417(value759 * 0.26, value759 * 0.24);
    } else {
      value417(-value759 * 0.5, value759 * 0.16);
      value417(value759 * 0.34, value759 * 0.16);
    }
  } else if (item.type === "pillar") {
    planCtx.beginPath();
    planCtx.rect(-value759 / 2, -value760 / 2, value759, value760);
    planCtx.fill();
    planCtx.stroke();
    planCtx.strokeStyle = selected ? "rgba(255, 193, 116, .95)" : "rgba(25, 34, 43, .5)";
    planCtx.lineWidth = 1;
    planCtx.strokeRect(-value759 * 0.36, -value760 * 0.36, value759 * 0.72, value760 * 0.72);
  } else if (item.type === "bar") {
    planCtx.beginPath();
    planCtx.roundRect(-value759 / 2, -value760 / 2, value759, value760, Math.min(5, value760 * 0.16));
    planCtx.fill();
    planCtx.stroke();
    planCtx.beginPath();
    planCtx.moveTo(-value759 * 0.44, -value760 * 0.18);
    planCtx.lineTo(value759 * 0.44, -value760 * 0.18);
    planCtx.stroke();
    for (const factor of [-0.28, 0, 0.28]) {
      planCtx.beginPath();
      planCtx.arc(value759 * factor, value760 * 0.38, Math.max(2, value760 * 0.12), 0, Math.PI * 2);
      planCtx.stroke();
    }
    ...
    planCtx.fill();
    planCtx.stroke();
    planCtx.strokeRect(-value759 * 0.43, -value760 * 0.34, value759 * 0.86, value760 * 0.68);
    for (const value348 of [-0.25, 0.18]) {
      planCtx.beginPath();
      planCtx.arc(value759 * value348, value760 * (value348 > 0 ? 0.08 : -0.06), Math.max(2, value760 * 0.08), 0, Math.PI * 2);
      planCtx.stroke();
    }
  } else if (item.type === "coffeetable") {
    const value349 = Math.min(value759, value760) * 0.32;
    const value350 = value349 * 0.7;
    planCtx.beginPath();
    planCtx.arc(-value759 * 0.16, value760 * 0.08, value349, 0, Math.PI * 2);
    planCtx.fill();
    planCtx.stroke();
    planCtx.beginPath();
    planCtx.arc(value759 * 0.24, -value760 * 0.2, value350, 0, Math.PI * 2);
    planCtx.fill();
    planCtx.stroke();
  } else if (roundTableTypes.has(item.type)) {
    planCtx.beginPath();
    planCtx.ellipse(0, 0, value759 * 0.32, value760 * 0.32, 0, 0, Math.PI * 2);
    planCtx.fill();
    planCtx.stroke();
    for (const [value317, value318] of [[-0.38, 0], [0.38, 0], [0, -0.38], [0, 0.38]]) {
      planCtx.beginPath();
      planCtx.roundRect(value759 * value317 - value759 * 0.085, value760 * value318 - value760 * 0.095, value759 * 0.17, value760 * 0.19, Math.min(value759, value760) * 0.035);
      planCtx.fill();
      planCtx.stroke();
    }
    if (hasRoundTableTurntable(item)) {
      planCtx.beginPath();
      planCtx.ellipse(0, 0, value759 * 0.27, value760 * 0.27, 0, 0, Math.PI * 2);
      planCtx.fill();
      planCtx.stroke();
    }
  } else if (item.type === "squarecoffeetable") {
    planCtx.beginPath();
    planCtx.roundRect(-value759 * 0.47, -value760 * 0.47, value759 * 0.94, value760 * 0.94, Math.min(value759, value760) * 0.08);
    planCtx.fill();
    planCtx.stroke();
    planCtx.strokeRect(-value759 * 0.33, -value760 * 0.38, value759 * 0.66, value760 * 0.76);
    planCtx.beginPath();
    planCtx.moveTo(-value759 * 0.08, -value760 * 0.38);
    planCtx.lineTo(-value759 * 0.08, value760 * 0.38);
    planCtx.moveTo(value759 * 0.08, -value760 * 0.38);
    planCtx.lineTo(value759 * 0.08, value760 * 0.38);
    planCtx.stroke();
    planCtx.fillStyle = "rgba(25, 34, 43, .58)";
    for (const value307 of [-0.39, 0.39]) {
      for (const value278 of [-0.34, 0.34]) {
        planCtx.beginPath();
        planCtx.arc(value759 * value307, value760 * value278, Math.max(1.5, Math.min(value759, value760) * 0.045), 0, Math.PI * 2);
        planCtx.fill();
      }
    }
  } else if (item.type === "floorlamp") {
    const value308 = -value759 * 0.34;
    const value309 = value759 * 0.31;
    const value310 = Math.min(value760 * 0.34, value759 * 0.13);
    const value311 = Math.min(value760 * 0.46, value759 * 0.14);
    planCtx.lineCap = "round";
    planCtx.lineWidth = selected ? 2.4 : 1.5;
    planCtx.beginPath();
    planCtx.moveTo(value308, 0);
    planCtx.lineTo(value309, 0);
    planCtx.stroke();
    planCtx.beginPath();
    planCtx.arc(value308, 0, value310, 0, Math.PI * 2);
    planCtx.fill();
    planCtx.stroke();
    planCtx.beginPath();
    planCtx.arc(value309, 0, value311, 0, Math.PI * 2);
    planCtx.fill();
    planCtx.stroke();
  } else if (item.type === "toilet") {
    planCtx.beginPath();
    planCtx.moveTo(-value759 * 0.42, -value760 * 0.42);
    planCtx.lineTo(value759 * 0.42, -value760 * 0.42);
    planCtx.bezierCurveTo(value759 * 0.48, -value760 * 0.08, value759 * 0.48, value760 * 0.28, 0, value760 * 0.48);
    planCtx.bezierCurveTo(-value759 * 0.48, value760 * 0.28, -value759 * 0.48, -value760 * 0.08, -value759 * 0.42, -value760 * 0.42);
    planCtx.closePath();
    planCtx.fill();
    planCtx.stroke();
  } else if (item.type === "squattoilet") {
    planCtx.beginPath();
    planCtx.roundRect(-value759 / 2, -value760 / 2, value759, value760, Math.min(value759, value760) * 0.12);
    planCtx.fill();
    planCtx.stroke();
    planCtx.beginPath();
    planCtx.ellipse(0, value760 * 0.04, value759 * 0.18, value760 * 0.31, 0, 0, Math.PI * 2);
    planCtx.stroke();
    for (const value263 of [-1, 1]) {
      planCtx.strokeRect(value263 * value759 * 0.38 - value759 * 0.07, -value760 * 0.25, value759 * 0.14, value760 * 0.5);
    }
  } else if (item.type === "urinal") {
    planCtx.beginPath();
    planCtx.roundRect(-value759 * 0.38, -value760 * 0.42, value759 * 0.76, value760 * 0.84, Math.min(value759, value760) * 0.28);
    planCtx.fill();
    planCtx.stroke();
    planCtx.beginPath();
    planCtx.ellipse(0, value760 * 0.04, value759 * 0.2, value760 * 0.28, 0, 0, Math.PI * 2);
    planCtx.stroke();
  } else if (item.type === "bathtub") {
    planCtx.beginPath();
    planCtx.roundRect(-value759 * 0.48, -value760 * 0.45, value759 * 0.96, value760 * 0.9, Math.min(value759, value760) * 0.36);
    planCtx.fill();
    planCtx.stroke();
    planCtx.beginPath();
    planCtx.ellipse(0, 0, value759 * 0.34, value760 * 0.29, 0, 0, Math.PI * 2);
    planCtx.stroke();
  } else if (item.type === "walllamp") {
    planCtx.beginPath();
    planCtx.arc(0, 0, Math.min(value759, value760) * 0.36, 0, Math.PI * 2);
    planCtx.fill();
    planCtx.stroke();
    planCtx.beginPath();
    planCtx.moveTo(-value759 * 0.42, value760 * 0.38);
    planCtx.lineTo(value759 * 0.42, value760 * 0.38);
    planCtx.stroke();
    ...
    planCtx.beginPath();
    planCtx.ellipse(0, 0, value759 * 0.46, value760 * 0.44, 0, 0, Math.PI * 2);
    planCtx.fill();
    planCtx.stroke();
    planCtx.fillStyle = "rgba(47, 58, 69, .82)";
    planCtx.beginPath();
    planCtx.roundRect(-value759 * 0.2, -value760 * 0.28, value759 * 0.4, value760 * 0.22, Math.min(value759, value760) * 0.08);
    planCtx.fill();
    planCtx.strokeStyle = "rgba(25, 34, 43, .5)";
    planCtx.beginPath();
    planCtx.moveTo(-value759 * 0.3, value760 * 0.36);
    planCtx.lineTo(-value759 * 0.3, value760 * 0.52);
    planCtx.moveTo(value759 * 0.3, value760 * 0.36);
    planCtx.lineTo(value759 * 0.3, value760 * 0.52);
    planCtx.stroke();
  } else if (item.type === "gaswaterheater") {
    planCtx.beginPath();
    planCtx.roundRect(-value759 * 0.46, -value760 * 0.46, value759 * 0.92, value760 * 0.92, Math.min(value759, value760) * 0.1);
    planCtx.fill();
    planCtx.stroke();
    planCtx.fillStyle = "rgba(47, 58, 69, .82)";
    planCtx.fillRect(-value759 * 0.22, -value760 * 0.26, value759 * 0.44, value760 * 0.17);
    planCtx.strokeStyle = "rgba(25, 34, 43, .5)";
    planCtx.beginPath();
    planCtx.moveTo(-value759 * 0.24, value760 * 0.44);
    planCtx.lineTo(-value759 * 0.24, value760 * 0.58);
    planCtx.moveTo(0, value760 * 0.44);
    planCtx.lineTo(0, value760 * 0.58);
    planCtx.moveTo(value759 * 0.24, value760 * 0.44);
    planCtx.lineTo(value759 * 0.24, value760 * 0.58);
    planCtx.stroke();
  } else if (item.type === "pipelinewaterpurifier") {
    planCtx.beginPath();
    planCtx.roundRect(-value759 * 0.46, -value760 * 0.46, value759 * 0.92, value760 * 0.92, Math.min(value759, value760) * 0.08);
    planCtx.fill();
    planCtx.stroke();
    planCtx.fillStyle = "rgba(31, 39, 45, .9)";
    planCtx.beginPath();
    planCtx.roundRect(-value759 * 0.4, -value760 * 0.35, value759 * 0.8, value760 * 0.28, Math.min(value759, value760) * 0.05);
    planCtx.fill();
    planCtx.strokeStyle = "rgba(231, 235, 236, .74)";
    planCtx.beginPath();
    planCtx.moveTo(-value759 * 0.12, -value760 * 0.18);
    planCtx.lineTo(value759 * 0.12, -value760 * 0.18);
    planCtx.stroke();
    planCtx.fillStyle = "rgba(47, 58, 69, .7)";
    for (const value150 of [-0.2, 0.2]) {
      planCtx.beginPath();
      planCtx.arc(value759 * value150, value760 * 0.18, Math.max(1.5, Math.min(value759, value760) * 0.055), 0, Math.PI * 2);
      planCtx.fill();
    }
    planCtx.strokeStyle = "rgba(25, 34, 43, .5)";
    planCtx.beginPath();
    planCtx.moveTo(-value759 * 0.26, value760 * 0.36);
    planCtx.lineTo(-value759 * 0.26, value760 * 0.5);
    planCtx.moveTo(value759 * 0.26, value760 * 0.36);
    planCtx.lineTo(value759 * 0.26, value760 * 0.5);
    planCtx.stroke();
  } else if (item.type === "tea_bar_machine") {
    planCtx.beginPath();
    planCtx.rect(-value759 * 0.46, -value760 * 0.46, value759 * 0.92, value760 * 0.92);
    planCtx.fill();
    planCtx.stroke();
    planCtx.fillStyle = "rgba(38, 44, 47, .86)";
    planCtx.fillRect(-value759 * 0.38, -value760 * 0.36, value759 * 0.76, value760 * 0.2);
    planCtx.strokeStyle = "rgba(25, 34, 43, .5)";
    planCtx.beginPath();
    planCtx.moveTo(-value759 * 0.46, value760 * 0.08);
    planCtx.lineTo(value759 * 0.46, value760 * 0.08);
    planCtx.moveTo(-value759 * 0.46, value760 * 0.3);
    planCtx.lineTo(value759 * 0.46, value760 * 0.3);
    planCtx.stroke();
    ...
    planCtx.beginPath();
    planCtx.ellipse(0, 0, value759 * 0.46, value760 * 0.46, 0, 0, Math.PI * 2);
    planCtx.fill();
    planCtx.stroke();
  } else {
    planCtx.beginPath();
    planCtx.roundRect(-value759 / 2, -value760 / 2, value759, value760, Math.min(6, value759 / 5, value760 / 5));
    planCtx.fill();
    planCtx.stroke();
  }
  ...
  if (item.type === "coffeetable") {
    const radius = Math.min(value759, value760) * 0.32;
    planCtx.beginPath();
    planCtx.arc(-value759 * 0.16, value760 * 0.08, radius * 0.32, 0, Math.PI * 2);
    planCtx.stroke();
    planCtx.beginPath();
    planCtx.arc(value759 * 0.24, -value760 * 0.2, radius * 0.23, 0, Math.PI * 2);
    planCtx.stroke();
  } else if (item.type === "squarecoffeetable") {
    planCtx.strokeRect(-value759 * 0.35, -value760 * 0.35, value759 * 0.7, value760 * 0.7);
  } else if (item.type === "bed") {
    planCtx.strokeRect(-value759 * 0.4, -value760 * 0.4, value759 * 0.8, value760 * 0.23);
    planCtx.beginPath();
    planCtx.moveTo(-value759 / 2, -value760 * 0.12);
    planCtx.lineTo(value759 / 2, -value760 * 0.12);
    planCtx.stroke();
  } else if (item.type === "sofa") {
    planCtx.beginPath();
    planCtx.moveTo(-value759 / 2, -value760 * 0.27);
    planCtx.lineTo(value759 / 2, -value760 * 0.27);
    planCtx.stroke();
    planCtx.strokeRect(-value759 * 0.42, -value760 * 0.12, value759 * 0.84, value760 * 0.43);
  } else if (item.type === "table") {
    for (const factor of [-0.38, 0.38]) {
      for (const factor2 of [-0.32, 0.32]) {
        planCtx.beginPath();
        planCtx.arc(value759 * factor, value760 * factor2, Math.max(1.5, Math.min(value759, value760) * 0.045), 0, Math.PI * 2);
        planCtx.stroke();
      }
    }
  } else if (roundTableTypes.has(item.type) && hasRoundTableTurntable(item)) {
    planCtx.beginPath();
    planCtx.arc(0, 0, Math.min(value759, value760) * 0.17, 0, Math.PI * 2);
    planCtx.stroke();
    ...
    planCtx.beginPath();
    planCtx.arc(0, 0, Math.min(value759, value760) * 0.26, 0, Math.PI * 2);
    planCtx.stroke();
    planCtx.beginPath();
    planCtx.moveTo(-value759 * 0.29, value760 * 0.36);
    planCtx.lineTo(value759 * 0.29, value760 * 0.36);
    planCtx.stroke();
  } else if (item.type === "gaswaterheater") {
    planCtx.beginPath();
    planCtx.moveTo(-value759 * 0.36, -value760 * 0.08);
    planCtx.lineTo(value759 * 0.36, -value760 * 0.08);
    planCtx.stroke();
    planCtx.beginPath();
    planCtx.moveTo(-value759 * 0.3, value760 * 0.36);
    planCtx.lineTo(value759 * 0.3, value760 * 0.36);
    planCtx.stroke();
  } else if (item.type === "pipelinewaterpurifier") {
    planCtx.beginPath();
    planCtx.moveTo(-value759 * 0.3, value760 * 0.34);
    planCtx.lineTo(value759 * 0.3, value760 * 0.34);
    planCtx.stroke();
    planCtx.beginPath();
    planCtx.arc(value759 * 0.02, value760 * 0.16, Math.min(value759, value760) * 0.08, 0, Math.PI * 2);
    planCtx.stroke();
  } else if (item.type === "tea_bar_machine") {
    planCtx.beginPath();
    planCtx.moveTo(-value759 * 0.38, -value760 * 0.02);
    planCtx.lineTo(value759 * 0.38, -value760 * 0.02);
    planCtx.moveTo(-value759 * 0.42, value760 * 0.2);
    planCtx.lineTo(value759 * 0.42, value760 * 0.2);
    planCtx.stroke();
  } else if (["dishwasher", "steamoven", "microwave"].includes(item.type)) {
    planCtx.beginPath();
    planCtx.moveTo(-value759 * 0.42, value760 * 0.3);
    planCtx.lineTo(value759 * 0.42, value760 * 0.3);
    planCtx.stroke();
  } else if (item.type === "glasspartition") {
    planCtx.strokeStyle = "rgba(183, 218, 231, .72)";
    planCtx.beginPath();
    planCtx.moveTo(-value759 * 0.46, 0);
    planCtx.lineTo(value759 * 0.46, 0);
    planCtx.stroke();
  } else if (item.type === "ricecooker") {
    planCtx.beginPath();
    planCtx.arc(0, -value760 * 0.04, Math.min(value759, value760) * 0.28, 0, Math.PI * 2);
    planCtx.stroke();
  } else if (item.type === "tv") {
    if (item.tvMountStyle === "mobile") {
      planCtx.fillStyle = "rgba(46, 53, 61, .88)";
      planCtx.fillRect(-value759 * 0.36, -value760 * 0.4, value759 * 0.72, value760 * 0.8);
      planCtx.strokeRect(-value759 * 0.08, -value760 * 0.42, value759 * 0.16, value760 * 0.84);
    } else if (item.tvMountStyle === "tabletop") {
      planCtx.strokeRect(-value759 * 0.3, -value760 * 0.34, value759 * 0.6, value760 * 0.68);
    }
    planCtx.fillStyle = item.screenEnabled === false ? "rgba(8, 12, 15, .8)" : "rgba(48, 113, 153, .86)";
    planCtx.fillRect(-value759 * 0.44, -value760 * 0.28, value759 * 0.88, value760 * 0.56);
    if (item.screenEnabled !== false) {
      planCtx.fillStyle = "rgba(255, 159, 54, .92)";
      planCtx.fillRect(-value759 * 0.37, -value760 * 0.18, value759 * 0.05, value760 * 0.36);
    }
  } else if (item.type === "plant") {
    planCtx.beginPath();
    planCtx.arc(0, 0, Math.min(value759, value760) * 0.31, 0, Math.PI * 2);
    planCtx.stroke();
  } else if (Vl.has(item.type)) {
    for (let value235 = 1; value235 < 10; value235 += 1) {
      const value231 = -value760 / 2 + value760 * value235 / 10;
      planCtx.beginPath();
      planCtx.moveTo(-value759 / 2, value231);
      planCtx.lineTo(value759 / 2, value231);
      planCtx.stroke();
    }
    planCtx.beginPath();
    planCtx.moveTo(0, value760 * 0.34);
    planCtx.lineTo(0, -value760 * 0.3);
    planCtx.lineTo(-Math.min(value759, value760) * 0.08, -value760 * 0.2);
    planCtx.moveTo(0, -value760 * 0.3);
    planCtx.lineTo(Math.min(value759, value760) * 0.08, -value760 * 0.2);
    planCtx.stroke();
  }
  if (selected) {
    ...
    for (const [fx, fy] of [[-value759 / 2, -value760 / 2], [value759 / 2, -value760 / 2], [value759 / 2, value760 / 2], [-value759 / 2, value760 / 2]]) {
      planCtx.fillRect(fx - value621 / 2, fy - value621 / 2, value621, value621);
      planCtx.strokeRect(fx - value621 / 2, fy - value621 / 2, value621, value621);
    }
    const value622 = -value760 / 2 - 17;
    planCtx.strokeStyle = "#ff9d2e";
    planCtx.beginPath();
    planCtx.moveTo(0, -value760 / 2);
    planCtx.lineTo(0, value622 + 4);
    planCtx.stroke();
    planCtx.fillStyle = "#111820";
    planCtx.beginPath();
    planCtx.arc(0, value622, 4, 0, Math.PI * 2);
    planCtx.fill();
    planCtx.stroke();
  }
  planCtx.restore();
...
function rotatedItemCorner(planPoint, arg1, arg2) {
  ...
  return {
    x: planPoint.x + arg1 * Math.cos(value) - arg2 * Math.sin(value),
    y: planPoint.y + arg1 * Math.sin(value) + arg2 * Math.cos(value)
  };
}
function itemPlanBounds(size) {
  ...
  return {
    corners: [{
      ...
      ...planPoint,
      point: rotatedItemCorner(size, planPoint.x * ipbV2, planPoint.y * ipbV3),
      opposite: rotatedItemCorner(size, -planPoint.x * ipbV2, -planPoint.y * ipbV3)
    })),
    rotationStem: rotatedItemCorner(size, 0, -ipbV3),
    rotationHandle: rotatedItemCorner(size, 0, -ipbV3 - 17 / Math.max(planView.zoom, 0.01))
  };
}
function beginItemDrag(arg0) {
  ...
  const corner = controls.corners.find(point => distance(arg0, point.point) <= value);
  if (corner) {
    return {
      type: "resize-item",
      item,
...
function marqueeSelectHits(planPoint, point2) {
  ...
  for (const item of floorScene2.items) {
    ...
    const some2 = [[-mshV5, -mshV6], [mshV5, -mshV6], [mshV5, mshV6], [-mshV5, mshV6]].map(([arg0, arg02]) => ({
      x: item.x + arg0 * cos - arg02 * sin,
      y: item.y + arg0 * sin + arg02 * cos
    }));
    if (pointInBounds(item, minX) || some2.some(point3 => pointInBounds(point3, minX)) || some.some(arg0 => pointInRotatedRectangle(arg0, item, value))) {
      list.push({
        kind: "item",
        id: item.id
      });
...
function ensureMeasureCanvas() {
  if (!planCanvas2.width || !planCanvas2.height || !measureCtx) {
    ...
    if (fn.width !== planCanvas2.width) {
      fn.width = planCanvas2.width;
    }
    if (fn.height !== planCanvas2.height) {
      fn.height = planCanvas2.height;
    }
    measureCtx.setTransform(1, 0, 0, 1, 0, 0);
    measureCtx.clearRect(0, 0, fn.width, fn.height);
...
function blitMeasureOverlay({
  ...
  if (!fn.width || !fn.height || fn.width !== planCanvas2.width || fn.height !== planCanvas2.height) {
    return false;
  }
  ...
  planCtx.drawImage(fn, Math.round(arg0 * value), Math.round(arg02 * fitHeight));
  planCtx.restore();
  return true;
}
...
function drawPlan() {
  ...
  if (Tt && at) {
    const value623 = onPlanPointerDown(at);
    ...
    drawPlanPoint(at.point, value623 ? "#76cfa1" : at.kind ? "#43d2e6" : "#ff9d2e", value623 ? 5 : 3.5);
...
function placeCatalogItemAt(arg0) {
  ...
  for (const item of [...floorScene2.items].reverse()) {
    if (lightItemTypes2.has(item.type) === flag && pointInRotatedRectangle(arg0, item, value)) {
      return {
        kind: "item",
        id: item.id
      };
  ...
  for (const size of [...floorScene2.windows].reverse()) {
    const isStart = wallAttachmentWorldPoint(size);
    if (isStart && projectPointToSegment(arg0, isStart.start, isStart.end).distance <= 10 / planView.zoom) {
      return {
        kind: "window",
        id: size.id
      };
  ...
  for (const size of [...floorScene2.doors].reverse()) {
    const isStart = wallAttachmentWorldPoint(size);
    if (isStart && projectPointToSegment(arg0, isStart.start, isStart.end).distance <= 12 / planView.zoom) {
      return {
        kind: "door",
        id: size.id
      };
  ...
  for (const wall of [...floorScene2.walls].reverse()) {
    ...
    if (projectPointToSegment(arg0, wall.start, wall.end).distance <= maxValue) {
      return {
        kind: "wall",
        id: wall.id
      };
...
function updateProgressChecklist() {
  ...
  const layerKeys = ["background", "scale", "walls", "items", "lights", "export"].find(arg0 => !value[arg0]) || "export";
...
function studioLayoutMetrics() {
  ...
  const slmV3 = detailsResizer.parentElement?.getBoundingClientRect().height || 14;
  ...
  return {
    ...
    maximumHeightRatio: clamp((value - slmV3 - 170) / value, 0.5, 0.94),
...
function syncSnapUi() {
  ...
  value930(snapTolerance, clamp(Math.round(finite(floorScene2.settings.snapTolerance, 13)), 6, 24));
...
function onDetailsResizePointerMove(event) {
  ...
  floorScene2.settings.previewPanelRatio = clamp(detailsResizeDrag.startPreviewRatio + odrpV4, value.minimumHeightRatio, value.maximumHeightRatio);
  floorScene2.settings.detailsPanelWidthRatio = clamp(detailsResizeDrag.startWidthRatio - odrpV5, value.minimumWidthRatio, value.maximumWidthRatio);
...
function refreshStudioPanels() {
  ...
  value930(globalWallHeight, floorScene2.settings.wallHeight.toFixed(2));
  value930(globalWallThickness, floorScene2.settings.wallThickness.toFixed(2));
  value930(globalWallOpacity, Math.round(floorScene2.settings.wallOpacity * 100));
...
function updateSelectionInspector() {
  ...
  if (!!lightGroup && !!selection) {
    ...
    if (selection.kind === "wall") {
      ...
      value930(selectEl("#wall-length"), wallLengthMeters(lightGroup, pixelsPerMeter() || 1).toFixed(2) + " m");
      value930(selectEl("#wall-height"), lightGroup.height.toFixed(2));
      value930(selectEl("#wall-thickness"), lightGroup.thickness.toFixed(2));
      ...
      value930(selectEl("#wall-opacity"), Math.round((flag2 ?? floorScene2.settings.wallOpacity) * 100));
      ...
      value930(selectEl("#window-width"), lightGroup.width.toFixed(2));
      value930(selectEl("#window-height"), lightGroup.height.toFixed(2));
      value930(selectEl("#window-sill"), lightGroup.sill.toFixed(2));
      selectEl("#window-divider").value = lightGroup.hasDivider === false ? "without" : "with";
      syncStudioSelect(selectEl("#window-divider"));
      value930(selectEl("#window-position"), Math.round(lightGroup.t * 100) + "%");
      ...
      value930(selectEl("#door-width"), lightGroup.width.toFixed(2));
      value930(selectEl("#door-height"), lightGroup.height.toFixed(2));
      value930(selectEl("#door-position"), Math.round(lightGroup.t * 100) + "%");
      ...
      value930(selectEl("#railing-width"), lightGroup.width.toFixed(2));
      value930(selectEl("#railing-height"), lightGroup.height.toFixed(2));
      value930(selectEl("#railing-position"), Math.round(lightGroup.t * 100) + "%");
    } else {
      ...
      if (flag2) {
        value930(selectEl("#label-title"), lightGroup.title || "家庭总览");
        value930(selectEl("#label-title-spacing"), Math.round(clamp(finite(lightGroup.titleSpacing, 1.05), 0, 1.8) * 100));
        value930(selectEl("#label-subtitle"), lightGroup.subtitle || "HOME PLAN");
        value930(selectEl("#label-subtitle-spacing"), Math.round(clamp(finite(lightGroup.subtitleSpacing, 0.08), 0, 0.6) * 100));
        value930(selectEl("#label-line-length"), Math.round(clamp(finite(lightGroup.lineLength, 0.86), 0.3, 1) * 100));
      }
      if (flag3) {
        ...
        value930(selectEl("#light-temperature"), Math.round(clamp(finite(lightGroup.lightTemperature, temperature.temperature), 2200, 6500)));
        value930(selectEl("#light-brightness"), Math.round(clamp(finite(lightGroup.lightBrightness, temperature.brightness), 0, 100)));
        value930(selectEl("#light-range"), clamp(finite(lightGroup.lightRange, temperature.range), 0.5, 10).toFixed(1));
        selectEl("#light-angle").max = String(defaultItemDepth(lightGroup.type));
        value930(selectEl("#light-angle"), Math.round(clamp(finite(lightGroup.lightAngle, temperature.angle), 15, defaultItemDepth(lightGroup.type))));
        ...
        value930(selectEl("#item-vertical-rotation"), Math.round(value * 100) / 100);
        value930(itemStripRoll, Math.round(clamp(finite(lightGroup.stripRollRotation, 0), 0, 360) * 100) / 100);
      ...
      value930(selectEl("#item-x"), (lightGroup.x / (pixelsPerMeter() || 1)).toFixed(2));
      value930(selectEl("#item-y"), (lightGroup.y / (pixelsPerMeter() || 1)).toFixed(2));
      value930(selectEl("#item-width"), lightGroup.width.toFixed(2));
      ...
      value930(selectEl("#item-height"), lightGroup.type === "rug" ? lightGroup.height.toFixed(3) : lightGroup.height.toFixed(2));
      value930(selectEl("#item-depth"), lightGroup.depth.toFixed(2));
      value930(selectEl("#item-elevation"), (lightGroup.elevation || 0).toFixed(2));
      value930(selectEl("#item-rotation"), Math.round(lightGroup.rotation * 100) / 100);
    }
  }
}
...
function setActiveTool(arg0) {
  ...
  const flag = activeTool === "wall" && arg0 !== "wall" && Ar > 0;
  activeTool = arg0;
  planCanvas2.dataset.tool = arg0;
  planCanvas2.style.cursor = "";
  for (const element of Zd) {
    element.classList.toggle("active", element.dataset.tool === arg0);
  }
  [activeToolLabel2.textContent, toolHelp.textContent] = assetCategory === "light" ? ["灯光编辑", "户型已锁定；框选多盏灯后可整体拖动，Shift 锁轴，Option/Alt 复制"] : toolHelpText[arg0];
  yr.hidden = arg0 !== "wall" || !Tt;
  if (arg0 !== "wall") {
    resetWallDrawing2();
  }
  if (arg0 !== "scale") {
...
function Cs(arg0, arg1, width12 = {}) {
  const lastMotionRenderAt = furnitureCatalog[arg0];
  if (!lastMotionRenderAt || !requireCalibration()) {
    return;
  }
  const temperature2 = defaultLightPresets[arg0] || defaultLightPresets.downlight;
  const id2 = lightItemTypes2.has(arg0) ? ensureDefaultLightGroup() : null;
  pushHistory();
  const id3 = {
    id: makeId("item"),
    type: arg0,
    ...
    ...(arg0 === "tv" ? {
      screenEnabled: true,
      screenLayerName: "电视画面 " + (ms().length + 1),
    ...
    ...(stairItemTypes.has(arg0) ? {
      stairDirection: "right"
    } : {}),
    ...(arg0 === "shoecabinet" ? {
      shoeCabinetMirrored: false
    } : {}),
    ...(roundTableTypes.has(arg0) ? {
      roundTableTurntable: false
    } : {}),
    ...(lightItemTypes2.has(arg0) ? {
      lightGroupId: id2.id,
      verticalRotation: 0,
      ...(arg0 === "striplight" ? {
        stripRollRotation: 0,
...
function cloneSelectedItems() {
  ...
  return floorScene2.items.filter(id => value.has(id.id) && lightItemTypes2.has(id.type) === csiV2);
}
...
function applyPreviewEnvironment() {
  ...
  const value761 = yt ? 0.5 : 1;
  if (hemisphereLight) {
    hemisphereLight.color.setHex(14278376);
    hemisphereLight.groundColor.setHex(1909296);
    hemisphereLight.intensity = exposure.hemisphereIntensity * value761;
  }
  if (ambientLight) {
    ambientLight.color.setHex(9673384);
    ambientLight.intensity = exposure.ambientIntensity * value761;
  }
  if (previewSpotLight) {
    previewSpotLight.color.setHex(15922426);
    previewSpotLight.intensity = exposure.mainIntensity * value761;
...
function syncBaseLightingControls() {
  for (const el2 of baseLightControlEls) {
    ...
    value930(el2, flag ? Math.round(toFixed) : Number(toFixed.toFixed(2)));
  }
}
function applyBaseLighting(arg0) {
  const value762 = baseLighting;
  baseLighting = normalizeBaseLighting(arg0);
  syncBaseLightingControls();
  applyPreviewEnvironment();
  requestRender({
    shadows: Object.keys(DEFAULT_BASE_LIGHTING).some(arg52 => value762[arg52] !== baseLighting[arg52])
  });
}
...
function isPreviewQualityReady() {
  if (yt) {
    ...
    return previewQualityReady && !floorSelectionQuery && !Wo && !$o;
  }
}
...
function syncCameraModeButtons(arg0 = getCameraProjectionMode()) {
  for (const element of cameraModeEls) {
    element.classList.toggle("active", element.dataset.cameraMode === arg0);
  }
  syncCameraFocalControls(arg0);
}
function ui(arg0 = cameraViewMode()) {
...
function syncCameraFocalControls(arg0 = getCameraProjectionMode()) {
  for (const disabled of cameraFocalLengthEls) {
    value930(disabled, Math.round(getCameraFocalLength()));
    disabled.disabled = arg0 !== "perspective";
    disabled.closest(".camera-focal-control")?.classList.toggle("is-disabled", disabled.disabled);
  }
}
function applyCameraFocalLength(isPerspectiveCamera = camera2, arg168 = getCameraFocalLength()) {
  if (isPerspectiveCamera?.isPerspectiveCamera) {
    isPerspectiveCamera.setFocalLength(clamp(finite(arg168, 50), 18, 120));
  }
}
...
function computeStudioPixelRatio(flag = false) {
  ...
  if (yt) {
    const value624 = Math.min(window.devicePixelRatio || 1, 1.6) * ur;
    if (flag) {
      return Math.min(value624, 1);
    } else {
      return value624;
    }
  }
  ...
  if (isStageEmbed && (flag || flag2)) {
    ...
    if (value625 > value626) {
      value = Math.min(value, clamp(Math.sqrt(value626 / value625) * 0.85, 0.5, 0.85));
    }
  }
  return value;
}
...
function Hc(isPerspectiveCamera, arg169) {
  if (Number.isFinite(arg169)) {
    isPerspectiveCamera.push(arg169);
...
function averagePerfSample(list) {
  if (list.length) {
    return list.reduce((arg0, arg1) => arg0 + arg1, 0) / list.length;
  } else {
    return null;
  }
}
function ks(list, arg1) {
  if (!list.length) {
    return null;
  }
  const length24 = [...list].sort((arg66, arg67) => arg66 - arg67);
  const value763 = Math.min(length24.length - 1, Math.max(0, Math.ceil(length24.length * arg1) - 1));
  return length24[value763];
}
function fitCameraToSelection(list, arg170 = 1) {
  if (Number.isFinite(list)) {
    return Number(list.toFixed(arg170));
  } else {
    return null;
  }
}
...
function collectGpuTimingResults() {
  ...
  for (const value of gpuQueriesPending.gpuQueriesPending) {
    ...
    Hc(gpuQueriesPending.gpuRenderTimes, queryParameter / 1000000);
    isGetParameter.deleteQuery(value);
...
function fitCameraToSelection2(toFixed, Number2) {
  ...
  const value764 = performance.now();
  try {
    return Number2();
  } finally {
    const totalMs = performance.now() - value764;
...
function setPerfMotionActive(lastTick, flag = true) {
  const perfStats = isPerfDiagnosticsEnabled && perfStats.floorSwitch;
  if (!!perfStats && !(performance.now() > perfStats.until)) {
    if (!flag) {
      perfStats.lastTick = null;
      return;
    }
    if (perfStats.lastTick != null) {
      const value479 = lastTick - perfStats.lastTick;
      perfStats.maxTickMs = Math.max(perfStats.maxTickMs || 0, value479);
      if (value479 > 50) {
        perfStats.tickLongFrames = (perfStats.tickLongFrames || 0) + 1;
      }
    }
    perfStats.lastTick = lastTick;
    perfStats.checksSinceRender = (perfStats.checksSinceRender || 0) + 1;
  }
}
function qp(arg0, arg1, arg171) {
  if (!isPerfDiagnosticsEnabled) {
    return;
  }
  ...
  if (left && last <= left.until) {
    ...
    left.cpuMs += arg1;
    left.maxRenderCpuMs = Math.max(left.maxRenderCpuMs || 0, arg1);
    if (arg1 > 50) {
      left.slowRenderCount = (left.slowRenderCount || 0) + 1;
    }
  }
  if (arg171) {
    Hc(value.cpuRenderTimes, arg1);
    if (value.lastMotionRenderAt > 0) {
      const value480 = arg0 - value.lastMotionRenderAt;
      if (value480 >= 2 && value480 <= 250) {
        Hc(value.frameIntervals, value480);
      }
    }
    value.lastMotionRenderAt = arg0;
  } else {
  ...
  previewScene2?.traverse(light => {
    if (light.isMesh) {
      ...
      if (light.userData.runtimeFurnitureStats) {
        for (const value357 of Object.keys(meshes)) {
          meshes[value357] += light.userData.runtimeFurnitureStats[value357];
        }
      }
...
function publishPerfHud(arg0 = performance.now()) {
  ...
  const flag2 = ks(frameIntervals.frameIntervals, 0.5);
  ...
  const frame = {
    ...
    frame: {
      ...
      p95Ms: fitCameraToSelection(ks(frameIntervals.frameIntervals, 0.95), 2)
    },
    cpuRenderMs: {
      ...
      p95: fitCameraToSelection(ks(frameIntervals.cpuRenderTimes, 0.95), 2)
    },
    gpuRenderMs: {
      ...
      p95: fitCameraToSelection(ks(frameIntervals.gpuRenderTimes, 0.95), 2)
    },
    render: {
      calls: Number(domElement.dataset.renderCalls || 0),
      triangles: Number(domElement.dataset.renderTriangles || 0),
...
let Ds = null;
function capturePreviewCanvas(arg0, arg1, arg172) {
  Ds?.cancel();
  ...
  const imageData = () => !frame.cancelled && arg172();
  const arg02 = () => {
    ...
    window.removeEventListener("pagehide", arg02);
    window.removeEventListener("pointerdown", arg02, true);
    window.removeEventListener("wheel", arg02, true);
    blob?.removeEventListener("start", arg02);
    blob?.removeEventListener("change", arg02);
    if (arg1) {
      arg1.width = arg1.height = 0;
      arg1 = null;
    }
    if (Ds === frame) {
      Ds = null;
  ...
  frame.cancel = arg02;
  Ds = frame;
  const value765 = arg79 => window.HABridgeLog?.error(arg79, {
  ...
  const value766 = () => {
    ...
    Promise.resolve().then(() => {
      if (imageData()) {
        return renderCache2?.write(arg0, arg1, imageData);
      }
    }).catch(value765).finally(arg02);
  };
  try {
    window.addEventListener("pagehide", arg02, {
      once: true
    });
    window.addEventListener("pointerdown", arg02, {
      capture: true,
      passive: true
    });
    window.addEventListener("wheel", arg02, {
      capture: true,
      passive: true
    });
    blob?.addEventListener("start", arg02);
    blob?.addEventListener("change", arg02);
      ...
      frame.timer = window.setTimeout(() => {
        ...
        try {
          if (typeof window.requestIdleCallback == "function") {
            frame.idle = window.requestIdleCallback(value766);
          } else {
            value766();
          }
        } catch (value298) {
...
async function finishStageSessionWarmup() {
  ...
  if (!Vo || renderCache2?.closed || !renderer || stageSession || previewOrbitLocked || isLeavingStudio || isBakingLightCache || isCapturingFrame || isStageWarmup || Wo || $o || rs) {
    return;
  }
  ...
  const onComplete = () => Vo && !renderCache2?.closed && !hasPendingModelLoads() && value === lightCacheEpoch && !stageSession && !previewOrbitLocked && !isLeavingStudio && !isCapturingFrame && !isStageWarmup && !Wo && !$o && !rs;
...
function Qp(arg0, arg1 = LIGHT_CACHE_TILE_MS) {
  const value = [...new Set(arg0)].filter(Boolean);
  if (!value.length) {
    return;
  }
  const flag = value.map(arg68 => ({
    groupId: previewScopedItemKey(activeFloorId, arg68),
    from: clamp(finite(pendingModelLoads.get(previewScopedItemKey(activeFloorId, arg68)), findLightGroupById(arg68)?.enabled === false ? 0 : 1), 0, 1),
    to: findLightGroupById(arg68)?.enabled === false ? 0 : 1
  }));
  ...
  const nowResult = arg80 => {
    const value550 = clamp((arg80 - list) / arg1, 0, 1);
    const value551 = value550 * value550 * (3 - value550 * 2);
    for (const from of flag) {
      pendingModelLoads.set(from.groupId, from.from + (from.to - from.from) * value551);
    }
...
function findLightGroupById(arg0) {
  return floorScene2.lightGroups?.find(item => item.id === arg0) || null;
}
function Jp(arg0, arg173 = LIGHT_CACHE_TILE_MS) {
  if (!worldGroup) {
    return false;
  }
  const value = new Set(arg0);
  const wrcmV2 = isPreviewQualityReady() && !stageSession;
  const wrcmV3 = [];
  worldGroup.traverse(userData21 => {
    if (!userData21.isLight || !value.has(userData21.userData?.lightGroupId)) {
      return;
    }
    const to3 = findLightGroupById(userData21.userData.lightGroupId)?.enabled !== false && !wrcmV2 ? finite(userData21.userData.lightOnIntensity, 0) : 0;
    if (to3 > 0) {
  ...
  const value767 = arg82 => {
    const value552 = clamp((arg82 - flag) / arg173, 0, 1);
    const value553 = value552 * value552 * (3 - value552 * 2);
    for (const from2 of wrcmV3) {
      from2.object.intensity = from2.from + (from2.to - from2.from) * value553;
    }
    updateLightPreview();
    if (value552 < 1) {
      is = requestAnimationFrame(value767);
    } else {
      is = 0;
      let value418 = false;
      for (const to of wrcmV3) {
        if (!(to.to > 0)) {
...
function collectWorldItemKeys() {
  ...
  worldGroup?.traverse(object3d => {
    ...
    const value = layerScopedKey(object3d.userData?.lightFloorId, arg1);
    if (!map.has(value)) {
      map.set(value, []);
    }
...
function ensureWorldItemsCached(entry) {
  ...
  if (entry.every(itemKey => has.has(itemKey.itemKey))) {
    return has;
  }
  if (isStageEmbed) {
    return em(entry, has);
...
function em(arg0, has13) {
  const value768 = floorScene2;
  const value769 = activeFloorId;
  const value770 = residentCacheMode;
  const value771 = getPreviewFloorMode2() === "all";
  try {
    residentCacheMode = true;
    for (const {
      floor: id2,
      ...
      if (has13.has(value554) || !value771 && id2.id !== value769) {
        continue;
      }
      const add = value771 ? worldGroup?.children.find(userData6 => userData6.userData?.floorId === id2.id) : worldGroup;
      if (!add) {
        continue;
      }
      floorScene2 = id2.scene;
      activeFloorId = id2.id;
      const value481 = pixelsPerMeter();
      if (!value481) {
        continue;
      }
      const minX = getPreviewFloorMode();
      const value482 = value771 ? finite(id2.originX, 0) : (minX.minX + minX.maxX) / 2;
      const value483 = value771 ? finite(id2.originY, 0) : (minX.minY + minX.maxY) / 2;
      const userData17 = new THREE.Group();
      buildWallCornerCaps(userData17, x28, shadowCastingLightIdSet());
      userData17.position.set((x28.x - value482) / value481, x28.elevation || 0, (x28.y - value483) / value481);
      ...
      if (push5.length) {
        has13.set(value554, push5);
      }
    }
  } finally {
    floorScene2 = value768;
    activeFloorId = value769;
    residentCacheMode = value770;
  }
  Ht(worldGroup, {
    rebuildAtlas: false
  });
...
function setGroupVisibilityByKey(arg0, arg174 = "") {
  for (const [value697, value698] of arg0) {
    const visible10 = value697 === arg174;
    for (const shadow4 of value698) {
...
function waitTwoAnimationFrames() {
  ...
  return new Promise(arg0 => requestAnimationFrame(() => requestAnimationFrame(arg0)));
}
...
function yieldToScheduler() {
  if (globalThis.scheduler?.yield) {
    ...
    return new Promise(arg0 => requestAnimationFrame(() => arg0()));
  }
}
function yieldToIdle() {
  if (globalThis.scheduler?.yield) {
    ...
    return new Promise(arg0 => requestIdleCallback(() => arg0(), {
      timeout: 80
    }));
  } else {
    return new Promise(arg0 => requestAnimationFrame(() => arg0()));
...
function setPreviewPixelRatio(arg175, {
  preserveLightCache: preserveLightCache3 = false
} = {}) {
  if (!renderer || stageSession && !isAutoDiagramEmbed2) {
    return;
  }
  const value = stageSession ? stageEmbedPixelRatio(arg175) : computeStudioPixelRatio(arg175);
...
function syncFloorCameraChrome() {
  ...
  value930(previewFloorGapInput, finite(projectDoc2?.previewFloorGap, 3).toFixed(1));
...
function setActiveFixedCameraView(arg0) {
  if (getPreviewFloorMode2() === "all") {
    projectDoc2.combinedFixedCameraView = arg0;
  } else {
    floorScene2.settings.fixedCameraView = arg0;
  }
}
...
const sm = 0.32;
const lm = 0.006;
function getCameraPose(isPerspectiveCamera2 = camera2, arg176 = orbitControls?.target) {
  if (!isPerspectiveCamera2 || !arg176) {
    return false;
  }
  const value772 = Math.max(isPerspectiveCamera2.position.distanceTo(arg176), 1);
  const near = isPerspectiveCamera2.isPerspectiveCamera ? clamp(value772 * lm, am, sm) : 0.02;
  const far = Math.max(value772 * (isPerspectiveCamera2.isPerspectiveCamera ? 8 : 5), 100);
...
function resetOrbitTarget(arg0, arg177) {
  if (arg0?.isOrthographicCamera) {
    return Math.abs(arg0.top - arg0.bottom) / Math.max(arg0.zoom || 1, 0.000001);
  }
  if (arg0?.isPerspectiveCamera) {
    const value628 = Math.max(arg0.position.distanceTo(arg177), 0.0001);
    const value629 = THREE.MathUtils.degToRad(arg0.getEffectiveFOV());
    return value628 * 2 * Math.tan(value629 / 2);
  }
  return 10;
}
...
function nudgeCamera(arg0, preserveView = {}) {
  const value = arg0 === "top" ? "top" : "free";
  ...
  if (camera2.isPerspectiveCamera) {
    ...
    const value630 = THREE.MathUtils.degToRad(camera2.getEffectiveFOV());
    const value631 = Math.max(vector3 / (Math.tan(value630 / 2) * 2), 8);
    camera2.position.set(x45.x, x45.y + value631, x45.z);
  } else {
...
function setCameraProjectionMode(view, viewportAspect = {}) {
  const value773 = view === "perspective" ? "perspective" : "orthographic";
  syncCameraModeButtons(value773);
  if (!renderer) {
    return;
  }
  if (value773 === "perspective" == !!camera2?.isPerspectiveCamera) {
    applyCameraFocalLength();
    syncOrbitControls();
    return;
  }
  const value774 = viewportAspect.preserveView !== false;
  const userData36 = camera2;
  const value775 = orbitControls?.target.clone() || new THREE.Vector3(0, 0.6, 0);
  const length25 = userData36 ? userData36.position.clone().sub(value775) : new THREE.Vector3(1.12, 1.42, 1.2);
  const value776 = Math.max(length25.length(), 2);
  const value777 = length25.lengthSq() > 1e-8 ? length25.normalize() : new THREE.Vector3(1.12, 1.42, 1.2).normalize();
  ...
  const frameSize = value774 && userData36 ? resetOrbitTarget(userData36, value775) : userData36?.userData.frameSize || 10;
  orbitControls?.dispose();
  if (value773 === "perspective") {
    camera2 = new THREE.PerspectiveCamera(36, viewportAspect2, 0.02, 200);
    applyCameraFocalLength(camera2);
    const value632 = frameSize / (Math.tan(THREE.MathUtils.degToRad(camera2.getEffectiveFOV()) / 2) * 2);
    const value633 = value774 ? value632 : value776;
    camera2.position.copy(value775).addScaledVector(value777, Math.max(value633, 2));
  } else {
    camera2 = new THREE.OrthographicCamera(-5, 5, 5, -5, 0.02, 200);
    camera2.position.copy(value775).addScaledVector(value777, value776);
  ...
  getCameraPose(camera2, value775);
  camera2.lookAt(value775);
  camera2.updateProjectionMatrix();
  orbitControls = createOrbitControls(camera2);
  orbitControls.target.copy(value775);
  syncOrbitControls();
  ...
  try {
    ...
    if (yt) {
      ...
      shadowAtlas = createSpotShadowAtlasController({
        ...
        requestFrame: updateLightPreview,
        canBuild: () => !document.hidden && !stageSession && !previewOrbitLocked && !isStageWarmup && !Wo && !$o && !isLeavingStudio && !isBakingLightCache && externalModels2.modelLoadState().active === 0 && externalModels2.modelLoadState().queued === 0
      });
    }
    applyPreviewEnvironment();
    ...
    const handler = (param = performance.now()) => {
      ...
      renderer.render(previewScene2, camera2);
      const iprV10 = isPerfDiagnosticsEnabled ? performance.now() - iprV9 : 0;
      endGpuTimingQuery(beginGpuTimingQueryResult);
      qp(param, iprV10, iprV7);
    ...
    if (isStageEmbed) {
      demandFrameLoop = createDemandFrameLoop({
        ...
        step(arg34) {
          const value358 = handler(arg34);
          renderer.domElement.dataset.renderFrameChecks = String(demandFrameLoop.stats.frames);
          return value358;
        }
      });
      const value484 = () => demandFrameLoop.wake();
      const value485 = () => {
        nowResult = performance.now();
        const value398 = !document.hidden && Vo;
        demandFrameLoop.setAvailable(value398);
        if (value398) {
          updateLightPreview();
          if (previewQualityJustBecameReady) {
            scheduleAdaptiveQuality();
      ...
      renderer.domElement.addEventListener("hb-i3d-parent-visibility", value486);
      for (const value451 of ["pointerdown", "pointermove", "pointerup", "pointercancel", "wheel", "keydown", "keyup"]) {
        renderer.domElement.addEventListener(value451, value484, {
          passive: true
        });
      }
      document.addEventListener("visibilitychange", value485);
      window.addEventListener("pagehide", () => {
        demandFrameLoop.dispose();
        document.removeEventListener("visibilitychange", value485);
        shadowAtlas?.dispose?.();
        studioReady?.dispose();
        renderCache?.dispose();
        renderer.domElement.removeEventListener("hb-i3d-parent-visibility", value486);
        for (const value359 of ["pointerdown", "pointermove", "pointerup", "pointercancel", "wheel", "keydown", "keyup"]) {
          renderer.domElement.removeEventListener(value359, value484);
        }
      }, {
        once: true
      });
...
function focusCameraOnPoint(arg0, arg1, left2 = camera2) {
  if (!left2?.isOrthographicCamera) {
    return;
  }
  const blob = Math.max(arg0, 1) / 2;
  if (arg1 >= 1) {
    left2.left = -blob * arg1;
    left2.right = blob * arg1;
    left2.top = blob;
    left2.bottom = -blob;
  } else {
    left2.left = -blob;
    left2.right = blob;
    left2.top = blob / Math.max(arg1, 0.1);
    left2.bottom = -blob / Math.max(arg1, 0.1);
  }
...
function onPreviewContainerResize() {
  ...
  const opcrV4 = isStageEmbed ? camera2.projectionMatrix.elements.join(",") : "";
  if (!flag) {
  ...
  if (!flag || opcrV4 !== camera2.projectionMatrix.elements.join(",")) {
    requestRender();
  }
}
...
function pi(arg0, flag = arg0?.viewportAspect || 1) {
  if (!!arg0 && !!renderer) {
    setCameraProjectionMode(arg0.mode, {
      preserveView: false
    });
    camera2.position.copy(arg0.position);
    camera2.up.copy(arg0.up);
    camera2.zoom = arg0.zoom || 1;
    camera2.near = arg0.near;
    camera2.far = arg0.far;
    camera2.userData.frameSize = arg0.frameSize;
    camera2.userData.viewportAspect = flag;
    camera2.userData.cameraView = arg0.cameraView || cameraViewMode();
    camera2.userData.topRotation = arg0.topRotation || 0;
    if (camera2.isPerspectiveCamera) {
      camera2.fov = arg0.fov;
      camera2.aspect = flag;
    } else {
      focusCameraOnPoint(arg0.frameSize, flag, camera2);
    }
    camera2.lookAt(arg0.target);
    camera2.updateProjectionMatrix();
    orbitControls.target.copy(arg0.target);
    syncOrbitControls();
    orbitControls.update();
  }
}
...
function syncExportResolutionLabel() {
  ...
  mg.textContent = serlV2 <= 32 && serlV3 <= 32 ? serlV2 + " : " + serlV3 : (width / height).toFixed(2) + " : 1";
...
function resizeStageEmbedViewport() {
  ...
  camera2.userData.viewportAspect = arg1;
  if (camera2.isOrthographicCamera) {
    focusCameraOnPoint(camera2.userData.frameSize || 10, arg1, camera2);
  } else {
    camera2.aspect = arg1;
    applyCameraFocalLength();
  }
...
function onExportDimensionInput(arg0, silent = false) {
  const isWidth = Number((arg0 === "width" ? exportWidth : exportHeight).value);
  if (!Number.isFinite(isWidth) || isWidth <= 0) {
    return;
  }
  let id2 = arg0 === "width" ? isWidth : Number(exportWidth.value);
  let arg02 = arg0 === "height" ? isWidth : Number(exportHeight.value);
  id2 = Number.isFinite(id2) && id2 > 0 ? id2 : hc;
  arg02 = Number.isFinite(arg02) && arg02 > 0 ? arg02 : fc;
  if (exportLockRatio.checked) {
    if (arg0 === "width") {
      if (silent) {
        id2 = clamp(id2, 320, 4096);
        arg02 = Math.round(id2 / Tn);
        if (arg02 < 320) {
          arg02 = 320;
          id2 = Math.round(arg02 * Tn);
        }
        if (arg02 > 4096) {
          arg02 = 4096;
          id2 = Math.round(arg02 * Tn);
        ...
        arg02 = Math.round(clamp(id2 / Tn, 320, 4096));
      }
    } else if (silent) {
      arg02 = clamp(arg02, 320, 4096);
      id2 = Math.round(arg02 * Tn);
      if (id2 < 320) {
        id2 = 320;
        arg02 = Math.round(id2 / Tn);
      ...
      id2 = Math.round(clamp(arg02 * Tn, 320, 4096));
    }
  }
  if (silent) {
    id2 = Math.round(clamp(id2, 320, 4096));
    arg02 = Math.round(clamp(arg02, 320, 4096));
    exportWidth.value = String(id2);
    exportHeight.value = String(arg02);
...
function normalizeProjectExportPresets() {
  ...
  const map = new Map((projectDoc2?.floors || []).map(floor => [floor.id, floor.name]));
  h0.replaceChildren(...normalizeExportPresetSlots2.map((preset, arg1) => {
    ...
    el2.textContent = preset?.name || map.get(preset?.floorId) || "未命名存档";
...
function activateExportPresetSlot(floor, silent = {}) {
  ...
  const value778 = flag.floorMode === "all" && projectDoc2.floors.length > 1 ? "all" : id2?.id || activeFloor()?.id || activeFloorId;
  setExportFloorScope(value778);
  ...
  const value779 = has14.has("televisionOn");
  const value780 = has14.has("vehicleCharging");
  for (const dataset4 of exportDialog.querySelectorAll("input[data-export-file]")) {
    const startsWith = dataset4.dataset.exportFile;
    dataset4.checked = has14.has(startsWith) || value779 && startsWith.startsWith("screen:") || value780 && startsWith.startsWith("vehicle:");
  }
...
function selectExportPresetIndex(arg0) {
  ...
  if (!Number.isInteger(arg0) || arg0 < 0 || arg0 >= value || orbitSuspended) {
    return;
  }
  flushExportUiDebounce();
  projectDoc2.activeExportPresetSlot = arg0;
  const flag = activateExportPresetSlot(arg0);
  exportPresetEditorOpen2 = false;
  normalizeProjectExportPresets();
  scheduleSave();
  if (!flag) {
    exportStatus.textContent = "存档 " + String(arg0 + 1).padStart(2, "0") + " 没有设置";
  }
}
...
function defaultExportPresetLabel(arg0, arg178) {
  if (!arg0) {
    return "存档 " + String(arg178 + 1).padStart(2, "0");
  }
  const normalizeLabelText2 = (projectDoc2?.floors || []).find(item => item.id === arg0.floorId)?.name;
  return arg0.name || normalizeLabelText2 || "存档 " + String(arg178 + 1).padStart(2, "0");
}
function Iu(size, arg179 = -1) {
  const blob = normalizeLabelText(size, "导出视角", 24);
  const isPutImageData = new Set((projectDoc2?.exportPresets || []).map((arg43, arg44) => arg44 === arg179 ? "" : defaultExportPresetLabel(arg43, arg44)).filter(Boolean));
...
function renderExportFileChecklist() {
  ...
  const handler = (arg0, arg1, arg2) => {
    ...
    el2.textContent = arg1;
    const el22 = document.createElement("small");
    el22.textContent = arg2;
    append2.append(button, el2);
    append.append(append2, el22);
    list2.push(append);
  };
...
function currentExportFloorScope() {
  ...
  value930(exportFloorGap2, finite(projectDoc2.exportFloorGap, 3).toFixed(1));
  syncFloorCameraChrome();
}
function setExportFloorScope(arg0) {
  ...
  const flag = arg0 === "all" && projectDoc2.floors.length > 1;
  if (!flag) {
    const id2 = projectDoc2.floors.find(item => item.id === arg0);
    if (!id2) {
      return;
    }
...
function openExportDialog() {
  ...
  pi(cameraSettings.camera, cameraSettings.camera.viewportAspect);
  syncCameraModeButtons(cameraSettings.cameraSettings.mode);
  ui(cameraSettings.cameraSettings.view);
...
async function capturePreviewCanvas2(planPoint, floor, arg2 = {}) {
  forceTripleRender();
  const value = document.createElement("canvas");
  value.width = planPoint;
  value.height = floor;
  const count = value.getContext("2d", {
    willReadFrequently: arg2.pixels === true
  });
  if (!count) {
...
async function composeExportCanvas(flag, flag2) {
  ...
  const value781 = buildLightDeltaPixels(flag.data, flag2.data);
  putImageData2.putImageData(new ImageData(value781, flag.width, flag.height), 0, 0);
  return canvasToBlob(width15);
}
async function drawExportAnnotations(data, lights, arg2, value, arg4, arg5) {
  ...
  try {
    for (let count = 0; count < list.length; count += 1) {
      ...
      exportStatus.textContent = "正在渲染灯组 " + (arg4 + 1) + "/" + arg5 + "：" + lights.name + "（" + (count + 1) + "/" + list.length + "）";
      ...
      const imageData = await capturePreviewCanvas2(arg2, value, {
        pixels: true
      });
      ...
      isClearRect.putImageData(new ImageData(buildLightDeltaPixels2, arg2, value), 0, 0);
...
async function buildExportImageCanvas(flag, floor, arg2 = null) {
  ...
  if (arg2) {
    ...
    putImageData.putImageData(arg2, 0, 0);
    fillStyle.drawImage(width9, 0, 0);
  }
  return canvasToBlob(width16);
}
function sanitizeExportFileName(arg0, arg1) {
  return String(arg0 || "").normalize("NFKC").replace(/[<>:"/\\|?*\u0000-\u001f]/g, "-").replace(/\s+/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "").slice(0, 48) || arg1;
}
function uniqueExportFileName(flag, arg1, idSet, arg3 = EXPORT_IMAGE_EXTENSION) {
  const value = sanitizeExportFileName(flag, "灯组-" + (arg1 + 1));
...
function vm(arg0, scene10 = activeFloor()) {
  const value782 = scene10?.scene?.calibration?.pixelsPerMeter || 1;
  return {
    id: arg0.id,
    ...
    position: {
      x: arg0.x / value782,
      z: arg0.y / value782,
      elevation: floorStackOffsetY(scene10) + (arg0.elevation || 0)
    },
    rotation: arg0.rotation || 0,
    verticalRotation: arg0.verticalRotation || 0,
    stripRollRotation: arg0.type === "striplight" && arg0.stripRollRotation || 0,
    size: {
      width: arg0.width,
      depth: arg0.depth
    },
    temperature: arg0.lightTemperature,
    brightness: arg0.lightBrightness,
    range: arg0.lightRange,
    angle: arg0.lightAngle
  };
}
function projectItemToScreenNorm(x46, scene11, arg2 = previewFloorEntries()) {
  if (!x46 || !scene11 || !camera2) {
    return null;
  }
  const value783 = scene11.scene?.calibration?.pixelsPerMeter || 1;
  let value784 = 0;
  let value785 = Math.max(0, finite(x46.elevation, 0)) + Math.max(0.02, finite(x46.height, 0.1)) / 2;
  let value786 = 0;
  if (getPreviewFloorMode2() === "all") {
    const value634 = (finite(x46.x, 0) - finite(scene11.originX, 0)) / value783;
    const value635 = (finite(x46.y, 0) - finite(scene11.originY, 0)) / value783;
    const value636 = -THREE.MathUtils.degToRad(finite(scene11.rotation, 0));
    value784 = value634 * Math.cos(value636) + value635 * Math.sin(value636) + finite(scene11.offsetX, 0);
    value786 = -value634 * Math.sin(value636) + value635 * Math.cos(value636) + finite(scene11.offsetZ, 0);
    const findIndex = [...arg2].sort((elevation7, elevation8) => elevation7.elevation - elevation8.elevation);
    const value637 = Math.max(0, findIndex.findIndex(wall2 => wall2.id === scene11.id));
    value785 += value637 * finite(projectDoc2.exportFloorGap, 3);
  } else {
    ...
    value784 = (finite(x46.x, 0) - (minX2.minX + minX2.maxX) / 2) / value783;
    value786 = (finite(x46.y, 0) - (minX2.minY + minX2.maxY) / 2) / value783;
  }
  camera2.updateMatrixWorld(true);
  const z3 = new THREE.Vector3(value784, value785, value786).project(camera2);
...
function bm(arg0, arg1, arg181 = previewFloorEntries()) {
  for (const value699 of arg0 || []) {
    const value638 = projectItemToScreenNorm(value699, arg1, arg181);
    if (value638) {
      return value638;
    }
  }
...
function id(arg182 = "", arg183 = "", arg184 = "") {
  for (const {
    key: value700,
    group: enabled5
  } of collectLightGroupsAcrossFloors()) {
    enabled5.enabled = arg182 === "*" || value700 === arg182;
  }
  for (const {
    key: value701,
    item: screenEnabled
  } of collectTvsAcrossFloors()) {
    screenEnabled.screenEnabled = arg183 === "*" || value701 === arg183;
  }
  for (const {
    key: value702,
    item: chargingEnabled
  } of collectSmallCarsAcrossFloors()) {
    chargingEnabled.chargingEnabled = arg184 === "*" || value702 === arg184;
  }
  rebuildWorldPreview2();
}
...
function promptExportOverwrite(arg0) {
  ...
  return new Promise(arg02 => {
    exportOverwriteResolver2 = arg02;
  });
}
function notifyAutoDiagramExport(arg0, message2) {
  if (!!isAutoDiagramEmbed2 && !!autoDiagramComponentId && window.parent !== window) {
    window.parent.postMessage({
...
async function runExportPipeline() {
  ...
  const has = new Set(Object.values(background).map(toLocaleLowerCase => toLocaleLowerCase.toLocaleLowerCase()));
  const list = previewFloorEntries();
  const list2 = collectLightGroupsAcrossFloors(list).map(({
    ...
    file: uniqueExportFileName(list.length > 1 ? floor.name + "-" + group.name : group.name, index, has),
    lights: floor.scene.items.filter(item => lightItemTypes2.has(item.type) && item.lightGroupId === group.id)
  ...
  for (let count = 0; count < list3.length; count += 1) {
    ...
    file.file = uniqueExportFileName(file.name, count, has);
  }
  ...
  try {
    ...
    const manifest = {
      ...
      groups: list2.map(floor => ({
        ...
        name: floor.name,
        file: floor.file,
        anchor: bm(floor.lights, floor.floor, list),
        enabledInEditor: floor.enabledInEditor,
        lights: floor.lights.map(item => vm(item, floor.floor))
      })),
      screens: list3.map(({
        key,
        floor,
...
function addBoxMesh(group, width, height, depth, arg185, arg186, arg187, color, options = {}) {
  ...
  mesh.position.set(arg185, arg186, arg187);
...
function buildMergedBoxGeometry(list) {
  const list2 = list.map(normalizeBoxPartSpec).filter(size => [size.width, size.height, size.depth].every(arg0 => Number.isFinite(arg0) && arg0 > 0.0001));
  if (!list2.length) {
    return null;
  }
...
function addSharedArchMesh(group, entries, arg2, roughness = {}) {
  const flag = buildMergedBoxGeometry(entries);
  if (!flag) {
    return null;
  }
  const light = new THREE.Mesh(flag, Em(arg2, roughness));
...
function createBoxPartMesh(arg0, arg1, arg2, arg3, arg4, arg5) {
  const value = Math.max(Math.min(arg0, arg1, arg2), 0.001);
  const minValue = Math.min(value * 0.14, value * 0.45, 0.08);
  const boxGeometry = new RoundedBoxGeometry(arg0, arg1, arg2, 2, minValue);
  boxGeometry.translate(arg3, arg4, arg5);
  return boxGeometry;
}
function mergeBoxPartGeometries(list) {
  const item = list.map(arg0 => createBoxPartMesh(...arg0));
  const mergeGeometries2 = mergeGeometries(item);
  item.forEach(dispose => dispose.dispose());
  return mergeGeometries2;
}
function Cm(arg0, arg1, arg2) {
  const list = arg0 + ":" + arg1 + ":" + arg2;
  if (lightPropertyMeta.has(list)) {
    return lightPropertyMeta.get(list);
  }
  const handler = mergeBoxPartGeometries([[arg0 * 0.92, arg1 * 0.28, arg2 * 0.72, 0, arg1 * 0.28, arg2 * 0.06], [arg0 * 0.92, arg1 * 0.55, arg2 * 0.18, 0, arg1 * 0.56, -arg2 * 0.35], [arg0 * 0.1, arg1 * 0.48, arg2 * 0.75, -arg0 * 0.46, arg1 * 0.39, arg2 * 0.03], [arg0 * 0.1, arg1 * 0.48, arg2 * 0.75, arg0 * 0.46, arg1 * 0.39, arg2 * 0.03]]);
  const list2 = mergeBoxPartGeometries([[arg0 * 0.42, arg1 * 0.12, arg2 * 0.55, -arg0 * 0.22, arg1 * 0.47, arg2 * 0.07], [arg0 * 0.42, arg1 * 0.12, arg2 * 0.55, arg0 * 0.22, arg1 * 0.47, arg2 * 0.07]]);
  const value787 = handler && list2 ? {
    frame: handler,
    cushions: list2
  } : null;
  if (value787) {
    lightPropertyMeta.set(list, value787);
  } else {
    handler?.dispose();
    list2?.dispose();
  }
...
function addSofaMeshes(group, size, color, cushionColor) {
  const sofaGeometry = Cm(size.width, size.height, size.depth);
  if (!sofaGeometry) {
    return false;
  }
...
function Im(group, id, has) {
  const value = clamp(id, 0.004, 0.018);
  const flag = group + ":" + value + ":" + has;
  if (!isStudioRoute.has(flag)) {
    const value639 = Math.max(Math.min(group, value, has), 0.001);
    const value640 = Math.min(Math.min(group, has) * 0.018, value639 * 0.45, 0.08);
    const base = new RoundedBoxGeometry(group, value, has, 2, value640);
    const inset = new THREE.PlaneGeometry(group * 0.88, has * 0.84);
...
function getCachedRugMaterial(object3d, arg188 = false) {
  const value788 = (arg188 ? "inset" : "base") + ":" + object3d;
  if (!Zl.has(value788)) {
    Zl.set(value788, new THREE.MeshStandardMaterial({
      color: object3d,
      roughness: 1,
      metalness: 0,
      ...(arg188 ? {
  ...
  return Zl.get(value788);
}
function addRugMeshes(group, size, color, color2) {
  const base = Im(size.width, size.height, size.depth);
  if (!base) {
    return false;
  }
  ...
  group.add(light);
  const armV2 = flag ? getCachedRugMaterial(color2, true).clone() : getCachedRugMaterial(color2, true);
...
function createGlassMaterial(color, arg189 = false, depthWrite = false) {
  const flag = color.material;
  if (!color.isMesh || color.isSkinnedMesh || color.isBatchedMesh || color.morphTargetInfluences || Array.isArray(flag) || !flag?.isMeshStandardMaterial || flag.transparent || flag.opacity < 1 || flag.transmission > 0 || flag.alphaHash || flag.displacementMap || flag.onBeforeCompile !== THREE.Material.prototype.onBeforeCompile || flag.customProgramCacheKey !== THREE.Material.prototype.customProgramCacheKey || color.onBeforeRender !== THREE.Object3D.prototype.onBeforeRender || flag.clippingPlanes?.length || !arg189 && Object.values(flag).some(isTexture2 => isTexture2?.isTexture)) {
    return "";
  }
  const push16 = [];
  for (const value703 of Object.keys(flag).sort()) {
    if (["id", "uuid", "name", "userData", "version", "_listeners"].includes(value703)) {
      continue;
    }
    const isTexture4 = flag[value703];
    if (value703 === "color" && depthWrite) {
      push16.push([value703, [1, 1, 1]]);
      continue;
    }
    if (isTexture4 == null || ["number", "boolean", "string"].includes(typeof isTexture4)) {
      push16.push([value703, isTexture4]);
    } else if (isTexture4.isTexture) {
      push16.push([value703, isTexture4.uuid]);
      ...
      push16.push([value703, isTexture4.toArray()]);
    } else if (Array.isArray(isTexture4) && isTexture4.every(arg35 => ["number", "boolean", "string"].includes(typeof arg35))) {
      push16.push([value703, isTexture4]);
    } else if (value703 === "defines") {
      push16.push([value703, Object.entries(isTexture4).sort(([localeCompare], [arg23]) => localeCompare.localeCompare(arg23))]);
    } else {
      return "";
    }
  }
...
function meshMaterialSignature(light) {
  const material = Object.entries(light.geometry?.attributes || {}).sort(([localeCompare2], [arg53]) => localeCompare2.localeCompare(arg53)).map(([arg69, itemSize2]) => [arg69, itemSize2.itemSize, itemSize2.normalized, itemSize2.array?.constructor?.name]);
  return JSON.stringify([!!light.geometry?.index, material, Object.keys(light.geometry?.morphAttributes || {}).sort()]);
}
function Ys(geometry) {
  const value = [];
...
function mergeSimilarItemMeshes(object3d, type) {
  ...
  const list = Ys(object3d);
  const before = new Map();
  const map = new Map();
  for (const light of list) {
    const flag = light.geometry?.parameters;
    const msimV2 = flag ? light.geometry.type + ":" + JSON.stringify(flag) : "";
    if (msimV2 && before.has(msimV2)) {
      light.geometry.dispose();
      light.geometry = before.get(msimV2);
    } else if (msimV2) {
      before.set(msimV2, light.geometry);
    }
    const value641 = collectChildMeshes(light);
    if (value641 && map.has(value641)) {
      light.material.dispose();
      light.material = map.get(value641);
    } else if (value641) {
      map.set(value641, light.material);
    }
  }
...
function addStripLightHelpers(updateMatrixWorld, item) {
  if (!hasProjectLoaded.has(item)) {
    return;
  }
  const group2 = Ys(updateMatrixWorld);
  const emissive = group2.length;
  const clamp2 = new Map();
  for (const userData35 of group2) {
    if (userData35.userData.televisionScreen || userData35.userData.televisionGlow) {
      continue;
    }
    const value642 = createGlassMaterial(userData35);
    if (!value642) {
      continue;
    }
    const value643 = value642 + ":" + meshMaterialSignature(userData35);
    if (!clamp2.has(value643)) {
      clamp2.set(value643, []);
    }
    clamp2.get(value643).push(userData35);
  }
  ...
  for (const length18 of clamp2.values()) {
    ...
    const forEach2 = length18.map(matrixWorld => {
      const value419 = new THREE.Matrix4().multiplyMatrices(clamp3, matrixWorld.matrixWorld);
      return matrixWorld.geometry.clone().applyMatrix4(value419);
    });
    const value644 = mergeGeometries(forEach2);
    forEach2.forEach(dispose => dispose.dispose());
    if (!value644) {
      continue;
    }
    const material8 = length18[0];
    const castShadow5 = new THREE.Mesh(value644, material8.material);
    ...
    length18.forEach((parent3, arg63) => {
  ...
  updateMatrixWorld.userData.optimizationStats = {
    type: item,
    before: emissive,
    after: Ys(updateMatrixWorld).length
  };
}
function addSoftBoxMesh(group, arg1, arg2, arg3, arg4, arg5, arg6, color, light = {}) {
  ...
  const light2 = new THREE.Mesh(new THREE.CylinderGeometry(arg1, arg2, arg3, light.segments ?? 24), value);
...
function applySelectionHighlight(object3d, flag) {
  ...
  const value789 = clamp(finite(flag.depth, 0.28), 0.1, 8);
  const value790 = clamp(finite(flag.width, 1), 0.1, 8);
  const value791 = value789;
  ...
  const value792 = clamp(finite(flag.lightRange, 3.5) * 0.16, 0.28, 0.72);
  materialFingerprint(addBoxMesh(rotation6, value790, 0.014, value791, 0, -value792, 0, emissive, {
    rounded: false,
    transparent: true,
    opacity: 0.24,
    depthWrite: false,
  ...
  materialFingerprint(addSoftBoxMesh(rotation6, 0.012, 0.012, value792, 0, -value792 * 0.5, 0, emissive, {
    segments: 10,
    transparent: true,
    opacity: 0.78,
    depthWrite: false,
  ...
  rotation7.position.set(0, -value792, 0);
...
function createWallTopMaterial(flag, arg1, arg190) {
  const flag2 = [{
    y: 0,
    halfWidth: flag * 0.31,
    backZ: -arg1 * 0.16,
  ...
  const value793 = halfWidth => {
    ...
    for (let value452 = 1; value452 <= 18; value452 += 1) {
      const value420 = value452 / 18 * Math.PI;
      push10.push(new THREE.Vector3(Math.cos(value420) * halfWidth.halfWidth, halfWidth.y, halfWidth.sideZ + Math.sin(value420) * (halfWidth.frontZ - halfWidth.sideZ)));
    }
    return push10;
  };
  const length26 = flag2.map(value793);
  const value794 = length26[0].length;
  const length27 = length26.flatMap(flatMap => flatMap.flatMap(point3 => [point3.x, point3.y, point3.z]));
  const push17 = [];
  for (let value704 = 0; value704 < length26.length - 1; value704 += 1) {
    const value645 = value704 * value794;
    const value646 = (value704 + 1) * value794;
    for (let value555 = 0; value555 < value794; value555 += 1) {
      const value487 = (value555 + 1) % value794;
      const value488 = value645 + value555;
      const value489 = value645 + value487;
      const value490 = value646 + value555;
      const value491 = value646 + value487;
      push17.push(value488, value491, value489, value488, value490, value491);
    }
  }
  const value795 = length27.length / 3;
  length27.push(0, flag2[0].y, arg1 * 0.08);
  const value796 = length27.length / 3;
  length27.push(0, flag2.at(-1).y, arg1 * 0.08);
  const value797 = (length26.length - 1) * value794;
  for (let value705 = 0; value705 < value794; value705 += 1) {
    const value647 = (value705 + 1) % value794;
    push17.push(value795, value705, value647);
    push17.push(value796, value797 + value647, value797 + value705);
  }
...
function buildTelevisionMesh(group, arg1, arg2, arg3, arg4, arg5, arg6, color, color2) {
  const group2 = new THREE.Group();
  const value798 = arg3 * 0.49;
  const height = arg3 * 0.12;
  const value799 = -arg2 * 0.39;
  addBoxMesh(group2, arg1 * 0.9, height, arg2 * 0.82, 0, value798, arg2 * 0.02, color, {
    radius: Math.min(arg1, arg2) * 0.06,
    roughness: 0.72
  });
  addBoxMesh(group2, arg1 * 0.78, arg3 * 0.34, arg2 * 0.09, 0, arg3 * 0.78, value799, color, {
    radius: Math.min(arg1, arg2) * 0.045,
    roughness: 0.72
  });
  for (const value of [-0.38, 0.38]) {
    addBoxMesh(group2, 0.05, arg3 * 0.47, 0.05, arg1 * value, arg3 * 0.235, arg2 * 0.34, color2, {
      rounded: false
    });
    addBoxMesh(group2, 0.05, arg3 * 0.94, 0.05, arg1 * value, arg3 * 0.47, value799, color2, {
      rounded: false
    });
  }
...
function stairRiserMaterialOptions(group, arg1, value, param) {
  const light = {
    rounded: false,
    metalness: 0.18,
    roughness: 0.36,
...
function countShadowLights(traverse, arg191) {
  if (!arg191) {
    return;
  }
  const maxTextureUnits2 = resolvedThemeColors();
  traverse.traverse(material3 => {
    const value556 = Array.isArray(material3.material) ? material3.material : material3.material ? [material3.material] : [];
    for (const isMeshStandardMaterial of value556) {
...
function markAsLightSourcePreview(light) {
  ...
  const value800 = normalizeLabelText(light.title, "家庭总览", 24);
  const value801 = normalizeLabelText(light.subtitle, "HOME PLAN", 36);
  const value802 = 115;
  const value803 = 184;
  light3.font = "700 " + value803 + "px sans-serif";
  drawTrackedText(light3, value800, value802, 130, value803 * clamp(finite(light.titleSpacing, 1.05), 0, 1.8), 1340);
  const value804 = 1580;
  const value805 = 130;
  const value806 = 170;
  light3.fillStyle = "#929baa";
  light3.beginPath();
  light3.moveTo(value804, value805 - value806 * 0.58);
  light3.lineTo(value804 + value806 * 0.56, value805 - value806 * 0.02);
  light3.lineTo(value804 + value806 * 0.38, value805 - value806 * 0.02);
  light3.lineTo(value804 + value806 * 0.38, value805 + value806 * 0.5);
  light3.lineTo(value804 - value806 * 0.38, value805 + value806 * 0.5);
  light3.lineTo(value804 - value806 * 0.38, value805 - value806 * 0.02);
  light3.lineTo(value804 - value806 * 0.56, value805 - value806 * 0.02);
  light3.closePath();
  ...
  light3.fillRect(value804 - value806 * 0.09, value805 + value806 * 0.2, value806 * 0.18, value806 * 0.3);
  light3.restore();
  light3.fillStyle = "#929baa";
  light3.textAlign = "left";
  const value807 = 310;
  light3.font = "400 " + value807 + "px \"Arial Narrow\", Arial, sans-serif";
  drawTrackedText(light3, value801, 72, 410, value807 * clamp(finite(light.subtitleSpacing, 0.08), 0, 0.6), 1880);
  const value808 = 74;
  const value809 = value808 + clamp(finite(light.lineLength, 0.86), 0.3, 1) * 1880;
  light3.strokeStyle = "rgba(146, 155, 170, 0.72)";
  light3.lineWidth = 16;
  light3.beginPath();
  light3.moveTo(value808, 590);
  light3.lineTo(value809, 590);
  light3.moveTo(value808, 566);
  light3.lineTo(value808, 614);
  light3.moveTo(value809, 566);
  light3.lineTo(value809, 614);
  light3.stroke();
...
function createPlanLabelSprite(size) {
  ...
  let value810 = Object.values(size).filter(isTexture3 => isTexture3?.isTexture === true).length;
...
function countMaterialTextures(traverse2 = worldGroup) {
  ...
  traverse2?.traverse(material4 => {
    ...
    const value557 = Array.isArray(material4.material) ? material4.material : material4.material ? [material4.material] : [];
    for (const value453 of value557) {
      length = Math.max(length, createPlanLabelSprite(value453));
    }
  });
...
function countSceneMeshes(object3d = worldGroup) {
  ...
  object3d?.traverse(object3d2 => {
    ...
    const value558 = String(object3d2.userData?.lightItemId || "");
    if (value558) {
      value.push({
        id: isArrayResult + ":" + value558,
...
function Ht(traverse3 = worldGroup, {
  rebuildAtlas: arg192 = true
} = {}) {
  if (yt) {
  ...
  const rectAreaLightTextureUnits = value811 ? deferredModelTimer : 0;
  const value812 = flag - flag2 - nonSpotShadowTextureUnits - rectAreaLightTextureUnits - externalModels;
  if (!stageSession && shadowAtlas && value812 >= 1) {
    const value648 = arg192 ? shadowAtlas.schedule(traverse3) : countSceneMeshes(traverse3).length;
    const value649 = shadowAtlas.sync(traverse3);
    ...
    dataset2.dataset.spotShadowLimit = String(value648);
    dataset2.dataset.activeSpotShadows = String(value649);
    let value650 = 0;
    ...
    dataset2.dataset.activeUserLights = String(value650);
    return value649;
  }
  const value813 = spotShadowTextureUnitLimit({
  ...
  const has15 = new Set(selectShadowCastingLightIds(countSceneMeshes(traverse3), value813));
  let value814 = 0;
  traverse3.traverse(userData22 => {
    if (!userData22.isSpotLight || !userData22.userData?.lightItemId) {
      return;
    }
    const value559 = String(userData22.userData?.lightFloorId || "") + ":" + String(userData22.userData.lightItemId);
    const castShadow4 = has15.has(value559);
  ...
  dataset8.dataset.spotShadowLimit = String(value813);
  dataset8.dataset.activeSpotShadows = String(value814);
  let value815 = 0;
  ...
  dataset8.dataset.activeUserLights = String(value815);
  return value814;
}
function buildWallCornerCaps(list, color, arg2) {
  ...
  const value816 = clamp(finite(color.lightBrightness, range2.brightness), 0, 100) / 100;
  const value817 = deferExternalModels[color.type] || 1.1;
  const flag22 = color.type === "striplight";
  const id2 = resolveLightGroup(color);
  if (flag22) {
    const object3d = clamp(finite(color.width, 2), 0.1, 8);
    const value651 = clamp(finite(color.depth, 0.28), 0.1, 8);
    const value652 = clamp(finite(color.lightRange, range2.range), 0.5, 10);
    const value653 = clamp(value652 / range2.range, 0.45, 1.65);
    const value654 = Math.max(finite(color.elevation, 2.7), 0.4);
    const value655 = clamp(Math.max(1, Math.pow(value654 / 2.7, 2)), 1, 4);
    ...
    const lightOnIntensity2 = (yt ? value816 : Math.pow(value816, 0.82)) * 48 * value653 * value655 * value817;
    const userData28 = new THREE.RectAreaLight(list2, list3 ? lightOnIntensity2 : 0, object3d * 0.94, value651 * 0.94);
    ...
    if (yt) {
      userData28.userData.regionFullIntensity = value653 * 48 * value655 * value817;
  ...
  const needsUpdate = arg2?.has(color.id) === true;
  const value818 = needsUpdate || flag2 || flag;
  const far2 = clamp(finite(color.lightRange, range2.range), 0.5, 10);
  const value819 = clamp(finite(color.lightAngle, range2.angle), 15, defaultItemDepth(color.type));
  const value820 = 1;
  const value821 = (color.type === "ceilinglight" ? 680 : 520) * (yt ? value816 : spotLightBrightnessResponse(color.type, value816)) * value817;
  for (let value = 0; value < value820; value += 1) {
    const worldPoint = value820 === 1 ? 0 : -color.width * 0.47 + color.width * 0.94 * value / (value820 - 1);
    const worldPoint2 = new THREE.SpotLight(list2, list3 ? value821 / value820 : 0, far2, THREE.MathUtils.degToRad(value819 / 2), 0.86, 2);
    ...
    if (value818) {
      const blurSamples = localSpotShadowSettings(color.type, far2, value819);
      const value492 = scaledShadowMapSize(blurSamples.mapSize);
      worldPoint2.shadow.mapSize.set(value492, value492);
    ...
    worldPoint2.userData.lightOnIntensity = value821 / value820;
    if (yt) {
      worldPoint2.userData.regionFullIntensity = (color.type === "ceilinglight" ? 680 : 520) * value817;
      studioReady?.register(worldPoint2, color);
    }
    const bwccV2 = new THREE.Object3D();
    bwccV2.position.set(worldPoint, -Math.max(finite(color.elevation, 2.68), 0.8), 0);
    list.add(bwccV2);
    worldPoint2.target = bwccV2;
...
function qu(item, arg193) {
  const group = item.tvMountStyle === "mobile";
  const width = item.tvMountStyle === "tabletop";
  return {
    bodyHeight: arg193 * (group ? 0.43 : width ? 0.56 : 0.62),
    centerY: arg193 * (group ? 0.76 : width ? 0.67 : 0.62)
  };
}
function addTvMountMeshes(group, tvMountStyle, arg2, arg3, arg4) {
  const {
    bodyHeight,
    centerY
  } = qu(tvMountStyle, arg4);
  const width = arg2 * 0.965;
  const height = bodyHeight * 0.94;
  const depth = 0.012;
  const value = Math.max(arg3 * 0.28, 0.05) * 0.5 + 0.006;
  const value822 = value - depth * 0.5;
  if (tvMountStyle.screenEnabled === false) {
    const userData29 = addBoxMesh(group, width, height, depth, 0, centerY, value822, 527122, {
      roughness: 0.18
    });
  ...
  light2.position.set(0, centerY, value822);
  light2.renderOrder = 7;
  light2.castShadow = false;
  light2.receiveShadow = false;
  group.add(light2);
}
function addSmallCarMeshes(group, chargingEnabled, arg2, arg3, arg4) {
  ...
  el2.height = 256;
  const canvasCtx = el2.getContext("2d");
  const ascmV2 = el2.width / 2;
  const addColorStop = canvasCtx.createRadialGradient(ascmV2, ascmV2, 0, ascmV2, ascmV2, ascmV2);
  ...
  for (let index = 18; index < el2.height - 18; index += 10) {
    for (let index2 = 18; index2 < el2.width - 18; index2 += 10) {
      const hypot = Math.hypot(index2 - ascmV2, index - ascmV2) / ascmV2;
  ...
  const map = new THREE.CanvasTexture(el2);
  map.colorSpace = THREE.SRGBColorSpace;
  map.needsUpdate = true;
  const light = new THREE.Mesh(new THREE.PlaneGeometry(arg2 * 1.72, arg3 * 1.42), new THREE.MeshBasicMaterial({
    map,
    transparent: true,
    opacity: 0.82,
    depthWrite: false,
...
function sd(type20, arg194 = null) {
  ...
  const value823 = type20.width;
  const value824 = type20.depth;
  const value825 = type20.height;
  const glass = resolvedThemeColors();
  const value826 = glass.furniture;
  const value827 = glass.appliance ?? value826;
  ...
  if (lightItemTypes2.has(type20.type)) {
    buildWallCornerCaps(rotation, type20, arg194);
    ...
    if (!attachExternalItemModel(rotation, type20)) {
      addBoxMesh(rotation, value823, value825, value824, 0, value825 * 0.5, 0, RESERVED_TEXTURE_UNITS.has(type20.type) ? value827 : value826, {
        rounded: false,
        roughness: 0.6,
        metalness: 0.1
      });
    ...
    if (!attachExternalItemModel(rotation, type20)) {
      addBoxMesh(rotation, value823 * 0.96, value825 * 0.38, value824 * 0.9, 0, value825 * 0.28, 0, value826, {
        roughness: 0.46,
        metalness: 0.18
      });
      addBoxMesh(rotation, value823 * 0.78, value825 * 0.42, value824 * 0.48, 0, value825 * 0.62, -value824 * 0.03, color5, {
        roughness: 0.38,
        metalness: 0.12
      });
      for (const value369 of [-0.48, 0.48]) {
        for (const value351 of [-0.3, 0.3]) {
          addBoxMesh(rotation, value823 * 0.1, value825 * 0.22, value824 * 0.17, value369 * value823, value825 * 0.17, value351 * value824, color7, {
            rounded: false,
            roughness: 0.82
          });
        }
    ...
    addSmallCarMeshes(rotation, type20, value823, value824, value825);
  } else if (type20.type === "curtain") {
    const value421 = ["left", "right", "split"].includes(type20.curtainPosition) ? type20.curtainPosition : "split";
    const value422 = rotation.children.length;
    if (isStageEmbed) {
      rotation.userData.curtainRigRoot = true;
      rotation.userData.curtainRigBasis = [value823, value825, value824];
    }
    const value423 = Math.min(Math.max(value824 * 0.09, 0.012), 0.028);
    const value424 = value825 - value423 * 1.8;
    const value425 = value424 - value423 * 1.8;
    const value426 = Math.max(value825 * 0.025, 0.025);
    const value427 = Math.max(value425 - value426, value825 * 0.72);
    addSoftBoxMesh(rotation, value423, value423, value823 * 1.06, 0, value424, 0, color7, {
      segments: 18,
      rotationZ: Math.PI / 2,
      metalness: 0.68,
      roughness: 0.22
    });
    for (const value399 of [-value823 * 0.52, value823 * 0.52]) {
      addSoftBoxMesh(rotation, value423 * 1.45, value423 * 1.45, value423 * 0.9, value399, value424, 0, color6, {
        segments: 18,
        rotationZ: Math.PI / 2,
        metalness: 0.52,
        roughness: 0.26
    ...
    const value428 = (arg36, arg37) => {
      const value360 = arg37 / 7;
      for (let value333 = 0; value333 < 7; value333 += 1) {
        const value319 = arg36 + value360 * (value333 + 0.5);
        const value320 = value333 % 2 === 0 ? value824 * 0.1 : -value824 * 0.1;
        addBoxMesh(rotation, value360 * 1.24, value427, value824 * 0.62, value319, value426 + value427 * 0.5, value320, color6, {
          radius: Math.min(value360 * 0.34, 0.035),
          roughness: 0.94,
          metalness: 0
        });
      }
      addBoxMesh(rotation, arg37 * 1.03, Math.max(value825 * 0.018, 0.025), value824 * 0.74, arg36 + arg37 * 0.5, value426 + value825 * 0.015, 0, color7, {
        radius: 0.01,
        roughness: 0.72,
        metalness: 0.02
      });
      addBoxMesh(rotation, arg37 * 1.06, Math.max(value825 * 0.025, 0.035), value824 * 0.82, arg36 + arg37 * 0.5, value426 + value427 * 0.52, 0, color6, {
        radius: 0.012,
        roughness: 0.48,
        metalness: 0.08
      });
    ...
    if (value421 === "left") {
      value428(-value823 * 0.5, value823 * 0.24);
    } else if (value421 === "right") {
      value428(value823 * 0.26, value823 * 0.24);
    } else {
      value428(-value823 * 0.5, value823 * 0.16);
      value428(value823 * 0.34, value823 * 0.16);
    }
    if (isStageEmbed) {
      rotation.children.slice(value422).forEach((userData3, arg28) => {
        userData3.userData.curtainPart = arg28 === 0 ? "rod" : arg28 < 3 ? "cap" : (arg28 - 3) % 9 < 7 ? "cloth" : "band";
      });
    }
  } else if (Vl.has(type20.type)) {
    const value400 = value824 / 10;
    for (let value370 = 0; value370 < 10; value370 += 1) {
      const value361 = value825 * (value370 + 1) / 10;
      const value362 = -value824 * 0.5 + value400 * (value370 + 0.5);
      addBoxMesh(rotation, value823, value361, value400 * 1.015, 0, value361 * 0.5, value362, value370 % 2 ? value826 : color5, {
        rounded: false,
        roughness: 0.82
      });
      addBoxMesh(rotation, value823 * 1.01, 0.018, value400 * 0.94, 0, value361 + 0.009, value362, color6, {
        rounded: false,
        castShadow: false,
        roughness: 0.72
      });
    ...
    const value371 = value825 * 0.14;
    const value372 = arg29 => arg29 - value371 + -0.008;
    addBoxMesh(rotation, value823 * 0.92, value825 * 0.28, value824 * 0.72, 0, value372(value825 * 0.28), value824 * 0.06, value826);
    addBoxMesh(rotation, value823 * 0.92, value825 * 0.55, value824 * 0.18, 0, value372(value825 * 0.56), -value824 * 0.35, value826);
    addBoxMesh(rotation, value823 * 0.1, value825 * 0.48, value824 * 0.75, -value823 * 0.46, value372(value825 * 0.39), value824 * 0.03, value826);
    addBoxMesh(rotation, value823 * 0.1, value825 * 0.48, value824 * 0.75, value823 * 0.46, value372(value825 * 0.39), value824 * 0.03, value826);
    addBoxMesh(rotation, value823 * 0.42, value825 * 0.12, value824 * 0.55, -value823 * 0.22, value372(value825 * 0.47), value824 * 0.07, color5);
    addBoxMesh(rotation, value823 * 0.42, value825 * 0.12, value824 * 0.55, value823 * 0.22, value372(value825 * 0.47), value824 * 0.07, color5);
  } else if (type20.type === "bed") {
    addBoxMesh(rotation, value823 * 0.996, value825 * 0.3, value824 * 0.996, 0, value825 * 0.15, 0, color7);
    addBoxMesh(rotation, value823 * 0.96, value825 * 0.32, value824 * 0.92, 0, value825 * 0.43, value824 * 0.03, color5);
    addBoxMesh(rotation, value823, value825 * 0.95, value824 * 0.09, 0, value825 * 0.48, -value824 * 0.455, value826);
    addBoxMesh(rotation, value823 * 0.38, value825 * 0.14, value824 * 0.22, -value823 * 0.23, value825 * 0.66, -value824 * 0.29, color6);
    addBoxMesh(rotation, value823 * 0.38, value825 * 0.14, value824 * 0.22, value823 * 0.23, value825 * 0.66, -value824 * 0.29, color6);
  } else if (type20.type === "nightstand") {
    addBoxMesh(rotation, value823, value825 * 0.78, value824, 0, value825 * 0.49, 0, value826);
    addBoxMesh(rotation, value823 * 1.04, value825 * 0.07, value824 * 1.05, 0, value825 * 0.91, 0, color5, {
      roughness: 0.5
    });
    addBoxMesh(rotation, value823 * 0.9, 0.014, value824 * 1.01, 0, value825 * 0.63, value824 * 0.01, color7, {
      rounded: false
    });
    addBoxMesh(rotation, value823 * 0.9, 0.014, value824 * 1.01, 0, value825 * 0.38, value824 * 0.01, color7, {
      rounded: false
    });
    addBoxMesh(rotation, value823 * 0.22, 0.022, 0.032, 0, value825 * 0.5, value824 * 0.52, color6, {
      metalness: 0.5
    });
    for (const value334 of [-0.38, 0.38]) {
      for (const value312 of [-0.35, 0.35]) {
        addBoxMesh(rotation, 0.035, value825 * 0.2, 0.035, value823 * value334, value825 * 0.1, value824 * value312, color7, {
          metalness: 0.18
        });
      }
    }
  } else if (type20.type === "table") {
    const value335 = value823 * 0.64;
    const value336 = value824 * 0.48;
    addBoxMesh(rotation, value335, value825 * 0.1, value336, 0, value825 * 0.93, 0, value826);
    for (const value321 of [-0.43, 0.43]) {
      for (const value299 of [-0.38, 0.38]) {
        addBoxMesh(rotation, 0.07, value825 * 0.9, 0.07, value335 * value321, value825 * 0.45, value336 * value299, color7);
      }
    }
    const value337 = Math.min(value823 * 0.2, 0.5);
    const value338 = Math.min(value824 * 0.27, 0.5);
    const value339 = value825 * 1.18;
    buildTelevisionMesh(rotation, value337, value338, value339, -value823 * 0.37, 0, Math.PI / 2, color5, color7);
    buildTelevisionMesh(rotation, value337, value338, value339, value823 * 0.37, 0, -Math.PI / 2, color5, color7);
    buildTelevisionMesh(rotation, value337, value338, value339, 0, -value824 * 0.35, 0, color5, color7);
    buildTelevisionMesh(rotation, value337, value338, value339, 0, value824 * 0.35, Math.PI, color5, color7);
  } else if (roundTableTypes.has(type20.type)) {
    const value322 = Math.min(value823, value824) * 0.32;
    const value323 = Math.max(value825 * 0.07, 0.045);
    const value324 = value825 * 0.92;
    addSoftBoxMesh(rotation, value322, value322, value323, 0, value324, 0, color6, {
      segments: 48,
      roughness: 0.5,
      metalness: 0.08
    });
    addSoftBoxMesh(rotation, Math.min(value823, value824) * 0.22, Math.min(value823, value824) * 0.3, value825 * 0.68, 0, value825 * 0.43, 0, value826, {
      segments: 36,
      roughness: 0.55,
      metalness: 0.06
    });
    addSoftBoxMesh(rotation, Math.min(value823, value824) * 0.34, Math.min(value823, value824) * 0.34, value825 * 0.07, 0, value825 * 0.045, 0, color7, {
      segments: 40,
      roughness: 0.42,
      metalness: 0.12
    });
    if (hasRoundTableTurntable(type20)) {
      addSoftBoxMesh(rotation, value322 * 0.58, value322 * 0.58, Math.max(value825 * 0.035, 0.025), 0, value324 + value323 * 0.58, 0, color5, {
        segments: 48,
        roughness: 0.48,
        metalness: 0.08
      });
      addSoftBoxMesh(rotation, value322 * 0.44, value322 * 0.44, 0.018, 0, value324 + value323 * 0.58 + 0.036, 0, color7, {
        segments: 48,
        roughness: 0.32,
        metalness: 0.12
      });
    }
    const value325 = Math.min(value823 * 0.17, 0.42);
    const value326 = Math.min(value824 * 0.19, 0.44);
    const value327 = value825 * 1.15;
    buildTelevisionMesh(rotation, value325, value326, value327, -value823 * 0.38, 0, Math.PI / 2, color5, color7);
    buildTelevisionMesh(rotation, value325, value326, value327, value823 * 0.38, 0, -Math.PI / 2, color5, color7);
    buildTelevisionMesh(rotation, value325, value326, value327, 0, -value824 * 0.38, 0, color5, color7);
    buildTelevisionMesh(rotation, value325, value326, value327, 0, value824 * 0.38, Math.PI, color5, color7);
  } else if (type20.type === "bar") {
    addBoxMesh(rotation, value823, value825 * 0.12, value824, 0, value825 * 0.94, 0, color5, {
      roughness: 0.5
    });
    addBoxMesh(rotation, value823 * 0.92, value825 * 0.78, value824 * 0.46, 0, value825 * 0.45, -value824 * 0.18, value826, {
      roughness: 0.65
    });
    addBoxMesh(rotation, value823 * 0.86, value825 * 0.48, 0.035, 0, value825 * 0.42, value824 * 0.28, color7, {
      rounded: false,
      roughness: 0.48
    });
    for (const value300 of [-0.3, 0, 0.3]) {
      addSoftBoxMesh(rotation, value824 * 0.14, value824 * 0.14, 0.045, value823 * value300, value825 * 0.66, value824 * 0.52, color5, {
        segments: 28,
        roughness: 0.52
      });
      addSoftBoxMesh(rotation, 0.025, 0.025, value825 * 0.62, value823 * value300, value825 * 0.34, value824 * 0.52, color7, {
        segments: 18,
        metalness: 0.38,
        roughness: 0.28
      });
      addSoftBoxMesh(rotation, value824 * 0.1, value824 * 0.12, 0.035, value823 * value300, 0.018, value824 * 0.52, color7, {
        segments: 24,
        metalness: 0.3,
        roughness: 0.34
      });
    ...
    const value301 = Math.max(value825 * 0.52, 0.45);
    const value302 = value301;
    const value303 = Math.max(value825 - value301, 0.2);
    const value304 = Math.min(Math.max(Math.min(value823, value824) * 0.025, 0.012), 0.028);
    const value305 = {
      rounded: false,
      transparent: true,
      opacity: 0.22,
    ...
    addBoxMesh(rotation, value823, value301, value824, 0, value301 * 0.5, 0, value826, {
      rounded: false,
      roughness: 0.56
    });
    addBoxMesh(rotation, value823, 0.035, value824, 0, value301, 0, glass.frame, {
      rounded: false,
      metalness: 0.34,
      roughness: 0.3
    });
    addBoxMesh(rotation, value823 - value304 * 2, value303, value304, 0, value302 + value303 * 0.5, -value824 * 0.5 + value304 * 0.5, glass.glass, value305);
    addBoxMesh(rotation, value823 - value304 * 2, value303, value304, 0, value302 + value303 * 0.5, value824 * 0.5 - value304 * 0.5, glass.glass, value305);
    addBoxMesh(rotation, value304, value303, value824 - value304 * 2, -value823 * 0.5 + value304 * 0.5, value302 + value303 * 0.5, 0, glass.glass, value305);
    addBoxMesh(rotation, value304, value303, value824 - value304 * 2, value823 * 0.5 - value304 * 0.5, value302 + value303 * 0.5, 0, glass.glass, value305);
  } else if (type20.type === "coffeetable") {
    const value279 = Math.min(value823 * 0.34, value824 * 0.42);
    const value280 = Math.min(value823 * 0.23, value824 * 0.29);
    const value281 = -value823 * 0.16;
    const value282 = value824 * 0.08;
    const value283 = value823 * 0.24;
    const value284 = -value824 * 0.2;
    const value285 = value825 * 0.58;
    const value286 = value825 * 0.76;
    const value287 = value825 * 0.68;
    const value288 = value825 * 0.9;
    addSoftBoxMesh(rotation, value279 * 0.3, value279 * 0.5, value285, value281, value285 * 0.5, value282, value826, {
      segments: 40,
      roughness: 0.82
    });
    const value289 = value825 * 0.07;
    const value290 = value825 * 0.055;
    const value291 = value285 + value289 * 0.5 + 0.001;
    const value292 = value291 + (value289 + value290) * 0.5 + 0.001;
    addSoftBoxMesh(rotation, value279 * 1.02, value279 * 1.02, value289, value281, value291, value282, color7, {
      segments: 48,
      roughness: 0.72
    });
    addSoftBoxMesh(rotation, value279, value279, value290, value281, value292, value282, color6, {
      segments: 48,
      roughness: 0.9
    });
    addSoftBoxMesh(rotation, value280 * 0.32, value280 * 0.52, value286, value283, value286 * 0.5, value284, color5, {
      segments: 40,
      roughness: 0.82
    });
    const value293 = value825 * 0.07;
    const value294 = value825 * 0.055;
    const value295 = value286 + value293 * 0.5 + 0.001;
    const value296 = value295 + (value293 + value294) * 0.5 + 0.001;
    addSoftBoxMesh(rotation, value280 * 1.02, value280 * 1.02, value293, value283, value295, value284, color7, {
      segments: 48,
      roughness: 0.72
    });
    addSoftBoxMesh(rotation, value280, value280, value294, value283, value296, value284, color6, {
      segments: 48,
      roughness: 0.9
    });
  } else if (type20.type === "squarecoffeetable") {
    const value264 = Math.max(value825 * 0.22, 0.085);
    const value265 = Math.max(value825 * 0.16, 0.065);
    const value266 = value264 + 0.012;
    const value267 = Math.max(value825 - value266 - value265, 0.16);
    const value268 = Math.max(value823 * 0.075, 0.045);
    const value269 = Math.max(value824 * 0.035, 0.018);
    const value270 = value267 * 0.72;
    const value271 = value266 + value267 * 0.48;
    const value272 = value823 * 0.27;
    const value273 = value823 * 0.34;
    const value274 = value826;
    const value275 = color5;
    addBoxMesh(rotation, value823 * 0.98, value265, value824 * 1.03, 0, value266 + value267 + value265 * 0.5, 0, value275, {
      radius: Math.min(value823, value824) * 0.035,
      roughness: 0.5,
      metalness: 0.02
    });
    addBoxMesh(rotation, value268, value267, value824 * 0.92, -value823 * 0.5 + value268 * 0.5, value266 + value267 * 0.5, 0, value274, {
      rounded: false,
      roughness: 0.6
    });
    addBoxMesh(rotation, value268, value267, value824 * 0.92, value823 * 0.5 - value268 * 0.5, value266 + value267 * 0.5, 0, value274, {
    ...
    addBoxMesh(rotation, value823 * 0.86, value267 * 0.9, 0.035, 0, value266 + value267 * 0.52, -value824 * 0.45, value274, {
      rounded: false,
      roughness: 0.75
    });
    addBoxMesh(rotation, value823 * 0.86, value267 * 0.1, value824 * 0.9, 0, value266 + value267 * 0.08, 0, value274, {
      rounded: false,
      roughness: 0.58
    });
    for (const value261 of [-value273, 0, value273]) {
      addBoxMesh(rotation, value272, value270, value269, value261, value271, value824 * 0.48, value275, {
        radius: Math.min(value823, value824) * 0.025,
        roughness: 0.52
      });
      const value251 = value261 === 0 ? 0 : value261 + (value261 < 0 ? value272 * 0.3 : -value272 * 0.3);
      addBoxMesh(rotation, 0.022, value270 * 0.2, 0.025, value251, value271, value824 * 0.505, color7, {
        radius: 0.009,
        metalness: 0.22,
        roughness: 0.3
      });
    }
    const value276 = Math.max(Math.min(value823, value824) * 0.055, 0.035);
    for (const value262 of [-value823 * 0.4, value823 * 0.4]) {
      for (const value236 of [-value824 * 0.36, value824 * 0.36]) {
        const rotation2 = addBoxMesh(rotation, value276, value264, value276, value262, value264 * 0.5, value236, color7, {
          radius: value276 * 0.22,
          roughness: 0.55,
          metalness: 0.02
        });
        rotation2.rotation.z = (value262 < 0 ? -1 : 1) * 0.09;
        rotation2.rotation.x = (value236 < 0 ? -1 : 1) * 0.06;
      }
    }
  } else if (type20.type === "chair") {
    buildTelevisionMesh(rotation, value823, value824, value825, 0, 0, 0, value826, color7);
  } else if (type20.type === "sideboard") {
    const value252 = Math.max(value825, 1.8);
    const value253 = value252 * 0.39;
    const value254 = value252 * 0.22;
    const value255 = value253 + value254;
    const value256 = value252 - value255;
    const value257 = color5;
    const value258 = value823 * 0.31;
    const value259 = value253 * 0.88;
    const value260 = value256 * 0.86;
    addBoxMesh(rotation, value823, value253, value824, 0, value253 * 0.5, 0, value826, {
      roughness: 0.58
    });
    addBoxMesh(rotation, value823, 0.045, value824, 0, value253, 0, color5, {
      roughness: 0.5
    });
    addBoxMesh(rotation, value823, value254, 0.045, 0, value253 + value254 * 0.5, -value824 * 0.46, value826, {
      rounded: false,
      roughness: 0.62
    });
    addBoxMesh(rotation, value823, value256, value824, 0, value255 + value256 * 0.5, 0, value826, {
      roughness: 0.58
    });
    for (const value237 of [-0.33, 0, 0.33]) {
      addBoxMesh(rotation, value258, value259, 0.026, value823 * value237, value253 * 0.48, value824 * 0.515, value257, {
        rounded: false,
        roughness: 0.45
      });
      addBoxMesh(rotation, value258, value260, 0.026, value823 * value237, value255 + value256 * 0.5, value824 * 0.515, value257, {
        rounded: false,
        roughness: 0.45
      });
    }
  } else if (type20.type === "shoecabinet") {
    const value238 = Math.max(value825, 1.9);
    const value239 = color5;
    const value240 = color5;
    const value241 = value823 * 0.5;
    const value242 = value823 * 0.5;
    const value243 = -value823 * 0.25;
    const value244 = value823 * 0.25;
    const value245 = value238 * 0.22;
    const value246 = value238 * 0.43;
    const value247 = value238 * 0.21;
    const value248 = value238 * 0.37;
    addBoxMesh(rotation, value823 * 0.98, value238 * 0.93, 0.045, 0, value238 * 0.48, -value824 * 0.47, value826, {
      rounded: false,
      roughness: 0.62
    });
    addBoxMesh(rotation, value241, value245 * 0.56, value824 * 0.92, value243, value245 * 0.35, 0, value826, {
      roughness: 0.56
    });
    addBoxMesh(rotation, value241 * 1.02, 0.055, value824 * 1.04, value243, value245 * 0.67, 0, value240, {
      roughness: 0.5
    });
    addBoxMesh(rotation, value242, value246, value824, value244, value246 * 0.5, 0, value826, {
      roughness: 0.58
    });
    for (const value232 of [-0.245, 0.245]) {
      addBoxMesh(rotation, value242 * 0.47, value246 * 0.84, 0.026, value244 + value242 * value232, value246 * 0.48, value824 * 0.515, value239, {
        rounded: false,
        roughness: 0.45
      });
    }
    const value249 = value238 - value247 - 0.045;
    addBoxMesh(rotation, value241, value247, value824, value243, value249 + value247 * 0.5, 0, value826, {
      roughness: 0.58
    });
    for (const value233 of [-0.245, 0.245]) {
      addBoxMesh(rotation, value241 * 0.47, value247 * 0.86, 0.026, value243 + value241 * value233, value249 + value247 * 0.5, value824 * 0.515, value239, {
        rounded: false,
        roughness: 0.45
      });
    }
    const value250 = value238 - value248 - 0.045;
    addBoxMesh(rotation, value242, value248, value824, value244, value250 + value248 * 0.5, 0, value826, {
      roughness: 0.58
    });
    for (const value234 of [-0.245, 0.245]) {
      addBoxMesh(rotation, value242 * 0.47, value248 * 0.9, 0.026, value244 + value242 * value234, value250 + value248 * 0.5, value824 * 0.515, value239, {
        rounded: false,
        roughness: 0.45
      });
    }
  } else if (type20.type === "cabinet") {
    addBoxMesh(rotation, value823, value825, value824, 0, value825 / 2, 0, value826);
    addBoxMesh(rotation, 0.018, value825 * 0.9, value824 * 1.01, 0, value825 * 0.52, value824 * 0.01, color7);
    addBoxMesh(rotation, 0.025, 0.16, 0.035, -0.08, value825 * 0.55, value824 * 0.515, color6, {
      metalness: 0.55
    });
    addBoxMesh(rotation, 0.025, 0.16, 0.035, 0.08, value825 * 0.55, value824 * 0.515, color6, {
      metalness: 0.55
    ...
    const value209 = Math.min(Math.max(Math.min(value823, value824) * 0.1, 0.032), 0.058);
    const value210 = Math.min(Math.max(value824 * 0.075, 0.018), 0.032);
    const value211 = Math.min(Math.max(value823 * 0.014, 0.016), 0.03);
    const value212 = Math.max(value823 - value209 * 2, value823 * 0.7);
    const value213 = 0.13;
    const value214 = 0.3;
    const value215 = 4;
    const value216 = value825 * value213;
    const value217 = value825 * value214;
    const value218 = value825 - value209 - value217;
    const value219 = value218 / value215;
    const length2 = [0.56, 0.24, 0.2];
    const slice = [0, length2[0], length2[0] + length2[1]];
    const value220 = slice.slice(1);
    const value221 = Math.max(value823 * 0.005, 0.005);
    const value222 = value824 * 0.5 + 0.012;
    const value223 = {
      rounded: false,
      roughness: 0.62,
      metalness: 0.015
    ...
    addBoxMesh(rotation, value209, value825, value824, -value823 * 0.5 + value209 * 0.5, value825 * 0.5, 0, value826, value223);
    addBoxMesh(rotation, value209, value825, value824, value823 * 0.5 - value209 * 0.5, value825 * 0.5, 0, value826, value223);
    addBoxMesh(rotation, value212, value825 - value209 * 2, value210, 0, value825 * 0.5, -value824 * 0.5 + value210 * 0.5, value826, {
      ...value223,
      roughness: 0.76
    });
    addBoxMesh(rotation, value823, value209, value824, 0, value825 - value209 * 0.5, 0, value826, value223);
    for (let value173 = 0; value173 < value215; value173 += 1) {
      addBoxMesh(rotation, value212, value209 * 0.72, value824 * 0.9, 0, value217 + value219 * value173, -value824 * 0.025, color5, value223);
    }
    addBoxMesh(rotation, value212, value209 * 0.72, value824 * 0.9, 0, value216, -value824 * 0.025, color5, value223);
    for (const value174 of value220) {
      const value151 = -value212 * 0.5 + value212 * value174;
      addBoxMesh(rotation, value209 * 0.72, value825 - value209, value824 * 0.9, value151, (value825 - value209) * 0.5, -value824 * 0.025, value826, value223);
    }
    const value227 = value217 - value216 - value209 * 0.9;
    const length3 = [value212 * length2[0], value212 * (1 - length2[0])];
    let value228 = -value212 * 0.5;
    for (let value175 = 0; value175 < length3.length; value175 += 1) {
      const value152 = length3[value175] - value221;
      const value153 = value228 + length3[value175] * 0.5;
      addBoxMesh(rotation, value152, value227, 0.03, value153, value216 + (value217 - value216) * 0.5, value222, value175 ? color5 : value826, {
        ...value224,
        roughness: 0.56
      });
      value228 += length3[value175];
    }
    for (let value176 = 0; value176 < length2.length; value176 += 1) {
      const value154 = -value212 * 0.5 + value212 * slice[value176];
      const value155 = value212 * length2[value176];
      const value156 = value154 + value155 * 0.5;
      const value157 = Math.max(value155 - value211 * 1.65, value155 * 0.76);
      const value158 = Math.max(value218 - value211 * 1.55, value218 * 0.86);
      addBoxMesh(rotation, value157, value158, 0.018, value156, value217 + value218 * 0.5, value222, glass.glass, value225);
      addBoxMesh(rotation, value211, value218, 0.032, value154 + value211 * 0.5, value217 + value218 * 0.5, value222 + 0.009, value826, value224);
      if (value176 === length2.length - 1) {
        addBoxMesh(rotation, value211, value218, 0.032, value154 + value155 - value211 * 0.5, value217 + value218 * 0.5, value222 + 0.009, value826, value224);
      }
      const value159 = Math.max(value157 * 0.025, 0.007);
      addBoxMesh(rotation, value159, value158 * 0.88, 0.006, value156 - value157 * 0.37, value217 + value218 * 0.52, value222 + 0.014, 14282227, value226);
      addBoxMesh(rotation, value157 * 0.34, Math.max(value211 * 0.11, 0.004), 0.006, value156 - value157 * 0.18, value217 + value218 * 0.88, value222 + 0.014, 15267578, value226);
    }
    addBoxMesh(rotation, value212, value211, 0.032, 0, value217 + value211 * 0.5, value222 + 0.009, value826, value224);
    addBoxMesh(rotation, value212, value211, 0.032, 0, value825 - value209 - value211 * 0.5, value222 + 0.009, value826, value224);
    const length4 = [14736852, 13025203, 10327434, 7301474, color5, color6];
    const value229 = (arg12, arg13, arg14 = 5, arg15 = 0) => {
      const value134 = value212 * length2[arg12];
      const value135 = -value212 * 0.5 + value212 * slice[arg12];
      const value136 = value217 + value219 * arg13 + value209 * 0.42;
      const value137 = Math.max(value134 * 0.022, 0.004);
      const value138 = (value134 * 0.72 - value137 * (arg14 - 1)) / arg14;
      let value139 = value135 + value134 * 0.13;
      for (let value117 = 0; value117 < arg14; value117 += 1) {
        const value112 = value138 * [0.8, 1.05, 0.9, 0.72, 0.96][(value117 + arg15) % 5];
        const value113 = value219 * [0.48, 0.58, 0.52, 0.64, 0.55][(value117 * 2 + arg15) % 5];
        addBoxMesh(rotation, value112, value113, value824 * 0.42, value139 + value112 * 0.5, value136 + value113 * 0.5, value824 * 0.12, length4[(value117 + arg15) % length4.length], {
          radius: Math.min(value112 * 0.12, 0.009),
          roughness: 0.78,
          metalness: 0
        });
        value139 += value112 + value137;
      }
    };
    const value230 = (arg16, arg17, arg18 = 3, arg19 = 0) => {
      const value140 = value212 * length2[arg16];
      const value141 = -value212 * 0.5 + value212 * slice[arg16] + value140 * 0.5;
      const value142 = value217 + value219 * arg17 + value209 * 0.42;
      for (let value118 = 0; value118 < arg18; value118 += 1) {
        addBoxMesh(rotation, value140 * 0.56, value209 * 0.48, value824 * 0.4, value141, value142 + value209 * (0.3 + value118 * 0.52), value824 * 0.12, length4[(value118 + arg19) % length4.length], {
          radius: 0.006,
          roughness: 0.78,
          metalness: 0
        });
    ...
    value230(0, 0, 3, 2);
    value230(1, 2, 3, 4);
    for (const [value177, value178, value179, value180] of [[1, 1, 0.16, 12169895], [0, 2, 0.075, 9406334], [2, 3, 0.14, 13749184]]) {
      const value160 = value212 * length2[value177];
      const value161 = -value212 * 0.5 + value212 * slice[value177] + value160 * 0.5;
      const value162 = value217 + value219 * value178 + value209 * 0.42;
      addSoftBoxMesh(rotation, value160 * value179 * 0.72, value160 * value179, value219 * 0.46, value161, value162 + value219 * 0.23, value824 * 0.12, value180, {
        segments: 24,
        roughness: 0.68
      });
    }
  } else if (type20.type === "bookcase") {
    const value181 = Math.min(Math.max(Math.min(value823, value824) * 0.11, 0.032), 0.062);
    const value182 = Math.min(Math.max(value824 * 0.075, 0.018), 0.032);
    const value183 = Math.max(value823 - value181 * 2, value823 * 0.72);
    const value184 = 0.255;
    const value185 = [value184, 0.47, 0.59, 0.79];
    const value186 = value825 - value181 * 0.5;
    const value187 = value183 * 0.27;
    const value188 = value823 * 0.5 - value181 * 1.5 - value187;
    const value189 = value183 - value187 - value181;
    const value190 = -value183 * 0.5 + value189 * 0.5;
    const value191 = value183 * 0.5 - value187 * 0.5;
    const value192 = {
      rounded: false,
      roughness: 0.62,
      metalness: 0.015
    };
    const value193 = value824 * 0.5 + 0.013;
    addBoxMesh(rotation, value181, value825, value824, -value823 * 0.5 + value181 * 0.5, value825 * 0.5, 0, value826, value192);
    addBoxMesh(rotation, value181, value825, value824, value823 * 0.5 - value181 * 0.5, value825 * 0.5, 0, value826, value192);
    addBoxMesh(rotation, value183, value825 * 0.95, value182, 0, value825 * 0.5, -value824 * 0.5 + value182 * 0.5, color7, {
      ...value192,
      roughness: 0.76
    });
    addBoxMesh(rotation, value183, value825 * value184 - value181 * 0.5, value824 * 0.9, 0, value825 * value184 * 0.5, -value824 * 0.025, color5, {
      ...value192,
      roughness: 0.66
    });
    for (const value163 of value185) {
      addBoxMesh(rotation, value183, value181, value824, 0, value825 * value163, 0, color5, value192);
    }
    addBoxMesh(rotation, value823, value181, value824, 0, value186, 0, value826, value192);
    const value194 = value825 * value185[1] + value181 * 0.5;
    const value195 = value825 - value181;
    addBoxMesh(rotation, value181, value195 - value194, value824, value188, (value195 + value194) * 0.5, 0, value826, value192);
    const value196 = 3;
    const value197 = value181 * 0.72;
    const value198 = value825 * value184 - value181 * 0.5;
    const value199 = Math.max(value198 - value197, value825 * 0.16);
    const value200 = Math.max(value823 * 0.008, 0.008);
    const value201 = -value183 * 0.18;
    const value202 = value201 + value183 * 0.5 - value200 * 0.5;
    const value203 = value183 - value202 - value200;
    const value204 = (value199 - value200 * (value196 - 1)) / value196;
    for (let value164 = 0; value164 < value196; value164 += 1) {
      const value143 = value197 + value204 * 0.5 + value164 * (value204 + value200);
      addBoxMesh(rotation, value202, value204, 0.026, -value183 * 0.5 + value202 * 0.5, value143, value193, value826, {
        ...value192,
        roughness: 0.55
      });
      addBoxMesh(rotation, value203, value204, 0.026, value201 + value200 * 0.5 + value203 * 0.5, value143, value193, color5, {
        ...value192,
        roughness: 0.55
      });
    }
    const value205 = value825 * (value185[2] - value185[1]) - value181 * 1.15;
    addBoxMesh(rotation, value189 * 0.97, value205, 0.028, value190, value825 * (value185[1] + value185[2]) * 0.5, value193, value826, {
      ...value192,
      roughness: 0.54
    });
    ...
    const value206 = ({
      ...
      maxWidth: arg2,
      shelfY: arg3,
      availableHeight: arg4,
      count: arg5 = 6,
      seed: arg6 = 0
    }) => {
      const value127 = Math.max(arg2 * 0.018, 0.006);
      const value128 = Math.max((arg2 - value127 * (arg5 + 1)) / arg5, 0.026);
      let value129 = arg + value127;
      for (let value114 = 0; value114 < arg5; value114 += 1) {
        const value105 = [0.72, 0.9, 0.78, 1.04, 0.82, 0.68][(value114 + arg6) % 6];
        const value106 = value128 * value105;
        const value107 = [0.72, 0.88, 0.78, 0.94, 0.82, 0.68][(value114 * 2 + arg6) % 6];
        const value108 = arg4 * value107;
        if (value129 + value106 > arg + arg2 - value127) {
          break;
        }
        const rotation2 = addBoxMesh(rotation, value106, value108, value824 * (0.47 + (value114 + arg6) % 3 * 0.045), value129 + value106 * 0.5, arg3 + value108 * 0.5, value824 * 0.15, length[(value114 + arg6) % length.length], {
          radius: Math.min(value106 * 0.14, 0.012),
          roughness: 0.78,
          metalness: 0
        });
        if (value114 === arg5 - 1 && arg6 % 2 === 1) {
          rotation2.rotation.z = -0.07;
        }
        value129 += value106 + value127;
      }
    };
    const value207 = (arg7, arg8, arg9, arg10 = 3, arg11 = 0) => {
      for (let value115 = 0; value115 < arg10; value115 += 1) {
        addBoxMesh(rotation, arg9, value181 * 0.54, value824 * 0.48, arg7, arg8 + value181 * (0.38 + value115 * 0.56), value824 * 0.15, length[(value115 + arg11) % length.length], {
          radius: 0.007,
          roughness: 0.78,
          metalness: 0
        });
    ...
    const shelfY = value825 * value185[0] + value181 * 0.5;
    value206({
      startX: -value183 * 0.47,
      maxWidth: value183 * 0.29,
      shelfY,
      availableHeight: value825 * 0.15,
      count: 5,
      seed: 2
    });
    value206({
      startX: value183 * 0.04,
      maxWidth: value183 * 0.38,
      shelfY,
      availableHeight: value825 * 0.16,
      count: 7,
      seed: 4
    });
    const shelfY2 = value825 * value185[1] + value181 * 0.5;
    value206({
      startX: value188 + value181 * 0.6,
      maxWidth: value187 * 0.82,
      shelfY: shelfY2,
      availableHeight: value825 * 0.075,
      count: 4,
      seed: 1
    });
    const shelfY3 = value825 * value185[2] + value181 * 0.5;
    value206({
      startX: -value183 * 0.47,
      maxWidth: value189 * 0.42,
      shelfY: shelfY3,
      availableHeight: value825 * 0.14,
      count: 6,
      seed: 0
    });
    addBoxMesh(rotation, value189 * 0.17, value825 * 0.12, 0.022, value190 + value189 * 0.27, shelfY3 + value825 * 0.065, value824 * 0.22, 11972517, {
      rounded: false,
      roughness: 0.5
    });
    addBoxMesh(rotation, value189 * 0.125, value825 * 0.085, 0.026, value190 + value189 * 0.27, shelfY3 + value825 * 0.065, value824 * 0.235, 7762283, {
      rounded: false,
      roughness: 0.42
    });
    value207(value191, shelfY3, value187 * 0.48, 3, 3);
    const value208 = value825 * value185[3] + value181 * 0.5;
    value207(value190 - value189 * 0.22, value208, value189 * 0.24, 2, 1);
    addSoftBoxMesh(rotation, value189 * 0.055, value189 * 0.075, value825 * 0.12, value190 - value189 * 0.08, value208 + value825 * 0.06, value824 * 0.12, 5196615, {
      segments: 22,
      roughness: 0.66
    });
    addSoftBoxMesh(rotation, value189 * 0.045, value189 * 0.064, value825 * 0.15, value190 + value189 * 0.08, value208 + value825 * 0.075, value824 * 0.12, 6643802, {
      segments: 22,
      roughness: 0.66
    });
    value207(value191, value208, value187 * 0.5, 2, 4);
  } else if (type20.type === "shelf") {
    const value165 = Math.min(Math.max(Math.min(value823, value824) * 0.07, 0.028), 0.052);
    const value166 = Math.min(Math.max(value825 * 0.022, 0.028), 0.052);
    const value167 = Math.max(value824 - value165 * 1.4, value824 * 0.78);
    const value168 = {
      rounded: false,
      metalness: 0.62,
      roughness: 0.28
    ...
    for (const value144 of [-value823 * 0.5 + value165 * 0.5, value823 * 0.5 - value165 * 0.5]) {
      for (const value119 of [-value824 * 0.5 + value165 * 0.5, value824 * 0.5 - value165 * 0.5]) {
        addBoxMesh(rotation, value165, value825, value165, value144, value825 * 0.5, value119, value826, value168);
      }
    }
    for (const value145 of [0.04, 0.26, 0.49, 0.72, 0.96]) {
      addBoxMesh(rotation, value823, value166, value167, 0, value825 * value145, 0, value145 === 0.96 ? color6 : color5, value169);
      addBoxMesh(rotation, value823, value165 * 0.65, value165, 0, value825 * value145, -value824 * 0.5 + value165 * 0.5, value826, value168);
    }
    const value170 = Math.max(value823 - value165 * 2, value165);
    const value171 = value825 * 0.84;
    const value172 = Math.hypot(value170, value171);
    for (const value146 of [-1, 1]) {
      const rotation2 = addBoxMesh(rotation, value165 * 0.48, value172, value165 * 0.42, 0, value825 * 0.5, -value824 * 0.5 + value165 * 0.18, value826, {
        ...value168,
        castShadow: false
      });
      rotation2.rotation.z = value146 * Math.atan2(value170, value171);
    }
  } else if (type20.type === "pillar") {
    const value147 = Math.min(glass.wallOpacity * 1.75, 0.55);
    const value148 = Ku(glass.wall, value147, {
      depthWrite: false,
      depthFunc: THREE.LessDepth
    });
    const value149 = Qu(glass.wall, value147, {
      topColor: glass.wallTop ?? glass.wall
    });
    const position2 = new THREE.Mesh(new THREE.BoxGeometry(value823, value825, value824), [value148, value148, value149, value148, value148, value148]);
    position2.position.y = value825 * 0.5;
    ...
    const value130 = value825 * 0.28;
    const value131 = value825 - value130;
    const value132 = 1.4 + value130;
    addBoxMesh(rotation, value823, value131, value824, 0, value132 + value131 * 0.5, 0, value826);
    addBoxMesh(rotation, value823, value130, 0.045, 0, 1.4 + value130 * 0.5, -value824 * 0.45, value826, {
      rounded: false,
      roughness: 0.62
    });
    addBoxMesh(rotation, value823 * 1.02, 0.045, value824 * 1.05, 0, 1.4, 0, color5, {
      roughness: 0.5
    });
    addBoxMesh(rotation, value823 * 1.02, 0.045, value824 * 1.05, 0, value132, 0, color5, {
      roughness: 0.5
    });
    addBoxMesh(rotation, 0.045, value130, value824, -value823 * 0.49, 1.4 + value130 * 0.5, 0, value826);
    addBoxMesh(rotation, 0.045, value130, value824, value823 * 0.49, 1.4 + value130 * 0.5, 0, value826);
    const value133 = value823 * 0.31;
    for (const value120 of [-0.33, 0, 0.33]) {
      addBoxMesh(rotation, value133, value131 * 0.9, 0.026, value823 * value120, value132 + value131 * 0.5, value824 * 0.515, color5, {
        rounded: false,
        roughness: 0.45
      });
    }
  } else if (["kitchenbase", "kitchensink", "kitchencooktop"].includes(type20.type)) {
    const value121 = value825 * 0.1;
    const value122 = value825 * 0.07;
    const value123 = value825 - value121 - value122;
    const value124 = value121 + value123 * 0.5;
    addBoxMesh(rotation, value823 * 0.96, value121, value824 * 0.8, 0, value121 * 0.5, -value824 * 0.06, color7, {
      rounded: false,
      roughness: 0.52
    });
    addBoxMesh(rotation, value823, value123, value824, 0, value124, 0, value826, {
      rounded: false,
      roughness: 0.58
    });
    addBoxMesh(rotation, value823 * 1.02, value122, value824 * 1.04, 0, value825 - value122 * 0.5, 0, color6, {
      rounded: false,
      roughness: 0.42
    });
    const value125 = Math.max(2, Math.min(6, Math.round(value823 / 0.6)));
    const value126 = value823 / value125;
    for (let value116 = 0; value116 < value125; value116 += 1) {
      const value109 = -value823 * 0.5 + value126 * (value116 + 0.5);
      addBoxMesh(rotation, value126 * 0.92, value123 * 0.9, 0.025, value109, value124, value824 * 0.515, color5, {
        rounded: false,
        roughness: 0.46
      });
      addBoxMesh(rotation, value126 * 0.28, 0.022, 0.03, value109, value121 + value123 * 0.83, value824 * 0.535, color7, {
        rounded: false,
        metalness: 0.28,
        roughness: 0.3
      });
    ...
    if (type20.type === "kitchensink") {
      addBoxMesh(rotation, value823 * 0.42, 0.025, value824 * 0.52, 0, value825 + 0.008, 0, color7, {
        rounded: false,
        metalness: 0.42,
        roughness: 0.28
      });
      addBoxMesh(rotation, value823 * 0.34, 0.02, value824 * 0.4, 0, value825 + 0.022, 0, color5, {
        rounded: false,
        metalness: 0.18,
        roughness: 0.34
      });
      addSoftBoxMesh(rotation, 0.018, 0.018, value825 * 0.22, value823 * 0.22, value825 + value825 * 0.11, -value824 * 0.17, color7, {
        segments: 20,
        metalness: 0.65,
        roughness: 0.2
      });
      addBoxMesh(rotation, value823 * 0.16, 0.035, 0.035, value823 * 0.14, value825 + value825 * 0.2, -value824 * 0.17, color7, {
        rounded: false,
        metalness: 0.65,
        roughness: 0.2
      });
    } else if (type20.type === "kitchencooktop") {
      addBoxMesh(rotation, value823 * 0.48, 0.025, value824 * 0.55, 0, value825 + 0.008, 0, color7, {
        rounded: false,
        metalness: 0.32,
        roughness: 0.25
      });
      for (const value97 of [-value823 * 0.13, value823 * 0.13]) {
        for (const value87 of [-value824 * 0.14, value824 * 0.14]) {
          addSoftBoxMesh(rotation, value823 * 0.06, value823 * 0.06, 0.018, value97, value825 + 0.028, value87, color5, {
            segments: 28,
            metalness: 0.42,
            roughness: 0.26
          });
          addSoftBoxMesh(rotation, value823 * 0.025, value823 * 0.025, 0.025, value97, value825 + 0.045, value87, color7, {
            segments: 24,
            metalness: 0.5,
            roughness: 0.22
          });
    ...
    addBoxMesh(rotation, value823, value825, value824, 0, value825 / 2, 0, color6, {
      metalness: 0.18,
      roughness: 0.5
    });
    addBoxMesh(rotation, value823 * 0.88, 0.018, value824 * 1.01, 0, value825 * 0.42, value824 * 0.01, color7, {
      metalness: 0.5
    });
    addBoxMesh(rotation, 0.025, value825 * 0.27, 0.035, value823 * 0.35, value825 * 0.65, value824 * 0.515, color7, {
      metalness: 0.7
    });
  } else if (type20.type === "storagewaterheater") {
    const value110 = Math.min(value824 * 0.43, value825 * 0.44);
    const value111 = value825 * 0.52;
    addBoxMesh(rotation, value823 * 0.78, value825 * 0.12, value824 * 0.22, 0, value825 * 0.1, -value824 * 0.36, color7, {
      rounded: false,
      metalness: 0.55,
      roughness: 0.3
    });
    addSoftBoxMesh(rotation, value110, value110, value823 * 0.82, 0, value111, 0, color6, {
      segments: 36,
      rotationZ: Math.PI / 2,
      metalness: 0.2,
      roughness: 0.42
    });
    for (const value102 of [-value823 * 0.43, value823 * 0.43]) {
      addSoftBoxMesh(rotation, value110 * 1.03, value110 * 1.03, 0.025, value102, value111, 0, color5, {
        segments: 36,
        rotationZ: Math.PI / 2,
        metalness: 0.28,
        roughness: 0.34
    ...
    addBoxMesh(rotation, value823 * 0.31, value825 * 0.23, 0.026, 0, value825 * 0.51, value824 * 0.44, color7, {
      rounded: false,
      metalness: 0.18,
      roughness: 0.25
    });
    addBoxMesh(rotation, value823 * 0.16, value825 * 0.045, 0.018, 0, value825 * 0.58, value824 * 0.46, 7706534, {
      rounded: false,
      emissive: 7706534,
      emissiveIntensity: 0.18,
      roughness: 0.2
    });
    for (const value103 of [-value823 * 0.28, value823 * 0.28]) {
      addSoftBoxMesh(rotation, 0.022, 0.022, value825 * 0.16, value103, value825 * 0.08, value824 * 0.12, value103 < 0 ? 4885698 : 12868184, {
        segments: 18,
        metalness: 0.55,
        roughness: 0.24
      });
      addSoftBoxMesh(rotation, 0.04, 0.04, 0.028, value103, 0.015, value824 * 0.12, color7, {
        segments: 20,
        metalness: 0.45,
        roughness: 0.28
      });
    ...
    const value104 = value825 * 0.82;
    addBoxMesh(rotation, value823, value104, value824, 0, value825 * 0.47, 0, color6, {
      rounded: false,
      metalness: 0.12,
      roughness: 0.46
    });
    addBoxMesh(rotation, value823 * 0.86, value104 * 0.42, 0.026, 0, value825 * 0.55, value824 * 0.515, color5, {
      rounded: false,
      roughness: 0.36
    });
    addBoxMesh(rotation, value823 * 0.44, value104 * 0.18, 0.018, 0, value825 * 0.67, value824 * 0.54, color7, {
      rounded: false,
      metalness: 0.14,
      roughness: 0.22
    });
    for (let value98 = 0; value98 < 5; value98 += 1) {
      addBoxMesh(rotation, value823 * 0.62, 0.012, 0.02, 0, value825 * (0.24 + value98 * 0.07), value824 * 0.525, color7, {
        rounded: false,
        roughness: 0.3
      });
    }
    addSoftBoxMesh(rotation, value824 * 0.17, value824 * 0.17, value825 * 0.18, 0, value825 * 0.91, 0, color7, {
      segments: 24,
      metalness: 0.58,
      roughness: 0.24
    });
    addSoftBoxMesh(rotation, value824 * 0.22, value824 * 0.22, 0.035, 0, value825 * 0.84, 0, color5, {
      segments: 24,
      metalness: 0.5,
      roughness: 0.25
    });
    for (const [value99, value100] of [[-value823 * 0.28, 4885698], [0, 9410205], [value823 * 0.28, 12868184]]) {
      addSoftBoxMesh(rotation, 0.018, 0.018, value825 * 0.18, value99, value825 * 0.09, value824 * 0.12, value100, {
        segments: 18,
        metalness: 0.62,
        roughness: 0.22
      });
      addSoftBoxMesh(rotation, 0.034, 0.034, 0.025, value99, 0.014, value824 * 0.12, color7, {
        segments: 18,
        metalness: 0.5,
        roughness: 0.25
      });
    ...
    const value101 = value825 * 0.9;
    addBoxMesh(rotation, value823, value101, value824, 0, value101 * 0.5, 0, color6, {
      rounded: false,
      metalness: 0.04,
      roughness: 0.4
    });
    addBoxMesh(rotation, value823 * 0.92, value101 * 0.27, 0.028, 0, value825 * 0.76, value824 * 0.515, color7, {
      rounded: false,
      metalness: 0.16,
      roughness: 0.18
    });
    addBoxMesh(rotation, value823 * 0.34, value101 * 0.045, 0.014, -value823 * 0.08, value825 * 0.79, value824 * 0.54, 10470608, {
      rounded: false,
      emissive: 10470608,
      emissiveIntensity: 0.2,
      roughness: 0.22
    });
    for (const value92 of [-0.26, 0, 0.26]) {
      addSoftBoxMesh(rotation, Math.min(value823, value824) * 0.055, Math.min(value823, value824) * 0.055, 0.018, value823 * value92, value825 * 0.66, value824 * 0.535, color5, {
        segments: 24,
        rotationX: Math.PI / 2,
        metalness: 0.22,
        roughness: 0.28
    ...
    for (const value93 of [-value823 * 0.2, value823 * 0.2]) {
      addSoftBoxMesh(rotation, 0.014, 0.014, value825 * 0.12, value93, value825 * 0.49, value824 * 0.48, color7, {
        segments: 18,
        metalness: 0.46,
        roughness: 0.22
      });
      addSoftBoxMesh(rotation, 0.024, 0.018, 0.035, value93, value825 * 0.425, value824 * 0.52, color7, {
        segments: 18,
        rotationX: Math.PI / 2,
        metalness: 0.46,
        roughness: 0.22
    ...
    addBoxMesh(rotation, value823 * 0.68, value825 * 0.045, value824 * 0.52, 0, value825 * 0.11, value824 * 0.16, color7, {
      rounded: false,
      metalness: 0.2,
      roughness: 0.3
    });
    addBoxMesh(rotation, value823 * 0.42, 0.028, value824 * 0.28, 0, value825 * 0.16, value824 * 0.28, color5, {
      rounded: false,
      roughness: 0.34
    });
  } else if (type20.type === "tea_bar_machine") {
    const value94 = value825 * 0.67;
    const value95 = value94 + value825 * 0.045;
    const value96 = value825 - value95;
    addBoxMesh(rotation, value823, value94, value824, 0, value94 * 0.5, 0, color6, {
      rounded: false,
      roughness: 0.5,
      metalness: 0.04
    });
    addBoxMesh(rotation, value823 * 0.94, 0.028, value824 * 1.02, 0, value94 * 0.5, value824 * 0.515, color7, {
      rounded: false,
      roughness: 0.34
    });
    addBoxMesh(rotation, value823 * 0.94, 0.032, value824 * 1.02, 0, value94, 0, color5, {
      rounded: false,
      roughness: 0.42
    });
    addBoxMesh(rotation, value823 * 0.94, value825 * 0.04, value824 * 1.04, 0, value95, 0, color6, {
      rounded: false,
      roughness: 0.36,
      metalness: 0.1
    });
    addBoxMesh(rotation, value823 * 0.92, value96, 0.032, 0, value95 + value96 * 0.5, -value824 * 0.46, color5, {
      rounded: false,
      roughness: 0.56
    });
    for (const value88 of [-value823 * 0.46, value823 * 0.46]) {
      addBoxMesh(rotation, value823 * 0.075, value96, value824 * 0.78, value88, value95 + value96 * 0.5, 0, color6, {
        rounded: false,
        roughness: 0.48
      });
    }
    addBoxMesh(rotation, value823 * 0.86, value825 * 0.14, value824 * 0.18, 0, value825 * 0.9, value824 * 0.48, color7, {
      rounded: false,
      metalness: 0.32,
      roughness: 0.22
    });
    for (const value89 of [-value823 * 0.22, value823 * 0.22]) {
      addSoftBoxMesh(rotation, Math.min(value823, value824) * 0.035, Math.min(value823, value824) * 0.035, 0.018, value89, value825 * 0.9, value824 * 0.585, color5, {
        segments: 20,
        rotationX: Math.PI / 2,
        metalness: 0.2,
        roughness: 0.25
      });
      addSoftBoxMesh(rotation, 0.012, 0.012, value825 * 0.11, value89, value825 * 0.79, value824 * 0.5, color7, {
        segments: 18,
        metalness: 0.48,
        roughness: 0.22
      });
      const value86 = Math.min(value823, value824) * 0.16;
      addSoftBoxMesh(rotation, value86 * 0.9, value86, value825 * 0.13, value89, value825 * 0.75, value824 * 0.12, value89 < 0 ? 8752009 : color5, {
        segments: 28,
        metalness: 0.08,
        roughness: 0.34
      });
      addSoftBoxMesh(rotation, value86 * 0.84, value86 * 0.84, 0.016, value89, value825 * 0.82, value824 * 0.12, color7, {
        segments: 28,
        metalness: 0.18,
        roughness: 0.28
      });
      addBoxMesh(rotation, value823 * 0.12, value825 * 0.06, 0.016, value89, value825 * 0.77, value824 * 0.3, color7, {
        radius: 0.012,
        metalness: 0.24,
        roughness: 0.24
      });
    }
    addBoxMesh(rotation, value823 * 0.92, value825 * 0.055, value824 * 0.82, 0, 0.028, 0, color7, {
      rounded: false,
      roughness: 0.36
    });
  } else if (type20.type === "washer" || type20.type === "dryer") {
    const value90 = type20.type === "dryer";
    const value91 = value823 * (value90 ? 0.32 : 0.29);
    addBoxMesh(rotation, value823, value825, value824, 0, value825 * 0.5, 0, color6, {
      rounded: false,
      metalness: 0.1,
      roughness: 0.48
    });
    addBoxMesh(rotation, value823 * 0.92, value825 * 0.18, 0.035, 0, value825 * 0.86, value824 * 0.505, color5, {
      rounded: false,
      roughness: 0.4
    });
    addSoftBoxMesh(rotation, value91, value91, 0.045, 0, value825 * 0.47, value824 * 0.515, color7, {
      segments: 40,
      rotationX: Math.PI / 2,
      metalness: 0.32,
      roughness: 0.28
    });
    addSoftBoxMesh(rotation, value91 * 0.74, value91 * 0.74, 0.03, 0, value825 * 0.47, value824 * 0.545, value90 ? 2502715 : 3622485, {
      segments: 40,
      rotationX: Math.PI / 2,
      metalness: 0.08,
      roughness: 0.22
    });
    addSoftBoxMesh(rotation, value823 * 0.055, value823 * 0.055, 0.035, value823 * 0.27, value825 * 0.86, value824 * 0.535, color7, {
      segments: 24,
      rotationX: Math.PI / 2,
      metalness: 0.4,
      roughness: 0.25
    });
    addBoxMesh(rotation, value823 * 0.25, value825 * 0.035, 0.026, -value823 * 0.23, value825 * 0.86, value824 * 0.535, color7, {
      rounded: false,
      roughness: 0.3
    });
    if (!value90) {
      addBoxMesh(rotation, value823 * 0.2, value825 * 0.055, 0.025, -value823 * 0.3, value825 * 0.12, value824 * 0.525, color5, {
        rounded: false,
        roughness: 0.42
      });
    }
    ...
    addBoxMesh(rotation, value823 * 0.93, value825 * 0.74, 0.028, 0, value825 * 0.46, value824 * 0.515, color5, {
      rounded: false,
      metalness: 0.08,
      roughness: 0.44
    });
    addBoxMesh(rotation, value823 * 0.93, value825 * 0.14, 0.032, 0, value825 * 0.87, value824 * 0.52, color7, {
      rounded: false,
      metalness: 0.18,
      roughness: 0.3
    });
    addBoxMesh(rotation, value823 * 0.58, value825 * 0.026, 0.034, 0, value825 * 0.78, value824 * 0.54, color7, {
      rounded: false,
      metalness: 0.5,
      roughness: 0.22
    });
    addBoxMesh(rotation, value823 * 0.9, value825 * 0.075, value824 * 0.78, 0, value825 * 0.038, -value824 * 0.04, color7, {
      rounded: false,
      roughness: 0.4
    });
    for (const value85 of [0.22, 0.31, 0.4]) {
      addSoftBoxMesh(rotation, value823 * 0.018, value823 * 0.018, 0.012, value823 * value85, value825 * 0.88, value824 * 0.545, color5, {
        segments: 18,
        rotationX: Math.PI / 2,
        metalness: 0.26,
        roughness: 0.24
    ...
    addBoxMesh(rotation, value823 * 0.94, value825 * 0.9, 0.026, 0, value825 * 0.5, value824 * 0.515, color7, {
      rounded: false,
      metalness: 0.18,
      roughness: 0.25
    });
    addBoxMesh(rotation, value823 * 0.78, value825 * 0.56, 0.018, -value823 * 0.03, value825 * 0.42, value824 * 0.54, 1515819, {
      rounded: false,
      metalness: 0.12,
      roughness: 0.18
    });
    addBoxMesh(rotation, value823 * 0.72, value825 * 0.035, 0.038, -value823 * 0.03, value825 * 0.74, value824 * 0.56, color5, {
      rounded: false,
      metalness: 0.52,
      roughness: 0.22
    });
    addBoxMesh(rotation, value823 * 0.24, value825 * 0.075, 0.022, 0, value825 * 0.86, value824 * 0.545, 7706534, {
      rounded: false,
      emissive: 7706534,
      emissiveIntensity: 0.18,
      roughness: 0.2
    });
    for (const value84 of [-0.36, 0.36]) {
      addSoftBoxMesh(rotation, value823 * 0.045, value823 * 0.045, 0.026, value823 * value84, value825 * 0.86, value824 * 0.55, color5, {
        segments: 24,
        rotationX: Math.PI / 2,
        metalness: 0.4,
        roughness: 0.24
    ...
    addBoxMesh(rotation, value823 * 0.93, value825 * 0.82, 0.025, 0, value825 * 0.49, value824 * 0.515, color7, {
      rounded: false,
      metalness: 0.16,
      roughness: 0.24
    });
    addBoxMesh(rotation, value823 * 0.62, value825 * 0.63, 0.018, -value823 * 0.13, value825 * 0.48, value824 * 0.54, 1515819, {
      rounded: false,
      metalness: 0.12,
      roughness: 0.18
    });
    addBoxMesh(rotation, value823 * 0.035, value825 * 0.54, 0.03, value823 * 0.19, value825 * 0.48, value824 * 0.55, color5, {
      rounded: false,
      metalness: 0.46,
      roughness: 0.22
    });
    addBoxMesh(rotation, value823 * 0.17, value825 * 0.1, 0.02, value823 * 0.36, value825 * 0.72, value824 * 0.54, 7706534, {
      rounded: false,
      emissive: 7706534,
      emissiveIntensity: 0.16,
      roughness: 0.2
    });
    for (const value82 of [0.48, 0.34, 0.2]) {
      addBoxMesh(rotation, value823 * 0.13, value825 * 0.055, 0.018, value823 * 0.36, value825 * value82, value824 * 0.545, color5, {
        rounded: false,
        roughness: 0.3
      });
    }
  } else if (type20.type === "ricecooker") {
    const value83 = Math.min(value823, value824) * 0.47;
    addSoftBoxMesh(rotation, value83 * 0.9, value83, value825 * 0.68, 0, value825 * 0.38, 0, color6, {
      segments: 36,
      roughness: 0.46
    });
    addSoftBoxMesh(rotation, value83 * 0.94, value83 * 0.94, value825 * 0.12, 0, value825 * 0.77, 0, color5, {
      segments: 36,
      roughness: 0.38
    });
    addSoftBoxMesh(rotation, value83 * 0.72, value83 * 0.74, value825 * 0.035, 0, value825 * 0.85, 0, color7, {
      segments: 32,
      metalness: 0.12,
      roughness: 0.28
    });
    addBoxMesh(rotation, value823 * 0.5, value825 * 0.16, 0.026, 0, value825 * 0.42, value824 * 0.47, color7, {
      radius: Math.min(value823, value824) * 0.04,
      roughness: 0.28
    });
    addBoxMesh(rotation, value823 * 0.24, value825 * 0.06, 0.018, 0, value825 * 0.43, value824 * 0.49, 7706534, {
      rounded: false,
      emissive: 7706534,
      emissiveIntensity: 0.16,
      roughness: 0.2
    });
    addBoxMesh(rotation, value823 * 0.05, value825 * 0.16, value824 * 0.1, -value823 * 0.27, value825 * 0.86, 0, color7, {
      roughness: 0.3
    });
    addBoxMesh(rotation, value823 * 0.05, value825 * 0.16, value824 * 0.1, value823 * 0.27, value825 * 0.86, 0, color7, {
      roughness: 0.3
    });
    addBoxMesh(rotation, value823 * 0.58, value825 * 0.055, value824 * 0.1, 0, value825 * 0.94, 0, color7, {
      roughness: 0.3
    });
  } else if (type20.type === "rangehood") {
    addBoxMesh(rotation, value823 * 0.34, value825 * 0.72, value824 * 0.42, 0, value825 * 0.6, -value824 * 0.18, value826, {
      rounded: false,
      metalness: 0.22,
      roughness: 0.42
    });
    addBoxMesh(rotation, value823, value825 * 0.22, value824, 0, value825 * 0.18, 0, color6, {
      rounded: false,
      metalness: 0.2,
      roughness: 0.4
    });
    addBoxMesh(rotation, value823 * 0.9, value825 * 0.07, value824 * 0.8, 0, value825 * 0.055, value824 * 0.02, color7, {
      rounded: false,
      metalness: 0.35,
      roughness: 0.28
    });
    addBoxMesh(rotation, value823 * 0.22, value825 * 0.035, 0.025, value823 * 0.3, value825 * 0.2, value824 * 0.515, color5, {
      rounded: false,
      emissive: color5,
    ...
    addBoxMesh(rotation, value823 * 0.9, value825 * 0.08, value824 * 0.18, 0, value825 * 0.18, value824 * 0.46, color7, {
      rounded: false,
      roughness: 0.3
    });
    addBoxMesh(rotation, value823 * 0.12, value825 * 0.08, value824 * 0.05, value823 * 0.34, value825 * 0.68, value824 * 0.51, color5, {
      rounded: false,
      emissive: color5,
      emissiveIntensity: 0.18
    });
  } else if (type20.type === "floorac") {
    addSoftBoxMesh(rotation, value823 * 0.46, value823 * 0.48, value825, 0, value825 * 0.5, 0, color6, {
      segments: 32,
      roughness: 0.48
    });
    addBoxMesh(rotation, value823 * 0.5, value825 * 0.42, 0.025, 0, value825 * 0.68, value824 * 0.48, color7, {
      rounded: false,
      roughness: 0.3
    });
    for (const value81 of [0.58, 0.68, 0.78]) {
      addBoxMesh(rotation, value823 * 0.42, 0.018, 0.03, 0, value825 * value81, value824 * 0.5, color5, {
        rounded: false,
        roughness: 0.34
      });
    }
  } else if (type20.type === "robotvacuum") {
    addBoxMesh(rotation, value823 * 0.82, 0.035, value824 * 0.92, 0, 0.018, 0, color5, {
      radius: Math.min(value823, value824) * 0.05,
      roughness: 0.58
    });
    addBoxMesh(rotation, value823 * 0.68, value825 * 0.82, value824 * 0.48, 0, value825 * 0.47, -value824 * 0.23, color6, {
      radius: Math.min(value823, value824) * 0.12,
      roughness: 0.48
    });
    addBoxMesh(rotation, value823 * 0.44, value825 * 0.16, 0.03, 0, value825 * 0.2, value824 * 0.02, color5, {
      radius: Math.min(value823, value824) * 0.04,
      roughness: 0.42
    });
    addBoxMesh(rotation, value823 * 0.34, value825 * 0.07, 0.035, 0, value825 * 0.14, value824 * 0.04, color7, {
      radius: Math.min(value823, value824) * 0.025,
      roughness: 0.28
    });
    addSoftBoxMesh(rotation, value823 * 0.31, value823 * 0.32, value825 * 0.12, 0, value825 * 0.07, value824 * 0.2, color6, {
      segments: 32,
      roughness: 0.42
    });
    addSoftBoxMesh(rotation, value823 * 0.085, value823 * 0.09, value825 * 0.055, -value823 * 0.08, value825 * 0.16, value824 * 0.15, color5, {
      segments: 24,
      roughness: 0.34
    });
    addBoxMesh(rotation, value823 * 0.36, value825 * 0.045, 0.025, 0, value825 * 0.08, value824 * 0.52, color7, {
      radius: Math.min(value823, value824) * 0.025,
      roughness: 0.24
    });
  } else if (type20.type === "nas") {
    addBoxMesh(rotation, value823, value825, value824, 0, value825 * 0.5, 0, value826, {
      rounded: false,
      metalness: 0.16,
      roughness: 0.46
    });
    addBoxMesh(rotation, value823 * 0.9, value825 * 0.88, 0.025, 0, value825 * 0.5, value824 * 0.515, color7, {
      rounded: false,
      metalness: 0.12,
      roughness: 0.32
    });
    for (const value78 of [-value823 * 0.23, value823 * 0.23]) {
      for (const value71 of [value825 * 0.3, value825 * 0.7]) {
        addBoxMesh(rotation, value823 * 0.38, value825 * 0.34, 0.018, value78, value71, value824 * 0.535, color5, {
          rounded: false,
          roughness: 0.38
        });
        addBoxMesh(rotation, value823 * 0.18, 0.018, 0.012, value78, value71 + value825 * 0.1, value824 * 0.55, color7, {
          rounded: false,
          metalness: 0.25,
          roughness: 0.3
        });
    ...
    for (const value79 of [0.18, 0.26, 0.34]) {
      addBoxMesh(rotation, 0.012, 0.012, 0.012, value823 * 0.42, value825 * value79, value824 * 0.55, 9550021, {
        rounded: false,
        emissive: 9550021,
    ...
    const value80 = Math.min(value823, value824) * 0.47;
    addSoftBoxMesh(rotation, value80 * 0.96, value80, value825 * 0.92, 0, value825 * 0.46, 0, color6, {
      segments: 36,
      roughness: 0.5
    });
    addSoftBoxMesh(rotation, value80 * 0.92, value80 * 0.92, value825 * 0.055, 0, value825 * 0.965, 0, color7, {
      segments: 36,
      metalness: 0.18,
      roughness: 0.3
    });
    addSoftBoxMesh(rotation, value80 * 0.55, value80 * 0.55, value825 * 0.018, 0, value825 * 1.005, 0, color5, {
      segments: 32,
      metalness: 0.08,
      roughness: 0.35
    });
    for (const value74 of [-0.28, -0.14, 0, 0.14, 0.28]) {
      addBoxMesh(rotation, 0.012, value825 * 0.45, 0.012, value823 * value74, value825 * 0.35, value824 * 0.46, color5, {
        rounded: false,
        roughness: 0.45
      });
    }
    ...
    const value77 = value76 - value75 * 0.5;
    if (type20.tvMountStyle === "mobile") {
      const value64 = Math.max(value825 * 0.045, 0.055);
      const value65 = value823 * 0.7;
      const value66 = Math.max(value824 * 0.78, 0.3);
      const value67 = Math.max(value77 - value64 * 0.7, value825 * 0.22);
      const value68 = value64 * 0.7 + value67 * 0.5;
      addBoxMesh(rotation, value65, value64, value66, 0, value64 * 0.72, 0, color7, {
        radius: Math.min(value64, value66) * 0.22,
        metalness: 0.18,
        roughness: 0.32
      });
      addBoxMesh(rotation, value823 * 0.075, value67, Math.max(value824 * 0.2, 0.06), -value823 * 0.035, value68, -value824 * 0.03, color7, {
        rounded: false,
        metalness: 0.2,
        roughness: 0.3
      });
      addBoxMesh(rotation, value823 * 0.105, value67 * 0.86, Math.max(value824 * 0.12, 0.04), value823 * 0.025, value68 + value67 * 0.02, value824 * 0.015, color5, {
        rounded: false,
        metalness: 0.35,
        roughness: 0.28
      });
      addBoxMesh(rotation, value823 * 0.34, Math.max(value825 * 0.018, 0.025), Math.max(value824 * 0.5, 0.2), 0, value77 * 0.76, value824 * 0.04, color7, {
        radius: 0.012,
        metalness: 0.22,
        roughness: 0.3
      });
      const value69 = Math.max(Math.min(value823, value824) * 0.045, 0.025);
      for (const value57 of [-value65 * 0.42, value65 * 0.42]) {
        for (const value55 of [-value66 * 0.34, value66 * 0.34]) {
          addSoftBoxMesh(rotation, value69, value69, Math.max(value69 * 0.56, 0.018), value57, value69, value55, 1448479, {
            segments: 20,
            rotationZ: Math.PI / 2,
            roughness: 0.38,
            metalness: 0.1
      ...
      const value58 = Math.max(value825 * 0.035, 0.028);
      const value59 = value823 * 0.34;
      const value60 = Math.max(value824 * 0.72, 0.16);
      const value61 = Math.max(value77 - value58, value825 * 0.12);
      addBoxMesh(rotation, value59, value58, value60, 0, value58 * 0.5, 0, color7, {
        radius: Math.min(value58, value60) * 0.26,
        metalness: 0.2,
        roughness: 0.3
      });
      addBoxMesh(rotation, value823 * 0.075, value61, Math.max(value824 * 0.24, 0.05), 0, value58 + value61 * 0.5, -value824 * 0.03, color5, {
        rounded: false,
        metalness: 0.34,
        roughness: 0.28
      });
      addBoxMesh(rotation, value823 * 0.18, Math.max(value825 * 0.025, 0.022), Math.max(value824 * 0.34, 0.08), 0, value77, 0, color7, {
        radius: 0.008,
        metalness: 0.22,
        roughness: 0.3
      });
    }
    addBoxMesh(rotation, value823, value75, Math.max(value824 * 0.28, 0.05), 0, value76, 0, color7, {
      radius: Math.min(value823, value75) * 0.012,
      roughness: 0.28
    });
  } else if (type20.type === "vanity") {
    const value72 = Math.min(0.76, value825 * 0.5);
    addBoxMesh(rotation, value823, 0.075, value824, 0, value72, 0, value826);
    addBoxMesh(rotation, value823 * 0.27, value72 * 0.82, value824 * 0.88, -value823 * 0.34, value72 * 0.42, 0, value826);
    addBoxMesh(rotation, value823 * 0.27, value72 * 0.82, value824 * 0.88, value823 * 0.34, value72 * 0.42, 0, value826);
    for (const value70 of [-0.34, 0.34]) {
      for (const value56 of [0.23, 0.48]) {
        addBoxMesh(rotation, value823 * 0.22, 0.012, value824 * 0.02, value823 * value70, value72 * value56, value824 * 0.46, color7, {
          rounded: false
        });
      }
    }
    const value73 = Math.max(value825 - value72 - 0.08, 0.45);
    addBoxMesh(rotation, value823 * 0.54, value73, 0.025, 0, value72 + value73 * 0.5, -value824 * 0.43, glass.glass, {
      transparent: true,
      opacity: 0.42,
      depthWrite: false,
      metalness: 0.22,
    ...
    addBoxMesh(rotation, value823 * 0.59, 0.045, 0.05, 0, value72 + value73, -value824 * 0.43, color6, {
      metalness: 0.12
    });
    addBoxMesh(rotation, value823 * 0.59, 0.045, 0.05, 0, value72, -value824 * 0.43, color6, {
      metalness: 0.12
    });
    addBoxMesh(rotation, 0.045, value73, 0.05, -value823 * 0.295, value72 + value73 * 0.5, -value824 * 0.43, color6, {
      metalness: 0.12
    });
    addBoxMesh(rotation, 0.045, value73, 0.05, value823 * 0.295, value72 + value73 * 0.5, -value824 * 0.43, color6, {
      metalness: 0.12
    ...
    addBoxMesh(rotation, value823, value825 * 0.1, value824, 0, value825 * 0.93, 0, value826);
    addBoxMesh(rotation, value823 * 0.05, value825 * 0.88, value824 * 0.82, -value823 * 0.44, value825 * 0.44, 0, color7);
    addBoxMesh(rotation, value823 * 0.05, value825 * 0.88, value824 * 0.82, value823 * 0.44, value825 * 0.44, 0, color7);
    addBoxMesh(rotation, value823 * 0.34, value825 * 0.18, value824 * 0.78, value823 * 0.22, value825 * 0.74, 0, value826);
    addBoxMesh(rotation, value823 * 0.27, 0.018, value824 * 0.04, value823 * 0.22, value825 * 0.73, value824 * 0.41, color6, {
      metalness: 0.35
    });
  } else if (type20.type === "desktop") {
    const value62 = value823 * 0.72;
    const value63 = value825 * 0.58;
    addBoxMesh(rotation, value62, value63, 0.035, -value823 * 0.06, value825 * 0.66, -value824 * 0.25, color7, {
      rounded: false,
      roughness: 0.24
    });
    addBoxMesh(rotation, value62 * 0.9, value63 * 0.84, 0.01, -value823 * 0.06, value825 * 0.66, -value824 * 0.19, 659481, {
      rounded: false,
      roughness: 0.18,
      emissive: 1517112,
      emissiveIntensity: 0.35
    });
    addBoxMesh(rotation, 0.035, value825 * 0.24, 0.035, -value823 * 0.06, value825 * 0.25, -value824 * 0.25, color7, {
      metalness: 0.5
    });
    addBoxMesh(rotation, value823 * 0.28, 0.025, value824 * 0.3, -value823 * 0.06, 0.02, -value824 * 0.22, color7, {
      metalness: 0.42
    });
    addBoxMesh(rotation, value823 * 0.58, 0.022, value824 * 0.38, -value823 * 0.08, 0.025, value824 * 0.24, color5, {
      rounded: false,
      roughness: 0.5
    });
    addBoxMesh(rotation, value823 * 0.1, 0.035, value824 * 0.22, value823 * 0.36, 0.028, value824 * 0.24, color5, {
      radius: Math.min(value823, value824) * 0.035,
      roughness: 0.46
    });
  } else if (type20.type === "laptop") {
    addBoxMesh(rotation, value823, 0.025, value824 * 0.72, 0, 0.018, value824 * 0.08, value826, {
      metalness: 0.28,
      roughness: 0.36
    });
    const rotation2 = addBoxMesh(rotation, value823 * 0.96, value825 * 0.78, 0.018, 0, value825 * 0.43, -value824 * 0.29, color7, {
      rounded: false,
      metalness: 0.22,
      roughness: 0.25
    });
    rotation2.rotation.x = -Math.PI * 0.08;
    addBoxMesh(rotation, value823 * 0.86, value825 * 0.63, 0.01, 0, value825 * 0.43, -value824 * 0.278, 659740, {
      rounded: false,
      emissive: 1585226,
      emissiveIntensity: 0.38,
      roughness: 0.18
    });
    addBoxMesh(rotation, value823 * 0.62, 0.009, value824 * 0.34, 0, 0.035, value824 * 0.12, color7, {
      rounded: false
    });
  } else if (type20.type === "toilet") {
    const castShadow3 = new THREE.Mesh(createWallTopMaterial(value823, value824, value825), new THREE.MeshStandardMaterial({
      color: color6,
      roughness: 0.4,
      metalness: 0.02
    }));
    ...
    bezierCurveTo.moveTo(-value823 * 0.46, -value824 * 0.44);
    bezierCurveTo.lineTo(value823 * 0.46, -value824 * 0.44);
    bezierCurveTo.bezierCurveTo(value823 * 0.49, -value824 * 0.05, value823 * 0.49, value824 * 0.29, 0, value824 * 0.47);
    bezierCurveTo.bezierCurveTo(-value823 * 0.49, value824 * 0.29, -value823 * 0.49, -value824 * 0.05, -value823 * 0.46, -value824 * 0.44);
    bezierCurveTo.closePath();
    const rotation2 = new THREE.Mesh(new THREE.ExtrudeGeometry(bezierCurveTo, {
      depth: value825 * 0.085,
      bevelEnabled: true,
      bevelSegments: 2,
      bevelSize: 0.012,
      bevelThickness: 0.01,
    ...
    rotation2.position.set(0, value825 * 0.82, 0);
    rotation2.castShadow = true;
    rotation2.receiveShadow = true;
    rotation.add(rotation2);
    addBoxMesh(rotation, value823 * 0.9, value825 * 0.095, value824 * 0.2, 0, value825 * 0.775, -value824 * 0.34, color6, {
      radius: Math.min(value823, value824) * 0.035,
      roughness: 0.36
    });
    addBoxMesh(rotation, 0.016, value825 * 0.38, 0.024, -value823 * 0.42, value825 * 0.38, -value824 * 0.18, color7, {
      rounded: false,
      roughness: 0.46
    });
    addSoftBoxMesh(rotation, value823 * 0.035, value823 * 0.035, 0.018, -value823 * 0.46, value825 * 0.66, value824 * 0.12, color5, {
      segments: 24,
      rotationZ: Math.PI / 2,
      metalness: 0.08,
      roughness: 0.3
    ...
    addBoxMesh(rotation, value823, value825 * 0.58, value824, 0, value825 * 0.29, 0, color6, {
      radius: Math.min(value823, value824) * 0.08,
      roughness: 0.38
    });
    addBoxMesh(rotation, value823 * 0.28, value825 * 0.12, value824 * 0.58, -value823 * 0.34, value825 * 0.66, 0, color5, {
      radius: Math.min(value823, value824) * 0.035,
      roughness: 0.4
    });
    addBoxMesh(rotation, value823 * 0.28, value825 * 0.12, value824 * 0.58, value823 * 0.34, value825 * 0.66, 0, color5, {
    ...
    const scale3 = new THREE.Mesh(new THREE.CylinderGeometry(value823 * 0.16, value823 * 0.2, value825 * 0.18, 28), new THREE.MeshStandardMaterial({
      color: color7,
      roughness: 0.34,
      metalness: 0.02
    }));
    scale3.scale.z = 1.75;
    scale3.position.y = value825 * 0.67;
    rotation.add(scale3);
  } else if (type20.type === "urinal") {
    addBoxMesh(rotation, value823 * 0.78, value825 * 0.88, value824 * 0.72, 0, value825 * 0.5, -value824 * 0.06, color6, {
      radius: Math.min(value823, value824) * 0.16,
      roughness: 0.32
    });
    const scale2 = new THREE.Mesh(new THREE.SphereGeometry(Math.min(value823, value824) * 0.3, 24, 16, 0, Math.PI * 2, 0, Math.PI * 0.58), new THREE.MeshStandardMaterial({
      color: color5,
      roughness: 0.28,
      metalness: 0.02,
      side: THREE.DoubleSide
    ...
    scale2.position.set(0, value825 * 0.53, value824 * 0.17);
    rotation.add(scale2);
    addSoftBoxMesh(rotation, 0.018, 0.018, value825 * 0.22, 0, value825 * 0.95, -value824 * 0.18, color7, {
      segments: 16,
      metalness: 0.72,
      roughness: 0.22
    });
  } else if (type20.type === "bathtub") {
    const value48 = value825 * 0.84;
    const value49 = Math.min(value823, value824) * 0.11;
    const value50 = value48 * 0.8;
    const value51 = Math.max(value823 - value49 * 2, value823 * 0.48);
    const value52 = Math.max(value824 - value49 * 2, value824 * 0.42);
    addBoxMesh(rotation, value51, value48 * 0.16, value52, 0, value48 * 0.14, 0, color5, {
      radius: Math.min(value823, value824) * 0.16,
      roughness: 0.33
    });
    addBoxMesh(rotation, value823, value50, value49, 0, value50 * 0.5, -value824 * 0.5 + value49 * 0.5, color6, {
      radius: value49 * 0.5,
      roughness: 0.34
    });
    addBoxMesh(rotation, value823, value50, value49, 0, value50 * 0.5, value824 * 0.5 - value49 * 0.5, color6, {
    ...
    addBoxMesh(rotation, value49, value50, value52, -value823 * 0.5 + value49 * 0.5, value50 * 0.5, 0, color6, {
      radius: value49 * 0.5,
      roughness: 0.34
    });
    addBoxMesh(rotation, value49, value50, value52, value823 * 0.5 - value49 * 0.5, value50 * 0.5, 0, color6, {
    ...
    const value53 = value48 * 0.075;
    const value54 = value50 + value53 * 0.5;
    addBoxMesh(rotation, value823, value53, value49, 0, value54, -value824 * 0.5 + value49 * 0.5, color6, {
      radius: value49 * 0.45,
      roughness: 0.29
    });
    addBoxMesh(rotation, value823, value53, value49, 0, value54, value824 * 0.5 - value49 * 0.5, color6, {
    ...
    addBoxMesh(rotation, value49, value53, value52, -value823 * 0.5 + value49 * 0.5, value54, 0, color6, {
      radius: value49 * 0.45,
      roughness: 0.29
    });
    addBoxMesh(rotation, value49, value53, value52, value823 * 0.5 - value49 * 0.5, value54, 0, color6, {
    ...
    for (const value45 of [-value823 * 0.38, value823 * 0.38]) {
      for (const value28 of [-value824 * 0.3, value824 * 0.3]) {
        addSoftBoxMesh(rotation, 0.026, 0.026, value825 * 0.16, value45, value825 * 0.08, value28, glass.furnitureDark, {
          segments: 12,
          metalness: 0.42,
          roughness: 0.3
        });
    ...
    addSoftBoxMesh(rotation, 0.016, 0.016, value825 * 0.25, value823 * 0.34, value825 * 0.96, -value824 * 0.22, color7, {
      segments: 16,
      metalness: 0.72,
      roughness: 0.2
    });
    addSoftBoxMesh(rotation, 0.016, 0.016, value824 * 0.28, value823 * 0.34, value825 * 1.08, -value824 * 0.08, color7, {
      segments: 16,
      rotationX: Math.PI / 2,
      metalness: 0.72,
      roughness: 0.2
    ...
    const value46 = -value824 * 0.5 + 0.018;
    const value47 = {
      rounded: false,
      metalness: 0.64,
      roughness: 0.22
    };
    addBoxMesh(rotation, value823 * 0.68, value825 * 0.5, 0.035, 0, value825 * 0.52, value46, glass.furniture, {
      radius: 0.02,
      metalness: 0.18,
      roughness: 0.46
    });
    addSoftBoxMesh(rotation, value823 * 0.11, value823 * 0.11, 0.035, 0, value825 * 0.57, value46 - 0.012, glass.furnitureSoft, {
      segments: 24,
      rotationX: Math.PI / 2,
      metalness: 0.3,
      roughness: 0.34
    });
    addBoxMesh(rotation, 0.04, value825 * 0.2, value824 * 0.28, 0, value825 * 0.57, -value824 * 0.3, glass.furniture, {
      ...value47,
      metalness: 0.18,
      roughness: 0.46
    });
    const position = new THREE.Mesh(new THREE.CylinderGeometry(value823 * 0.3, value823 * 0.19, value825 * 0.38, 24, 1, true), new THREE.MeshStandardMaterial({
      color: color6,
      roughness: 0.3,
      metalness: 0.04,
      emissive: 16767386,
    ...
    position.position.set(0, value825 * 0.4, -value824 * 0.16);
    position.castShadow = true;
    position.receiveShadow = true;
    rotation.add(position);
    addSoftBoxMesh(rotation, value823 * 0.15, value823 * 0.15, 0.025, 0, value825 * 0.19, -value824 * 0.16, 16769707, {
      segments: 24,
      emissive: 16760156,
      emissiveIntensity: 0.38,
      roughness: 0.24
    ...
    const value40 = Math.min(Math.max(value823 * 0.018, 0.018), 0.035);
    const value41 = Math.max(value824, 0.045);
    const value42 = Math.max(value823 - value40 * 2.4, value40);
    const value43 = Math.max(value825 - value40 * 2.4, value40);
    const value44 = {
      rounded: false,
      metalness: 0.58,
      roughness: 0.24,
    ...
    addBoxMesh(rotation, value42, value43, Math.max(value824 * 0.24, 0.012), 0, value825 * 0.5, 0, glass.glass, {
      rounded: false,
      transparent: true,
      opacity: 0.24,
      depthWrite: false,
    ...
    addBoxMesh(rotation, value823, value40, value41, 0, value40 * 0.5, 0, glass.frame, value44);
    addBoxMesh(rotation, value823, value40, value41, 0, value825 - value40 * 0.5, 0, glass.frame, value44);
    addBoxMesh(rotation, value40, value825, value41, -value823 * 0.5 + value40 * 0.5, value825 * 0.5, 0, glass.frame, value44);
    addBoxMesh(rotation, value40, value825, value41, value823 * 0.5 - value40 * 0.5, value825 * 0.5, 0, glass.frame, value44);
    for (const value29 of [-value823 * 0.34, value823 * 0.34]) {
      addBoxMesh(rotation, value40 * 1.7, value40 * 2.2, value41 * 1.18, value29, value40 * 1.3, 0, color5, value44);
    }
  } else if (type20.type === "shower") {
    const value30 = color6;
    const value31 = -value824 * 0.42;
    const value32 = value825 * 0.13;
    const value33 = value825 * 0.9;
    ...
    addBoxMesh(rotation, 0.045, value34, 0.045, 0, (value32 + value33) * 0.5, value31, value30, value35);
    for (const value21 of [value32, value825 * 0.47, value825 * 0.78, value33]) {
      addBoxMesh(rotation, 0.085, 0.085, 0.065, 0, value21, value31, value30, value35);
    }
    for (const value22 of [value32, value33]) {
      addSoftBoxMesh(rotation, value823 * 0.055, value823 * 0.055, 0.04, 0, value22, -value824 * 0.47, value30, {
        segments: 28,
        rotationX: Math.PI / 2,
        metalness: 0.7,
        roughness: 0.2
    ...
    const value36 = value825 * 0.12;
    addBoxMesh(rotation, value823 * 0.36, 0.065, 0.065, 0, value36, value31 + value824 * 0.08, value30, value35);
    for (const value23 of [-value823 * 0.2, value823 * 0.2]) {
      addSoftBoxMesh(rotation, value823 * 0.055, value823 * 0.055, 0.045, value23, value36, -value824 * 0.45, value30, {
        segments: 28,
        rotationX: Math.PI / 2,
        metalness: 0.7,
        roughness: 0.2
      });
      addBoxMesh(rotation, 0.05, 0.05, value824 * 0.13, value23, value36, value31 + value824 * 0.01, value30, value35);
    }
    addBoxMesh(rotation, 0.045, value825 * 0.12, 0.045, 0, value36 - value825 * 0.045, value31 + value824 * 0.13, value30, value35);
    const y = new THREE.Vector3(0, value33, value31);
    const z = new THREE.Vector3(0, value825 * 0.76, value824 * 0.08);
    const value37 = z.y - y.y;
    const value38 = z.z - y.z;
    const value39 = Math.hypot(value37, value38);
    const rotation2 = addBoxMesh(rotation, 0.055, value39, 0.055, 0, (y.y + z.y) * 0.5, (y.z + z.z) * 0.5, value30, value35);
    rotation2.rotation.x = Math.atan2(value38, value37);
    addBoxMesh(rotation, 0.055, value825 * 0.13, 0.055, 0, value825 * 0.705, z.z, value30, value35);
    addBoxMesh(rotation, value823 * 0.34, 0.035, value824 * 0.3, 0, value825 * 0.64, z.z + value824 * 0.03, value30, {
      rounded: false,
      metalness: 0.64,
      roughness: 0.24
    });
  } else if (type20.type === "basin") {
    addBoxMesh(rotation, value823 * 0.92, value825 * 0.72, value824 * 0.9, 0, value825 * 0.36, 0, value826);
    addBoxMesh(rotation, value823, 0.065, value824, 0, value825 * 0.75, 0, color6, {
      roughness: 0.3
    });
    addSoftBoxMesh(rotation, value823 * 0.25, value823 * 0.21, 0.08, 0, value825 * 0.8, value824 * 0.02, color5, {
      roughness: 0.28
    });
    addSoftBoxMesh(rotation, 0.018, 0.018, value825 * 0.2, 0, value825 * 0.9, -value824 * 0.2, color7, {
      metalness: 0.78,
      roughness: 0.18
    });
    addSoftBoxMesh(rotation, 0.018, 0.018, value824 * 0.2, 0, value825 * 0.99, -value824 * 0.11, color7, {
      rotationX: Math.PI / 2,
      metalness: 0.78,
      roughness: 0.18
    });
    addBoxMesh(rotation, 0.012, value825 * 0.62, value824 * 0.02, 0, value825 * 0.34, value824 * 0.46, color7, {
      rounded: false
    });
    const value24 = value825 * 1.05;
    const value25 = value823 * 0.72;
    const value26 = 0.72;
    const value27 = -value824 * 0.46;
    addBoxMesh(rotation, value25, value26, 0.018, 0, value24 + value26 * 0.5, value27, glass.glass, {
      rounded: false,
      transparent: true,
      opacity: 0.46,
      depthWrite: false,
    ...
    addBoxMesh(rotation, value25 + 0.055, 0.035, 0.045, 0, value24, value27, color7, {
      metalness: 0.35
    });
    addBoxMesh(rotation, value25 + 0.055, 0.035, 0.045, 0, value24 + value26, value27, color7, {
      metalness: 0.35
    });
    addBoxMesh(rotation, 0.035, value26, 0.045, -value25 * 0.5, value24 + value26 * 0.5, value27, color7, {
      metalness: 0.35
    });
    addBoxMesh(rotation, 0.035, value26, 0.045, value25 * 0.5, value24 + value26 * 0.5, value27, color7, {
      metalness: 0.35
    ...
    if (!addRugMeshes(rotation, type20, value826, color5)) {
      const value9 = clamp(value825, 0.004, 0.018);
      addBoxMesh(rotation, value823, value9, value824, 0, value9 * 0.5, 0, value826, {
        radius: Math.min(value823, value824) * 0.018,
        roughness: 1,
        metalness: 0,
        castShadow: false,
        receiveShadow: true
      });
      const rotation2 = new THREE.Mesh(new THREE.PlaneGeometry(value823 * 0.88, value824 * 0.84), new THREE.MeshStandardMaterial({
        color: color5,
        roughness: 1,
        metalness: 0,
        polygonOffset: true,
      ...
      rotation2.rotation.x = -Math.PI / 2;
      rotation2.position.y = value9 + 0.001;
    ...
    addBoxMesh(rotation, value823, value825 * 0.76, value824, 0, value825 * 0.42, 0, value826);
    const value19 = value825 * 0.08;
    const value20 = value825 * 0.8;
    addBoxMesh(rotation, value823 * 0.98, value19, value824, 0, value20 + value19 * 0.5, 0, color5);
    addBoxMesh(rotation, 0.018, value825 * 0.58, value824 * 1.01, 0, value825 * 0.43, value824 * 0.01, color7, {
      rounded: false
    });
    addBoxMesh(rotation, value823 * 0.91, 0.015, value824 * 1.01, 0, value825 * 0.43, value824 * 0.01, color7, {
      rounded: false
    });
    for (const value10 of [-0.4, 0.4]) {
      addBoxMesh(rotation, 0.055, value825 * 0.2, 0.055, value823 * value10, value825 * 0.1, 0, color7, {
        metalness: 0.32
      });
    }
  } else if (type20.type === "floorlamp") {
    const value11 = -value823 * 0.34;
    const value12 = value823 * 0.31;
    const value13 = Math.min(value823 * 0.13, value824 * 0.34);
    const value14 = Math.min(value823 * 0.18, value824 * 0.46);
    const value15 = value825 * 0.115;
    const value16 = value825 * 0.76;
    const value17 = value16 + value15;
    addSoftBoxMesh(rotation, value13 * 0.82, value13, 0.045, value11, 0.0225, 0, color7, {
      segments: 32,
      metalness: 0.38,
      roughness: 0.3
    });
    const value18 = new THREE.CubicBezierCurve3(new THREE.Vector3(value11, 0.045, 0), new THREE.Vector3(value11, value825 * 0.72, 0), new THREE.Vector3(value823 * 0.02, value825 * 1.01, 0), new THREE.Vector3(value12, value17, 0));
    const castShadow2 = new THREE.Mesh(new THREE.TubeGeometry(value18, 48, Math.max(0.012, value823 * 0.012), 8, false), new THREE.MeshStandardMaterial({
      color: color7,
      roughness: 0.3,
      metalness: 0.48
    }));
    ...
    const scale = new THREE.Mesh(new THREE.SphereGeometry(value14, 32, 16, 0, Math.PI * 2, 0, Math.PI / 2), new THREE.MeshStandardMaterial({
      color: color5,
      roughness: 0.62,
      metalness: 0.04
    }));
    scale.scale.set(1, value15 / value14, 1);
    scale.position.set(value12, value16, 0);
    scale.castShadow = true;
    rotation.add(scale);
    addSoftBoxMesh(rotation, value14 * 0.94, value14 * 0.98, 0.025, value12, value16, 0, color7, {
      segments: 32,
      metalness: 0.12,
      roughness: 0.5
    });
    addSoftBoxMesh(rotation, Math.max(0.018, value823 * 0.014), Math.max(0.022, value823 * 0.018), 0.045, value12, value17 + 0.012, 0, color7, {
      segments: 20,
      metalness: 0.42,
      roughness: 0.28
    });
  } else if (type20.type === "plant") {
    addSoftBoxMesh(rotation, value823 * 0.25, value823 * 0.21, value825 * 0.22, 0, value825 * 0.11, 0, color5, {
      segments: 24,
      roughness: 0.82
    });
    addSoftBoxMesh(rotation, value823 * 0.22, value823 * 0.22, 0.035, 0, value825 * 0.22, 0, color7, {
      segments: 24,
      roughness: 0.96
    });
    const value7 = [[new THREE.Vector3(0, value825 * 0.21, 0), new THREE.Vector3(-value823 * 0.06, value825 * 0.58, 0), new THREE.Vector3(-value823 * 0.27, value825 * 0.78, 0)], [new THREE.Vector3(value823 * 0.03, value825 * 0.21, 0), new THREE.Vector3(value823 * 0.06, value825 * 0.64, 0), new THREE.Vector3(value823 * 0.12, value825 * 0.94, 0)], [new THREE.Vector3(0, value825 * 0.28, 0), new THREE.Vector3(value823 * 0.18, value825 * 0.62, 0), new THREE.Vector3(value823 * 0.31, value825 * 0.79, 0)]];
    for (const value3 of value7) {
      const castShadow = new THREE.Mesh(new THREE.TubeGeometry(new THREE.CatmullRomCurve3(value3), 20, 0.014, 7, false), new THREE.MeshStandardMaterial({
        color: color7,
        roughness: 0.86
      }));
    ...
    const value8 = [[-0.27, 0.78, 0], [-0.1, 0.61, 0.02], [0.12, 0.94, 0], [0.31, 0.79, 0], [0.18, 0.63, -0.02], [0.02, 0.46, 0.03]];
    for (const [value4, value5, value6] of value8) {
      for (let value2 = 0; value2 < 5; value2 += 1) {
        const value = value2 / 5 * Math.PI * 2;
        const rotation2 = new THREE.Mesh(new THREE.SphereGeometry(0.5, 10, 6), new THREE.MeshStandardMaterial({
          color: value2 % 2 ? 7835779 : 6257261,
          roughness: 0.9
        }));
        rotation2.scale.set(value823 * 0.045, value825 * 0.085, value824 * 0.025);
        rotation2.position.set(value823 * value4 + Math.cos(value) * value823 * 0.08, value825 * value5 + Math.sin(value) * value825 * 0.035, value824 * value6 + Math.sin(value) * value824 * 0.06);
  ...
  if ((floorScene.has(type20.type) || projectDoc.has(type20.type)) && type20.offlineModelExport !== true) {
    ...
    if (attachExternalItemModel(rotation, type20)) {
      forEach3.forEach(arg45 => {
        rotation.remove(arg45);
        disposeObject3dResources(arg45);
      });
    }
  }
  if (type20.type === "tv" && type20.offlineModelExport !== true) {
    addTvMountMeshes(rotation, type20, value823, value824, value825);
  }
...
function instanceMergeIdenticalItems(object3d, arg1) {
  object3d.rotation.y = -THREE.MathUtils.degToRad(finite(arg1.rotation, 0));
  if (stairItemTypes.has(arg1.type)) {
    object3d.scale.x = arg1.stairDirection === "left" ? -1 : 1;
  }
  if (arg1.type === "shoecabinet" && arg1.shoeCabinetMirrored === true) {
    object3d.scale.x = -1;
  }
  if (lightItemTypes2.has(arg1.type)) {
    if (arg1.type === "striplight") {
      object3d.rotation.order = "YXZ";
      object3d.rotation.x = 0;
      object3d.rotation.z = 0;
    } else {
      const x17 = THREE.MathUtils.degToRad(clamp(finite(arg1.verticalRotation, 0), -90, 90));
      object3d.rotation.order = "YXZ";
...
function collectShadowLights(updateMatrixWorld2) {
  ...
  updateMatrixWorld2.traverse(isMesh2 => {
    ...
    const value560 = collectChildMeshes(isMesh2);
    if (!value560) {
      value828 = false;
      return;
    }
    push18.push({
      mesh: isMesh2,
      relativeMatrix,
      signature: JSON.stringify([isMesh2.geometry.uuid, value560, relativeMatrix.elements.map(arg26 => Math.round(arg26 * 1000000) / 1000000), isMesh2.castShadow, isMesh2.receiveShadow, isMesh2.renderOrder])
    });
  });
  if (value828 && push18.length) {
    return push18;
  } else {
    return null;
  }
}
function cd(object3d, arg1) {
  const map = new Map();
  for (const {
    item,
    group: object3d2
    ...
    const value656 = JSON.stringify([item.type, map6.map(signature2 => signature2.signature)]);
    if (!map.has(value656)) {
      map.set(value656, []);
    }
    map.get(value656).push({
      item,
      group: object3d2,
      descriptors: map6
    });
  ...
  for (const list2 of map.values()) {
    ...
    for (let value561 = 0; value561 < item; value561 += 1) {
      const userData18 = list2[0].descriptors[value561].mesh;
      const value493 = userData18.userData.externalModelSharedMaterial ? userData18.material : userData18.material.clone();
      const userData19 = new THREE.InstancedMesh(userData18.geometry, value493, list2.length);
      userData19.name = "ha-bridge-instance-" + list2[0].item.type + "-" + (value561 + 1);
      ...
      list2.forEach(({
        ...
        const value401 = new THREE.Matrix4().multiplyMatrices(value, arg46[value561].mesh.matrixWorld);
        userData19.setMatrixAt(arg47, value401);
      });
    ...
    for (const {
      ...
      object3d.remove(value562);
      disposeObject3dResources(value562);
    }
    reduce.push({
  ...
  object3d.userData.instanceBatchStats = reduce;
  if (renderer?.domElement) {
    renderer.domElement.dataset.instanceBatchCount = String(reduce.length);
    renderer.domElement.dataset.instanceCount = String(reduce.reduce((arg0, before) => arg0 + before.instances, 0));
    renderer.domElement.dataset.instanceDrawCallsSaved = String(reduce.reduce((arg48, before) => arg48 + before.before - before.after, 0));
  }
  return reduce;
}
function buildCanvasPathFromPoints(updateMatrixWorld3, item) {
  const t2424 = new Map();
  const value829 = material5 => {
    const filter = Object.keys(material5.geometry.attributes).filter(arg31 => arg31 !== "color" || material5.material.vertexColors).sort();
    if (!isStageEmbed || Object.values(material5.material).some(isTexture => isTexture?.isTexture)) {
      return filter;
    } else {
      return filter.filter(arg38 => arg38 === "position" || arg38 === "normal" || arg38 === "color" && material5.material.vertexColors);
    }
  };
  ...
  for (const {
    ...
    if (parent4.parent === updateMatrixWorld3 && !skipInstanceMergeTypes.has(type15.type) && (!isStageEmbed || !["wallac", "floorac", "airoutlet", "curtain", "nas", "tv", "robotvacuum"].includes(type15.type)) && !isSelected("item", type15.id)) {
      parent4.traverse(geometry2 => {
        ...
        const value402 = isStageEmbed && !geometry2.material?.vertexColors;
        const value403 = createGlassMaterial(geometry2, isStageEmbed, value402);
        if (!value403 || geometry2.matrixWorld.determinant() < 0) {
          return;
        }
        const push = value829(geometry2).map(arg27 => [arg27, geometry2.geometry.attributes[arg27].itemSize]);
        if (value402) {
          push.push(["color", 3]);
        }
        const value404 = isStageEmbed ? JSON.stringify(push) : meshMaterialSignature(geometry2);
        const value405 = value403 + ":" + value404 + ":" + value402;
        if (!t2424.has(value405)) {
          t2424.set(value405, []);
        }
        t2424.get(value405).push(geometry2);
      });
    }
  }
  const value830 = updateMatrixWorld3.matrixWorld.clone().invert();
  const t24242 = [];
  const add3 = new Set();
  for (const length19 of t2424.values()) {
    if (length19.length < 2) {
      continue;
    }
    const forEach4 = length19.map(geometry4 => {
      const value429 = new THREE.Matrix4().multiplyMatrices(value830, geometry4.matrixWorld);
      const attributes = geometry4.geometry.clone();
      if (isStageEmbed) {
        const includes = value829(geometry4);
        for (const value352 of Object.keys(attributes.attributes)) {
          if (!includes.includes(value352)) {
            attributes.deleteAttribute(value352);
          }
        }
        for (const value353 of includes) {
          const itemSize = geometry4.geometry.attributes[value353];
          if (!itemSize.isInterleavedBufferAttribute && !itemSize.normalized && itemSize.array instanceof Float32Array) {
            continue;
          }
          const value340 = new Float32Array(itemSize.count * itemSize.itemSize);
          const value341 = ["getX", "getY", "getZ", "getW"];
          for (let value328 = 0; value328 < itemSize.count; value328++) {
            for (let value306 = 0; value306 < itemSize.itemSize; value306++) {
              value340[value328 * itemSize.itemSize + value306] = value306 < 4 ? itemSize[value341[value306]](value328) : itemSize.getComponent(value328, value306);
            }
          }
          attributes.setAttribute(value353, new THREE.BufferAttribute(value340, itemSize.itemSize));
        }
        if (!geometry4.material.vertexColors) {
          const value342 = attributes.attributes.position.count;
          const value343 = new Float32Array(value342 * 3);
          const r = geometry4.material.color;
          for (let value329 = 0; value329 < value342; value329++) {
            value343[value329 * 3] = r.r;
            value343[value329 * 3 + 1] = r.g;
            value343[value329 * 3 + 2] = r.b;
          }
          attributes.setAttribute("color", new THREE.BufferAttribute(value343, 3));
        }
        if (!attributes.index) {
          const value344 = attributes.attributes.position.count;
          const value345 = value344 <= 65536 ? new Uint16Array(value344) : new Uint32Array(value344);
          for (let value330 = 0; value330 < value344; value330++) {
            value345[value330] = value330;
          }
          attributes.setIndex(new THREE.BufferAttribute(value345, 1));
        }
      }
      return attributes.applyMatrix4(value429);
    });
    const value657 = mergeGeometries(forEach4);
    forEach4.forEach(dispose2 => dispose2.dispose());
    if (!value657) {
      continue;
    }
    const material9 = length19[0];
    const value658 = isStageEmbed && !material9.material.vertexColors;
    const color3 = value658 ? material9.material.clone() : material9.material;
    if (value658) {
      color3.color.setRGB(1, 1, 1);
      color3.vertexColors = true;
    }
    const userData30 = new THREE.Mesh(value657, color3);
    ...
    userData30.userData.externalModelSharedMaterial = !value658 && material9.userData.externalModelSharedMaterial === true;
  ...
  for (const {
    ...
    if (parent5.parent === updateMatrixWorld3 && Ys(parent5).length === 0) {
      updateMatrixWorld3.remove(parent5);
    }
  }
  const add4 = new Set();
  updateMatrixWorld3.traverse(material6 => {
    for (const value454 of Array.isArray(material6.material) ? material6.material : material6.material ? [material6.material] : []) {
      add4.add(value454);
    }
  });
  ...
  updateMatrixWorld3.userData.staticItemBatchStats = t24242;
  if (renderer?.domElement) {
    renderer.domElement.dataset.staticItemBatchCount = String(t24242.length);
    renderer.domElement.dataset.staticItemDrawCallsSaved = String(t24242.reduce((arg49, before2) => arg49 + before2.before - before2.after, 0));
  }
  return t24242;
}
...
function meshInstanceDescriptors(object3d) {
  ...
  return value + ":" + list + ":" + flag + ":" + value831;
}
function collectWorldExternalMeshes() {
  ...
  const idSet = [...map.values()].flat();
  const list2 = new Set();
  const list22 = [];
  const value832 = meshInstanceDescriptors(list);
  const value833 = worldGroup.uuid + ":" + cs;
  if (materialTestTypeQuery !== value833) {
    materialTestTypeQuery = value833;
    instanceTestTypeQuery.clear();
  }
  const value834 = (arg83, filter2, arg84 = false) => {
    const signature3 = meshInstanceDescriptors(arg83);
    if (!list2.has(signature3)) {
      list2.add(signature3);
      if (!instanceTestTypeQuery.has(signature3)) {
        if (!!arg84 || signature3 === value832 || !(instanceTestTypeQuery.size + list22.length >= qg)) {
          list22.push({
  ...
  value834(list, [], true);
  value834([], idSet.map(light8 => ({
    light: light8,
    visible: false
  })), true);
  for (const list3 of map.values()) {
    const lights = list3.filter(object3d => object3d.visible === false);
    if (lights.length) {
      value834([...list, ...lights], lights.map(light5 => ({
        light: light5,
        visible: true
      })));
    }
    ...
    if (signature.length) {
      value834(list.filter(arg40 => !signature.includes(arg40)), signature.map(light6 => ({
        light: light6,
        visible: false
      })));
    }
  ...
  if (isStageEmbed) {
    value834(idSet, idSet.map(light7 => ({
      light: light7,
      visible: true
    })), true);
  }
...
const Xm = 4500;
function countLightPrecompileWork(arg0, traverse4, arg195, arg196) {
  const value = arg0.getContext();
  if (!arg196() || value.isContextLost() || arg0.extensions?.has("KHR_parallel_shader_compile") === false) {
    return Promise.resolve(false);
  }
  arg0.compile(traverse4, arg195);
  let count = false;
  ...
  if (count) {
    const value659 = arg0.getRenderTarget();
    const value660 = arg0.getActiveCubeFace();
    const value661 = arg0.getActiveMipmapLevel();
    const dispose5 = new THREE.WebGLRenderTarget(1, 1);
    try {
      arg0.setRenderTarget(dispose5);
      arg0.compile(traverse4, arg195);
    } finally {
      arg0.setRenderTarget(value659, value660, value661);
      dispose5.dispose();
    }
  }
  const count2 = [...arg0.info.programs];
  const count3 = performance.now() + Xm;
  return new Promise((arg85, arg86) => {
    const value563 = () => {
      try {
        if (!arg196() || arg0.getContext() !== value || value.isContextLost()) {
          arg85(false);
          return;
        }
        const has2 = new Set(arg0.info.programs);
        const value354 = program => has2.has(program) && program.program && value.isProgram(program.program);
        if (count2.some(arg24 => !value354(arg24))) {
          arg85(false);
          return;
        }
        if (count2.every(isReady => isReady.isReady())) {
          for (const getUniforms of count2) {
            if (!value354(getUniforms)) {
              arg85(false);
              return;
            }
          ...
          window.setTimeout(value563, 32);
        }
      } catch (value355) {
        arg86(value355);
      }
    };
    window.setTimeout(value563, 0);
  });
}
  ...
  if (!yt && !isAutoDiagramEmbed2 && !endBaseLightPanelDrag) {
    ...
    if (!ORBIT_DOLLY_SPEED_MAX && !activeFloorContentBounds()) {
      endDetailsPanelResize = window.setTimeout(async () => {
        ...
        const value373 = previewScene2;
        const domElement = renderer;
        const value374 = [uuid.uuid, getPreviewFloorMode2(), activeFloorId, cs, ...length5.map(signature => signature.signature).sort()].join("|");
        if (!length5.length) {
        ...
        const value375 = () => worldGroup === uuid && previewScene2 === value373 && renderer === domElement && !ORBIT_DOLLY_SPEED_SCALE && !activeFloorContentBounds() && !createInvisibleMaterial();
        let value376 = true;
        try {
          for (const changes of length5) {
            ...
            const value313 = changes.changes.map(({
              light,
              visible: nextVisible
            }) => ({
              light,
            ...
            try {
              ...
              value314 = countLightPrecompileWork(domElement, value373, camera2, value375);
            } finally {
              for (const light4 of value313) {
            ...
            if (!(await value314) || !value375()) {
              value376 = false;
              domElement.domElement.dataset.lightPrecompileDeferred = "true";
              ORBIT_DOLLY_SPEED_SCALE ||= !value375();
              break;
            }
            G0 += 1;
          ...
          if (value376) {
            PRECOMPILE_TIMEOUT_MS = value374;
          ...
          console.debug("3D first-light precompile skipped", value346);
        } finally {
...
function scheduleLightPrecompile(arg0 = 0) {
  if (!yt) {
    ...
    if (!isLightPrecompiling) {
      lightPrecompileTimer = window.setTimeout(async () => {
        ...
        try {
          ...
          active.forEach(arg21 => O0.add(arg21));
          cs += 1;
...
function rebuildPreviewMeshes(arg0 = {}) {
  const value835 = arg0.force === true;
  const value836 = ["items", "lights"].includes(arg0.scope) ? arg0.scope : "all";
  if (!isLivePreviewEnabled() && !value835) {
    if (arg0.transient !== true) {
  ...
  if (value835) {
    ...
    pendingRebuildReasons.add(value836);
  }
  if (arg0.precompile === true) {
    orbitAnimSettleTimer = true;
  }
  if (arg0.preserveLightCache !== true) {
    Ka = true;
  }
  if (!isSceneRebuildQueued) {
    ...
    requestAnimationFrame(() => {
      ...
      const value455 = isLivePreviewEnabled() || planZoomAnchor;
      ...
      const value456 = orbitAnimSettleTimer;
      orbitAnimSettleTimer = false;
      syncExternalModelDomStats();
      if (value456 || collectExternalSharedMeshes().length) {
...
function extrudeWallSegmentShape(wall, wall2, arg2, arg3, wall3 = {}) {
...
function splitFloorPolygonsByHoles(arg198) {
  const flag = [];
  const value = [];
  for (const loop4 of arg198) {
  ...
  if (!flag.length) {
    for (const value564 of value.splice(0)) {
      const loop2 = [...value564].reverse();
      flag.push({
        loop: loop2,
  ...
  for (const value706 of value) {
    const holes2 = flag.filter(loop => pointInPolygon(value706[0], loop.loop, 0.000001)).sort((area, area2) => area.area - area2.area)[0];
    if (holes2) {
      holes2.holes.push(value706);
    }
  }
  return flag.map(loop3 => {
    const holes = polygonCentroid(THREE.Shape, loop3.loop);
    for (const value457 of loop3.holes) {
      holes.holes.push(polygonCentroid(THREE.Path, value457));
    }
    return holes;
  });
}
function Ku(list, arg1, depthWrite = {}) {
  const list2 = arg1 >= 0.999;
...
function Qu(arg0, grid, topColor = {}) {
  const value = grid >= 0.999;
  return new THREE.MeshStandardMaterial({
    color: topColor.topColor ?? arg0,
    roughness: 0.76,
    metalness: 0,
    transparent: !value,
    ...
    side: THREE.DoubleSide,
    emissive: topColor.emissive ?? topColor.topColor ?? arg0,
    emissiveIntensity: topColor.emissiveIntensity ? topColor.emissiveIntensity * 0.3 : 0.08
  });
}
function addWallMeshBatch(list, arg1, arg2, color, opacity, light = {}) {
  ...
  for (const entry of value) {
    ...
    const invisibleMaterial = Ym();
    const glassMaterial = Ku(color, opacity, depthWrite);
...
function addFloorPolygonMeshes(list, arg1, flag2, value, depthWrite = {}) {
  if (!list.length) {
    return;
  }
  const validatedUnionPolygonLoops2 = validatedUnionPolygonLoops(list, 0.000001);
  ...
  for (const entry of splitFloorPolygonsByHolesResult) {
    ...
    const $0_2480 = Qu(flag2, value, topColor);
...
function resolvePlanSnap(arg0, anchor, arg199) {
  const value = [];
  const settings = [];
  for (let value707 = 0; value707 < arg0.length; value707 += 1) {
    const x36 = arg0[value707];
    const x37 = arg0[(value707 + 1) % arg0.length];
    const value662 = x37.x - x36.x;
    const value663 = x37.z - x36.z;
    const value664 = Math.hypot(value662, value663);
    if (value664 <= 0.001) {
      continue;
    }
    const value665 = (x36.x + x37.x) / 2;
    const value666 = (x36.z + x37.z) / 2;
    const value667 = -Math.atan2(value663, value662);
    const value668 = new THREE.Matrix4().compose(new THREE.Vector3(value665, arg199 + 0.021, value666), new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0, 1, 0), value667), new THREE.Vector3(1, 1, 1));
    const value669 = new THREE.Matrix4().compose(new THREE.Vector3(value665, arg199 + 0.026, value666), new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0, 1, 0), value667), new THREE.Vector3(1, 1, 1));
    value.push(new THREE.BoxGeometry(value664, 0.042, 0.038).applyMatrix4(value668));
    settings.push(new THREE.BoxGeometry(value664 + 0.025, 0.066, 0.078).applyMatrix4(value669));
  }
  const value837 = value.length ? mergeGeometries(value) : null;
  const value838 = settings.length ? mergeGeometries(settings) : null;
  value.forEach(dispose3 => dispose3.dispose());
  settings.forEach(dispose4 => dispose4.dispose());
  if (value837) {
    const userData31 = new THREE.Mesh(value837, new THREE.MeshBasicMaterial({
      color: anchor,
      transparent: true,
      opacity: 0.82,
      toneMapped: false
    ...
    userData31.userData.batchedFloorEdgeCount = arg0.length;
    worldGroup.add(userData31);
  }
  if (value838) {
    const userData32 = new THREE.Mesh(value838, new THREE.MeshBasicMaterial({
      color: anchor,
      transparent: true,
      opacity: 0.09,
      depthWrite: false,
    ...
    userData32.userData.batchedFloorEdgeCount = arg0.length;
    worldGroup.add(userData32);
  }
}
function splitFloorPolygonsByHoles2(arg0, arg200, arg201) {
  if (!Array.isArray(arg0) || arg0.length < 3) {
    return;
  }
  const list = arg0.map(planPoint => ({
    x: planPoint.x,
    y: planPoint.z
  }));
  const value839 = list.map((arg70, arg71) => distance(arg70, list[(arg71 + 1) % list.length]));
  const push19 = [0];
  for (const loop of value839) {
  ...
  const value840 = ({
    ...
    innerAlpha: arg89,
    outerAlpha: arg90,
    columnStrength: value565,
    y: arg91,
    renderOrder
  }) => {
    const value566 = addCeilingMeshes(list, arg88);
    const push11 = [];
    const push12 = [];
    const push13 = [];
    const push14 = [];
    for (let value458 = 0; value458 < list.length; value458 += 1) {
      const value430 = (value458 + 1) % list.length;
      const x10 = list[value458];
      const x11 = list[value430];
      const x12 = value566[value458];
      const x13 = value566[value430];
      push11.push(x10.x, arg91, x10.y, x12.x, arg91, x12.y, x13.x, arg91, x13.y, x10.x, arg91, x10.y, x13.x, arg91, x13.y, x11.x, arg91, x11.y);
      push12.push(arg89, arg90, arg90, arg89, arg90, arg89);
      push13.push(0, 1, 1, 0, 1, 0);
      const value431 = push19[value458];
      const value432 = push19[value458 + 1];
      push14.push(value431, value431, value432, value431, value432, value432);
    }
    const setAttribute = new THREE.BufferGeometry();
    setAttribute.setAttribute("position", new THREE.Float32BufferAttribute(push11, 3));
    setAttribute.setAttribute("glowAlpha", new THREE.Float32BufferAttribute(push12, 1));
    setAttribute.setAttribute("glowAcross", new THREE.Float32BufferAttribute(push13, 1));
    setAttribute.setAttribute("glowAlong", new THREE.Float32BufferAttribute(push14, 1));
    setAttribute.computeVertexNormals();
    const renderOrder2 = new THREE.Mesh(setAttribute, new THREE.ShaderMaterial({
      uniforms: {
        glowColor: {
          value: new THREE.Color(arg200)
        },
        glowColumnStrength: {
          value: value565
        }
...
function addCeilingMeshes(list, arg1) {
  const list2 = list.reduce((list3, arg12) => ({
    x: list3.x + arg12.x / list.length,
    y: list3.y + arg12.y / list.length
  }), {
    x: 0,
    y: 0
  });
  return list.map(item => {
    const value567 = item.x - list2.x;
    const value568 = item.y - list2.y;
    const value569 = Math.max(Math.hypot(value567, value568), 0.000001);
    return {
      x: item.x + value567 / value569 * arg1,
      y: item.y + value568 / value569 * arg1
    };
  });
}
function Um(list, arg1) {
  if (!list.length) {
    return;
  }
  const list2 = list.map(worldPoint => {
    const value494 = addCeilingMeshes(worldPoint, 0.028);
    const value495 = polygonArea(value494) >= 0 ? value494 : [...value494].reverse();
    return polygonCentroid(THREE.Shape, value495);
  });
  const forEach5 = list2.map(arg72 => {
    const rotateX = new THREE.ShapeGeometry(arg72, 1);
    rotateX.rotateX(Math.PI / 2);
    rotateX.translate(0, arg1, 0);
    return rotateX;
  });
  const value841 = mergeGeometries(forEach5);
  forEach5.forEach(spread => spread.dispose());
  if (!value841) {
    return;
  }
  const renderOrder3 = new THREE.Mesh(value841, new THREE.MeshBasicMaterial({
    color: 527122,
    transparent: true,
    opacity: 0.052,
    depthWrite: false,
...
function Zm(arg202, grid, arg203) {
  const ppm = Math.max(Math.round(arg202 / 1.25), 12);
  const material10 = new THREE.GridHelper(arg202, ppm, grid.grid, grid.grid);
  ...
  material10.onBeforeRender = (arg92, arg93, position6) => {
    const uniforms2 = material10.material.userData.depthFadeShader;
    if (!uniforms2) {
      return;
    }
    material10.getWorldPosition(bounds);
    const value570 = Math.max(position6.position.distanceTo(orbitControls?.target || bounds), 1);
    const value571 = position6.isOrthographicCamera ? Math.abs(position6.top - position6.bottom) / Math.max(position6.zoom, 0.001) : value570 * 2 * Math.tan(THREE.MathUtils.degToRad(position6.fov * 0.5));
    const value572 = Math.max(Math.hypot(value571 * Math.max(position6.aspect, 0.1), value571), 2);
    const element = value570 + value572 * 0.2;
    uniforms2.uniforms.gridDepthFadeNear.value = element;
    uniforms2.uniforms.gridDepthFadeFar.value = Math.max(element + 1, value570 + value572 * 0.85);
  };
  material10.position.y = arg203 + 0.012;
  material10.renderOrder = 2;
  material10.userData.exportRole = "grid";
  worldGroup.add(material10);
}
function setPreviewFloorMode(arg0, arg204, length28 = []) {
  if (!Array.isArray(arg0) || arg0.length < 3) {
    return;
  }
  const value842 = arg0.map(polygon => ({
    x: polygon.x,
    y: polygon.z
  }));
  [{
    ...
    offsetY: -0.19,
    opacity: 0.018
  }].forEach((spread, arg94) => {
    const value573 = addCeilingMeshes(value842, spread.spread).map(door => ({
      x: door.x + spread.offsetX,
      y: door.y + spread.offsetY
    }));
    const value574 = length28.length ? splitFloorPolygonsByHoles(subtractPolygonLoops([value573], length28)) : polygonCentroid(THREE.Shape, value573);
    const rotation3 = new THREE.Mesh(new THREE.ShapeGeometry(value574, 1), new THREE.MeshBasicMaterial({
      color: 329482,
      transparent: true,
    ...
    rotation3.position.y = arg204 + 0.001 + arg94 * 0.00015;
    rotation3.userData.floorPlanGroundShadow = true;
    rotation3.renderOrder = 1 + arg94;
...
function rebuildWorldPreview({
  ...
  const value = resolvedThemeColors();
  const rwpV2 = getPreviewFloorMode();
  const rwpV3 = Lo?.x ?? (rwpV2.minX + rwpV2.maxX) / 2;
  const item = Lo?.y ?? (rwpV2.minY + rwpV2.maxY) / 2;
  const rwpV4 = wall => ({
    x: (wall.x - rwpV3) / group,
    z: (wall.y - item) / group
  });
  const value843 = Math.max(rwpV2.width / group + 1, 3);
  const value844 = Math.max(rwpV2.height / group + 1, 3);
  const value845 = -0.008;
  const depth = 0.16;
  const value846 = value845 - depth;
  const value2 = value846 - 0.035;
  const value847 = Math.max(Math.max(value843, value844) * 16, 260);
  const backgroundPlane = new THREE.Mesh(new THREE.PlaneGeometry(value847, value847), new THREE.MeshBasicMaterial({
    color: value.ground,
    toneMapped: false
  }));
  backgroundPlane.rotation.x = -Math.PI / 2;
  backgroundPlane.position.y = value2;
  backgroundPlane.receiveShadow = false;
  backgroundPlane.userData.exportRole = "background";
  worldGroup.add(backgroundPlane);
  Zm(value847, value, value2);
  ...
  const length30 = floorScene2.items.filter(type2 => type2.type === "flooropening").map(arg73 => floorOpeningPolygon(arg73, group).map(arg54 => {
    const x14 = rwpV4(arg54);
    return {
      x: x14.x,
      y: x14.z
    };
  }));
  const value848 = arg95 => {
    setPreviewFloorMode(arg95, value2);
    if (floorScene2.settings.floorEdgeVisible !== false) {
      resolvePlanSnap(arg95, value.floorEdge, value846);
    }
  };
  const value849 = (arg96, arg97, arg98) => {
    const userData24 = new THREE.Mesh(arg97, dispose7);
    userData24.rotation.x = arg98 ? Math.PI / 2 : 0;
    userData24.position.y = arg98 ? value845 : value845 - depth / 2;
  ...
  if (length30.length) {
    const value670 = length29.length ? length29.map(footprint => footprint.map(arg39 => {
      const x2 = rwpV4(arg39);
      return {
        x: x2.x,
        y: x2.z
      };
    ...
    const filter3 = subtractPolygonLoops(value670, length30);
    const length12 = splitFloorPolygonsByHoles(filter3);
    if (length12.length) {
      value849([], new THREE.ExtrudeGeometry(length12, {
        depth,
        bevelEnabled: false,
        steps: 1
      }), true);
    ...
    for (const map3 of filter3.filter(arg64 => polygonArea(arg64) > 0)) {
      const value496 = map3.map(footprint => ({
        x: footprint.x,
        z: footprint.y
      }));
      setPreviewFloorMode(value496, value2, length30);
      if (floorScene2.settings.floorEdgeVisible !== false) {
        resolvePlanSnap(value496, value.floorEdge, value846);
      }
    }
  } else if (length29.length) {
    const map4 = length29.map(map2 => map2.map(rwpV4));
    const value575 = map4.map(forEach => {
      const moveTo = new THREE.Shape();
      forEach.forEach((floor, arg30) => {
        if (arg30 === 0) {
    ...
    value849(map4[0], new THREE.ExtrudeGeometry(value575, {
      depth,
      bevelEnabled: false,
      steps: 1
    }), true);
    for (const value497 of map4.slice(1)) {
      value848(value497);
    }
  } else {
    const value576 = [{
    ...
    value849(value576, new THREE.BoxGeometry(value843, depth, value844), false);
  }
  const value850 = [...floorScene2.windows, ...floorScene2.doors.map(arg55 => ({
    ...arg55,
    sill: 0
  ...
  const filter6 = [];
  const has16 = new Set();
  const value851 = getWallJoinExtensions(group);
  for (const opacity2 of floorScene2.walls) {
    ...
    for (const bottom4 of wallSolidPieces(opacity2, value850, group, opacity2.height)) {
      const footprint5 = extrudeWallSegmentShape(opacity2, bottom4, group, rwpV4, value851[opacity2.id]);
      if (!footprint5) {
        continue;
      }
      const value498 = [canonicalPolygonKey(footprint5, 4), bottom4.bottom.toFixed(5), bottom4.top.toFixed(5), toFixed2.toFixed(4)].join("|");
      if (!has16.has(value498)) {
        has16.add(value498);
        filter6.push({
          wallId: opacity2.id,
  ...
  const length31 = [...new Set(filter6.flatMap(bottom => [bottom.bottom, bottom.top]).map(toFixed => toFixed.toFixed(6)))].map(Number).sort((arg74, arg75) => arg74 - arg75);
  Um(filter6.filter(bottom2 => bottom2.bottom <= 0.000001).map(footprint6 => footprint6.footprint), value845 + 0.0025);
  for (let value708 = 0; value708 < length31.length - 1; value708 += 1) {
    const value671 = length31[value708];
    const value672 = length31[value708 + 1];
    if (value672 - value671 <= 0.000001) {
      continue;
    }
    const value673 = (value671 + value672) / 2;
    const filter4 = filter6.filter(bottom3 => value673 > bottom3.bottom - 0.000001 && value673 < bottom3.top + 0.000001);
    const has9 = new Map();
    for (const opacity of filter4) {
      const value499 = opacity.opacity.toFixed(4);
      if (!has9.has(value499)) {
        has9.set(value499, {
          opacity: opacity.opacity,
          volumes: []
        });
      }
      has9.get(value499).volumes.push(opacity);
    }
    for (const volumes of has9.values()) {
      addWallMeshBatch(volumes.volumes.map(footprint => footprint.footprint), value671, value672, value.wall, volumes.opacity, {
        castShadow: true,
        lightOccluder: true
      });
      const value500 = volumes.volumes.filter(item => Math.abs(item.top - value672) <= 0.000001).map(footprint2 => footprint2.footprint);
      addFloorPolygonMeshes(value500, value672, value.wall, volumes.opacity, {
        topColor: value.wall
      });
    }
    const length13 = filter4.filter(wallId2 => isSelected("wall", wallId2.wallId)).map(footprint4 => footprint4.footprint);
    if (length13.length) {
      addWallMeshBatch(length13, value671, value672, value.accent, 0.28, {
        depthWrite: false,
        depthFunc: THREE.LessEqualDepth,
        polygonOffset: true,
      ...
      const value501 = filter4.filter(wallId => isSelected("wall", wallId.wallId) && Math.abs(wallId.top - value672) <= 0.000001).map(footprint3 => footprint3.footprint);
      addFloorPolygonMeshes(value501, value672, value.accent, 0.28, {
        depthWrite: false,
        depthFunc: THREE.LessEqualDepth,
        polygonOffset: true,
        emissive: value.accent,
  ...
  for (const start6 of floorScene2.walls) {
    const value674 = start6.end.x - start6.start.x;
    const value675 = start6.end.y - start6.start.y;
    const value676 = Math.hypot(value674, value675);
    if (!value676) {
      continue;
    }
    const value677 = {
    ...
    const y3 = -Math.atan2(value675, value674);
    for (const sill of floorScene2.windows.filter(wallId4 => wallId4.wallId === start6.id)) {
      const value502 = clampWindowT(start6, sill, group);
      const value503 = {
        x: start6.start.x + value674 * value502,
        y: start6.start.y + value675 * value502
      };
      const x21 = rwpV4(value503);
      const position3 = new THREE.Group();
      position3.position.set(x21.x, 0, x21.z);
      position3.rotation.y = y3;
      const value504 = Math.min(sill.width, wallLengthMeters(start6, group));
      const value505 = Math.min(sill.height, Math.max(start6.height - sill.sill, 0.2));
      addSharedArchMesh(position3, [[value504 * 0.94, value505 * 0.92, 0.025, 0, sill.sill + value505 / 2, 0]], value.glass, {
        rounded: false,
        transparent: true,
        opacity: 0.2,
        depthWrite: false,
      ...
      const value506 = isSelected("window", sill.id) ? value.accent : value.frame;
      const value507 = 0.045;
      const value508 = {
        rounded: false,
        metalness: 0.15,
        castShadow: false,
      ...
      const push6 = [[value504, value507, 0.06, 0, sill.sill, 0], [value504, value507, 0.06, 0, sill.sill + value505, 0], [value507, value505, 0.06, -value504 / 2, sill.sill + value505 / 2, 0], [value507, value505, 0.06, value504 / 2, sill.sill + value505 / 2, 0]];
      const value509 = sill.hasDivider !== false && value504 > 1.2;
      if (value509) {
        push6.push([value507 * 0.7, value505, 0.055, 0, sill.sill + value505 / 2, 0]);
      }
      addSharedArchMesh(position3, push6, value506, value508);
      position3.userData.optimizationStats = {
        type: value509 ? "window-divided" : "window-plain",
        before: value509 ? 6 : 5,
        after: 2
      };
    ...
    for (const width5 of floorScene2.railings.filter(wallId5 => wallId5.wallId === start6.id)) {
      const value510 = clampWindowT(start6, width5, group);
      const value511 = {
        x: start6.start.x + value674 * value510,
        y: start6.start.y + value675 * value510
      };
      const x22 = rwpV4(value511);
      const position4 = new THREE.Group();
      position4.position.set(x22.x, 0, x22.z);
      position4.rotation.y = y3;
      const value512 = Math.min(width5.width, wallLengthMeters(start6, group));
      const value513 = Math.min(width5.height, start6.height);
      const value514 = isSelected("railing", width5.id) ? value.accent : value.frame;
      const value515 = Math.min(Math.max(value512 * 0.012, 0.028), 0.05);
      const value516 = 0.08;
      const value517 = Math.max(value513 - value516 - value515 * 1.4, 0.2);
      addSharedArchMesh(position4, [[Math.max(value512 - value515 * 2.4, 0.2), value517, 0.018, 0, value516 + value517 * 0.5, 0]], value.glass, {
        rounded: false,
        transparent: true,
        opacity: 0.24,
        depthWrite: false,
      ...
      const value519 = Math.max(2, Math.min(16, Math.ceil(value512 / 1.5) + 1));
      const push7 = [[value512, value515, 0.055, 0, value513, 0]];
      const push8 = [];
      for (let value459 = 0; value459 < value519; value459 += 1) {
        const value433 = -value512 / 2 + value512 * value459 / (value519 - 1);
        push7.push([value515, value513, 0.055, value433, value513 * 0.5, 0]);
        push8.push([value515 * 2, value515 * 0.8, 0.08, value433, value515 * 0.4, 0]);
      }
      addSharedArchMesh(position4, push7, value514, value518);
      addSharedArchMesh(position4, push8, value.furnitureSoft, value518);
    ...
    for (const swing of floorScene2.doors.filter(wallId6 => wallId6.wallId === start6.id)) {
      const value520 = clampWindowT(start6, swing, group);
      const value521 = {
        x: start6.start.x + value674 * value520,
        y: start6.start.y + value675 * value520
      };
      const x23 = rwpV4(value521);
      const userData20 = new THREE.Group();
      userData20.position.set(x23.x, 0, x23.z);
      userData20.rotation.y = y3;
      const value522 = Math.min(swing.width, wallLengthMeters(start6, group));
      const value523 = Math.min(swing.height, start6.height);
      const value524 = isSelected("door", swing.id);
      const value525 = swing.doorType || "solid";
      const value526 = value524 ? value.accent : value.frame;
      const value527 = 0.065;
      const value528 = {
        rounded: false,
        metalness: 0.08,
        castShadow: false,
      ...
      const push9 = [[value527, value523, 0.09, -value522 / 2, value523 / 2, 0], [value527, value523, 0.09, value522 / 2, value523 / 2, 0], [value522 + value527, value527, 0.09, 0, value523, 0]];
      if (value525 === "roller-shutter") {
        const value434 = swing.swing === -1 ? -1 : 1;
        push9.push([value522 + value527 * 0.6, value527 * 1.8, 0.13, 0, value523 - value527 * 0.35, value434 * 0.04]);
      }
      addSharedArchMesh(userData20, push9, value526, value528);
      if (value525 === "frame-only") {
      ...
      if (value525 === "sliding-glass") {
        const height3 = Math.max(value523 - value527 * 0.85, 0.4);
        const width = Math.max(value522 * 0.54, 0.28);
        const value435 = swing.hinge === "right" ? 1 : -1;
        const moving = slidingDoorPanelCenters(value522, value435);
        stairRiserMaterialOptions(userData20, [{
          ...
          centerZ: 0.024
        }], value526, value.glass);
        const value436 = moving.moving - value435 * width * 0.36;
        addSharedArchMesh(userData20, [[0.026, 0.15, 0.055, value436, value523 * 0.52, -0.052], [0.026, 0.15, 0.055, value436, value523 * 0.52, 0.052]], value.furnitureDark, {
          rounded: false,
          metalness: 0.5,
          castShadow: false,
          receiveShadow: false
      ...
      if (value525 === "roller-shutter") {
        const value437 = Math.max(value522 - value527 * 1.3, 0.4);
        const value438 = Math.max(value523 - value527 * 0.85, 0.8);
        const value439 = swing.swing === -1 ? -1 : 1;
        addSharedArchMesh(userData20, [[value437, value438, 0.045, 0, value438 * 0.5, value439 * 0.04]], value524 ? value.accent : value.furnitureSoft, {
          rounded: false,
          metalness: 0.36,
          roughness: 0.42,
          castShadow: false,
        ...
        const value440 = Math.max(5, Math.min(36, Math.round(value438 / 0.12)));
        const push2 = [];
        for (let value406 = 1; value406 < value440; value406 += 1) {
          const value377 = value438 * value406 / value440;
          push2.push([value437 * 0.98, 0.012, 0.052, 0, value377, value439 * 0.052]);
        }
      ...
      if (value525 === "entry") {
        const value441 = Math.max(value522 - value527 * 1.5, 0.4);
        const value442 = Math.max(value523 - value527 * 0.85, 0.8);
        const value443 = value524 ? value.accent : value.furnitureDark;
        addSharedArchMesh(userData20, [[value441, value442, 0.065, 0, value442 * 0.5, 0]], value443, {
          rounded: false,
          roughness: 0.58,
          metalness: 0.1,
          castShadow: false,
        ...
        addSharedArchMesh(userData20, [[value441 * 0.76, 0.022, 0.078, 0, value523 * 0.68, 0.012], [value441 * 0.76, 0.022, 0.078, 0, value523 * 0.34, 0.012]], value.furnitureSoft, {
          rounded: false,
          roughness: 0.5,
          castShadow: false,
          receiveShadow: false
        });
        const value444 = swing.hinge === "right" ? -value441 * 0.34 : value441 * 0.34;
        addSharedArchMesh(userData20, [[0.035, 0.18, 0.085, value444, value523 * 0.5, 0.055]], value.furnitureLight, {
          rounded: false,
          metalness: 0.58,
          roughness: 0.24,
          castShadow: false,
      ...
      if (value525 === "double") {
        const width2 = Math.max((value522 - value527 * 1.8) / 2, 0.25);
        const height4 = Math.max(value523 - value527 * 0.8, 0.4);
        const value445 = (swing.swing === -1 ? 1 : -1) * Math.PI * 0.42;
        const push3 = [];
        const push4 = [];
        for (const value407 of [-1, 1]) {
          const value378 = value407 * (value522 / 2 - value527 * 0.5);
          const rotationY = value407 < 0 ? value445 : -value445;
          const value379 = value407 < 0 ? width2 / 2 : -width2 / 2;
          const x5 = {
            x: value378 + value379 * Math.cos(rotationY),
            z: -value379 * Math.sin(rotationY)
          };
          push3.push({
            width: width2,
            height: height4,
          ...
          const value380 = value407 < 0 ? width2 * 0.42 : -width2 * 0.42;
          push4.push({
            width: 0.035,
            height: 0.055,
            depth: 0.065,
            x: value378 + value380 * Math.cos(rotationY) + Math.sin(rotationY) * 0.04,
            y: value523 * 0.5,
            z: -value380 * Math.sin(rotationY) + Math.cos(rotationY) * 0.04,
            rotationY
          });
        }
        addSharedArchMesh(userData20, push3, value524 ? value.accent : value.doorLeaf, {
          rounded: false,
          roughness: 0.66,
          castShadow: false,
          receiveShadow: false
      ...
      const value529 = swing.hinge === "right";
      const width4 = Math.max(value522 - value527 * 1.4, 0.2);
      const height6 = Math.max(value523 - value527 * 0.8, 0.4);
      const position5 = new THREE.Group();
      position5.position.x = value529 ? value522 / 2 - value527 * 0.5 : -value522 / 2 + value527 * 0.5;
      position5.rotation.y = doorLeafRotation(swing, Math.PI * 0.42);
      userData20.add(position5);
      const centerX = value529 ? -width4 / 2 : width4 / 2;
      if (value525 === "glass") {
        ...
        addSharedArchMesh(position5, [[width4, height6, 0.04, centerX, height6 / 2, 0]], value524 ? value.accent : value.doorLeaf, {
          rounded: false,
          roughness: 0.66,
          castShadow: false,
          receiveShadow: false
      ...
      const value530 = value529 ? -width4 * 0.42 : width4 * 0.42;
      addSharedArchMesh(position5, [[0.035, 0.055, 0.065, value530, value523 * 0.5, 0.04]], value.furnitureDark, {
        rounded: false,
        metalness: 0.45,
        castShadow: false,
        receiveShadow: false
      ...
      userData20.userData.optimizationStats = {
        type: value525 === "glass" ? "door-glass" : "door-solid",
        before: value525 === "glass" ? 9 : 5,
        after: value525 === "glass" ? 4 : 3
      };
      worldGroup.add(userData20);
    }
  }
  const value852 = shadowCastingLightIdSet();
  const push20 = [];
  for (const type16 of floorScene2.items) {
    const x38 = rwpV4(type16);
    if (type16.type === "flooropening") {
      continue;
    }
    const userData33 = sd(type16, value852);
...
function syncPreviewFloorButtons() {
  ...
  for (const entry of r0) {
    const value678 = entry.dataset.previewFloor === value;
    entry.classList.toggle("active", value678);
    entry.setAttribute("aria-pressed", String(value678));
...
function nh(arg0, {
  ...
  if (projectDoc2) {
    projectDoc2.previewFloorMode = arg0 === "all" && projectDoc2.floors.length > 1 ? "all" : "active";
    syncPreviewFloorButtons();
    syncFloorCameraChrome();
    setCameraProjectionMode(getCameraProjectionMode(), {
      preserveView: false
...
function rebuildWorldPreview2({
  ...
  const some = [...projectDoc2.floors].sort((elevation, elevation2) => elevation.elevation - elevation2.elevation);
  const value853 = stageSession ? finite(projectDoc2.exportFloorGap, 3) : finite(projectDoc2.previewFloorGap, 3);
  some.forEach((wall2, arg99) => {
    ...
    name4.position.set(finite(wall2.offsetX, 0), arg99 * value853, finite(wall2.offsetZ, 0));
...
function removeWorldModelLayer(arg0) {
  if (!arg0.size || !worldGroup) {
    return;
  }
  if (getPreviewFloorMode2() !== "all") {
    if (arg0.has(activeFloorId)) {
      rebuildWorldPreview2();
    }
    return;
  }
  const children = worldGroup;
  const value854 = floorScene2;
  const value855 = activeFloorId;
  const value856 = Lo;
  const some = [...projectDoc2.floors].sort((elevation9, elevation10) => elevation9.elevation - elevation10.elevation);
  if (some.some(kind => !children.children.some(userData14 => userData14.userData?.floorId === kind.id))) {
    rebuildWorldPreview2();
    return;
  }
  try {
    for (const [value577, id2] of some.entries()) {
      if (arg0.has(id2.id) && (worldGroup = children.children.find(userData5 => userData5.userData?.floorId === id2.id), floorScene2 = id2.scene, activeFloorId = id2.id, Lo = {
        x: finite(id2.originX, 0),
        y: finite(id2.originY, 0)
      }, worldGroup.position.set(finite(id2.offsetX, 0), value577 * projectDoc2.previewFloorGap, finite(id2.offsetZ, 0)), worldGroup.rotation.set(0, -THREE.MathUtils.degToRad(finite(id2.rotation, 0)), 0), worldGroup.scale.set(1, 1, 1), rebuildWorldPreview(), value577 > 0)) {
    ...
    worldGroup = children;
    floorScene2 = value854;
    activeFloorId = value855;
    Lo = value856;
  }
  Ht(children);
  flushPlanZoomFrame();
...
function flushPlanPanFrame() {
  ...
  const value857 = (minX4.minX + minX4.maxX) / 2;
  const value858 = (minX4.minY + minX4.maxY) / 2;
  return {
    ppm: event,
    floorSurfaceY: -0.008,
    floorPolygons: getFloorPolygons(event),
    toWorld: el => ({
      x: (el.x - value857) / event,
      z: (el.y - value858) / event
    })
  };
}
function rebuildPreviewItemMeshes(arg205) {
  if (worldGroup) {
    for (const userData26 of [...worldGroup.children]) {
      if (userData26.userData.modelLayer === arg205) {
...
function oh(arg0, {
  ...
  rebuildPreviewItemMeshes(arg0);
  const value859 = arg0 === "lights";
  const value860 = value859 ? shadowCastingLightIdSet() : null;
  const push21 = [];
  for (const type17 of floorScene2.items) {
    if (lightItemTypes2.has(type17.type) !== value859) {
      continue;
    }
    ...
    const userData34 = sd(type17, value860);
    ...
    userData34.userData.modelLayer = arg0;
  ...
  requestRender({
    shadows: !value859 && !preserveLightCache5,
    preserveLightCache: preserveLightCache5
  });
}
function rebuildPreviewLightMeshes(preserveLightCache = {}) {
  oh("items", preserveLightCache);
}
function rebuildPreviewLightMeshes2(arg206 = {}) {
  oh("lights", arg206);
}
function applyCameraView(arg207, has17) {
  for (let parent6 = arg207; parent6 && parent6 !== worldGroup; parent6 = parent6.parent) {
...
function isWallCloseSnap({
  excludeModelLayers: arg208 = null
} = {}) {
  const value = new THREE.Box3();
  worldGroup.updateWorldMatrix(true, true);
  worldGroup.traverse(geometry5 => {
    if (!!geometry5.isMesh && !["background", "grid", "light-source-preview"].includes(geometry5.userData?.exportRole) && (!arg208 || !applyCameraView(geometry5, arg208))) {
...
function flushPlanZoomFrame() {
  ...
  const value861 = new THREE.Vector3().setFromMatrixPosition(previewSpotLight.matrixWorld);
  const value862 = new THREE.Vector3().setFromMatrixPosition(previewSpotLight.target.matrixWorld);
  flag.position.copy(value861);
  flag.lookAt(value862);
  ...
  for (const value709 of [arg0.min.x, arg0.max.x]) {
    for (const value578 of [arg0.min.y, arg0.max.y]) {
      for (const value460 of [arg0.min.z, arg0.max.z]) {
        const value446 = new THREE.Vector3(value709, value578, value460).applyMatrix4(flag.matrixWorldInverse);
        x47.min(value446);
        x48.max(value446);
      }
    }
  }
  const value863 = Math.max(x48.x - x47.x, x48.y - x47.y, 1);
  const value864 = Math.max(sofaGeometryCache, value863 * 0.05);
  const value865 = -x48.z;
  const value866 = -x47.z;
  const value867 = Math.max(sofaGeometryCache, (value866 - value865) * 0.08);
  flag.left = x47.x - value864;
  flag.right = x48.x + value864;
  flag.bottom = x47.y - value864;
  flag.top = x48.y + value864;
  flag.near = Math.max(0.1, value865 - value867);
  flag.far = Math.max(flag.near + 1, value866 + value867);
...
function applyCameraView2(forceOrthogonalAxis = {}) {
  ...
  const uppV2 = topViewRotation();
  const value868 = getPreviewFloorMode2() === "all";
  const value869 = pixelsPerMeter() || 100;
  const width17 = getPreviewFloorMode();
  const isEmpty2 = isWallCloseSnap({
    excludeModelLayers: new Set(["items", "lights"])
  });
  const x49 = value868 && !isEmpty2.isEmpty() ? isEmpty2.getSize(new THREE.Vector3()) : null;
  const x50 = isEmpty2.isEmpty() ? null : isEmpty2.getCenter(new THREE.Vector3());
  const value870 = value868 && x49 ? clamp(Math.max(x49.x, x49.z), 5, 100) : clamp(Math.max(width17.width, width17.height) / value869, 5, 35);
  const value871 = value868 && x49 ? x49.y : Math.max(0, ...floorScene2.walls.map(height => height.height || 0));
  const frameSize2 = Math.max(value870 * 1.18, value870 + value871 * 0.32);
  camera2.userData.frameSize = frameSize2;
  camera2.userData.cameraView = cameraView4;
  camera2.userData.topRotation = uppV2;
  const x51 = x50 ? new THREE.Vector3(x50.x, x50.y, x50.z) : new THREE.Vector3(0, Math.min(0.78, value870 * 0.055), 0);
  let value872;
  if (camera2.isPerspectiveCamera) {
    ...
    value872 = Math.max(axisLockedPoint2 * 1.04, value870 * 1.65, 8);
  } else {
    focusCameraOnPoint(frameSize2, camera2.userData.viewportAspect || 1);
    value872 = Math.max(value870 * 3.2, 18);
  }
  if (cameraView4 === "top") {
    camera2.up.copy(topViewForwardVector(uppV2));
    camera2.position.set(x51.x, x51.y + value872, x51.z);
  } else {
    camera2.up.set(0, 1, 0);
    const value679 = new THREE.Vector3(1.08, 1.7, 1.12).normalize();
    camera2.position.copy(x51).addScaledVector(value679, value872);
  }
...
function onPlanPointerMove(event, anchor, forceOrthogonalAxis = false) {
  const value873 = pixelsPerMeter() || 100;
  if (!isSnapActive()) {
  ...
  return snapPoint(event, floorScene2.walls, {
    ...
    angleStepDegrees: 15,
    gridSize: value873 * 0.1,
    intersections: snapOrthogonal.snapIntersections === false ? [] : getWallIntersections(value873),
...
function Oo(arg209 = zr) {
  ...
  if (activeTool === "scale" && wallDrawAnchor2 && arg209) {
  ...
  if (activeTool === "wall") {
    at = onPlanPointerMove(Fi, Tt, arg209);
    No.textContent = onPlanPointerDown(at) ? "闭合：点击闭合空间" : at.kind ? (!isSnapActive() && arg209 ? "锁定" : "吸附") + "：" + at.label : isSnapActive() ? "吸附：自由" : planPoint2;
    ...
    if (wall) {
      ...
      const value462 = {
        wall: wall.wall,
        t: clampWindowT(wall.wall, value461, planPoint)
      };
      Uo = activeTool === "window" ? value462 : null;
      railingPlacementPreview2 = activeTool === "door" ? value462 : null;
      Ko = activeTool === "railing" ? value462 : null;
...
function nw(pointerId8) {
  ...
  if (activeTool === "wall") {
    ...
    const value680 = Math.max(0.75, pixelsPerMeter() * 0.01);
    const length14 = uncoveredCollinearWallSegments(start5, floorScene2.walls, value680);
    if (!length14.length) {
      Tt = {
        ...point3.point
      };
    ...
    const value681 = length14.length !== 1 || distance(length14[0].start, start5.start) > value680 || distance(length14[0].end, start5.end) > value680;
    pushHistory();
    const value682 = Math.max(1, pixelsPerMeter() * 0.01);
    const value683 = closedWallPolygons(floorScene2.walls, value682).length;
    const value684 = length14.map((start, arg56) => ({
      ...start5,
      id: arg56 === 0 ? start5.id : makeId("wall"),
      start: start.start,
      end: start.end
    }));
    floorScene2.walls.push(...value684);
    cachedWallOpenings();
    Ar += 1;
    const element2 = closedWallPolygons(floorScene2.walls, value682).length > value683;
    if (element2) {
      resetWallDrawing2();
    } else {
      Tt = {
    ...
    setSelection("wall", value684[0].id);
    yr.hidden = element2;
    refreshViews();
    scheduleSave();
    if (element2) {
  ...
  const value874 = activeSelectionLightGroupFilter();
  ...
  if (!kind4) {
    ...
    ensureMeasureCanvas();
    if (!pointerId8.shiftKey) {
      rebuildPreviewForAssetFilters(value874);
    }
    planCanvas2.setPointerCapture(pointerId8.pointerId);
    return;
  }
  ...
  const value875 = kind4.kind === "item" && multiSelection.length > 0 && isSelected("item", kind4.id);
  let map8 = [];
  let copied = false;
  if (kind4.kind === "item") {
    if (value875) {
      ...
      const value531 = floorScene2.items.find(item => item.id === kind4.id);
      if (value531) {
        map8 = [value531];
      }
    }
    if (pointerId8.altKey && map8.length) {
      const length6 = map8.map(arg41 => ({
        ...structuredClone(arg41),
        id: makeId("item")
      }));
  ...
  drawPlan();
  rebuildPreviewForAssetFilters(value874);
  const value876 = activeSelectionAssetCategory();
  const previewScope = value874 === value876 ? value876 : "all";
  if (kind4.kind === "item") {
    dragState = {
      type: "move-items",
      pointerId: pointerId8.pointerId,
  ...
  if (dragState?.pointerId === event.pointerId) {
    if (dragState.type === "draw-flooropening") {
      dragState.current = event.shiftKey ? (() => {
        const value363 = x53.x - dragState.start.x;
        const value364 = x53.y - dragState.start.y;
        const value365 = Math.max(Math.abs(value363), Math.abs(value364));
        return {
          x: dragState.start.x + Math.sign(value363 || 1) * value365,
          y: dragState.start.y + Math.sign(value364 || 1) * value365
        };
      })() : x53;
      drawPlan();
      return;
    ...
    if (dragState.type === "move-items") {
      ...
      const value532 = pixelsPerMeter() * 0.05;
      const value533 = isSnapActive() && floorScene2.settings.snapGrid !== false;
      let value534 = x53.x - dragState.start.x;
      let value535 = x53.y - dragState.start.y;
      if (event.shiftKey) {
        if (Math.abs(value534) >= Math.abs(value535)) {
          value535 = 0;
        } else {
          value534 = 0;
        }
      ...
      for (const x16 of dragState.originals) {
        ...
        if (x15) {
          x15.x = value533 ? Math.round((x16.x + value534) / value532) * value532 : x16.x + value534;
          x15.y = value533 ? Math.round((x16.y + value535) / value532) * value532 : x16.y + value535;
        }
      }
    ...
    if (dragState.type === "resize-item") {
      ...
      const value536 = Math.max(finite(dragState.originalItem.height, 0.05), 0.001);
      const value537 = event.shiftKey ? {
        minimum: Math.max(0.1 / Math.max(dragState.originalItem.width, 0.1), 0.1 / Math.max(dragState.originalItem.depth, 0.1), itemMinimumHeight(dragState.originalItem.type) / value536),
        maximum: Math.min(8 / Math.max(dragState.originalItem.width, 0.1), 8 / Math.max(dragState.originalItem.depth, 0.1), 6 / value536)
      } : undefined;
      const value538 = resizeRotatedItemFromCorner(dragState.originalItem, dragState.handle, dragState.anchor, x53, pixelsPerMeter() || 1, event.shiftKey, value537);
      Object.assign(x26, value538);
...
function cancelWallDrawing() {
  previewScene = 0;
  const value878 = defaultExportHeight;
  const value879 = exportAspectRatio;
  defaultExportHeight = 1;
  exportAspectRatio = null;
  if (value879 && Math.abs(value878 - 1) > 1e-8) {
    zoomPlanViewAt(value878, value879);
  }
}
function applyInspectorFields(arg0) {
  defaultExportHeight *= Math.exp(-arg0.deltaY * 0.0012);
  exportAspectRatio = pointerEventToCanvasPoint(arg0);
  hi();
...
function setHoveredInspectorTarget(arg0) {
  if (!dragState || dragState.pointerId !== arg0.pointerId) {
    return;
  }
  if (dragState.type === "draw-flooropening") {
    const {
    ...
    try {
      planCanvas2.releasePointerCapture(arg0.pointerId);
    } catch {}
    dragState = null;
    scheduleLeaveStudio();
    const value685 = Math.abs(x41.x - x40.x) / pixelsPerMeter();
    const value686 = Math.abs(x41.y - x40.y) / pixelsPerMeter();
    if (arg0.type !== "pointercancel" && value685 >= 0.1 && value686 >= 0.1) {
      Cs("flooropening", {
        x: (x40.x + x41.x) / 2,
        y: (x40.y + x41.y) / 2
      }, {
        width: Math.min(20, value685),
        depth: Math.min(20, value686)
      });
    } else {
      drawPlan();
    }
  ...
  if (dragState.type === "marquee") {
    const value687 = activeSelectionLightGroupFilter();
    const value688 = dragState.additive ? [...(selection ? [selection] : []), ...multiSelection] : [];
    const value689 = dragState.moved ? marqueeSelectHits(dragState.start, dragState.current) : [];
    const length15 = [...new Map([...value688, ...value689].map(kind2 => [kind2.kind + ":" + kind2.id, kind2])).values()];
    ...
    updateSelectionInspector();
    drawPlan();
    rebuildPreviewForAssetFilters(value687);
    scheduleLeaveStudio();
    return;
  }
  const type21 = dragState;
  const value880 = type21.moved || type21.copied;
  if (value880) {
  ...
  if (value880 && type21.type === "move-opening") {
    rebuildPreviewMeshes({
      scope: "all"
    });
  } else if (value880 && ["move-items", "resize-item", "rotate-item"].includes(type21.type)) {
    const value579 = selectedEntity();
    rebuildPreviewMeshes({
      scope: type21.previewScope || (value579 ? itemPreviewScope(value579) : activeSelectionAssetCategory())
    });
  }
  if (dragState.type === "pan") {
...
function applyInspectorFields2(arg0) {
  const value = selectedEntity();
  if (!value || selection?.kind !== arg0) {
    return;
  }
  const activeSelectionLightGroupFilterResult = arg0 === "item" ? itemPreviewScope(value) : "all";
  pushHistory();
  if (arg0 === "wall") {
    ...
    const value580 = floorScene2.walls.find(item => item.id === value.wallId);
    if (value580) {
      value.t = clampWindowT(value580, value, pixelsPerMeter());
    }
  } else if (arg0 === "door") {
    ...
    const value539 = floorScene2.walls.find(kind => kind.id === value.wallId);
    if (value539) {
      value.t = clampWindowT(value539, value, pixelsPerMeter());
    }
    ...
    const value463 = floorScene2.walls.find(floor3 => floor3.id === value.wallId);
    if (value463) {
      value.t = clampWindowT(value463, value, pixelsPerMeter());
    }
  } else {
    const value464 = pixelsPerMeter() || 1;
    value.x = finite(selectEl("#item-x").value, value.x / value464) * value464;
    value.y = finite(selectEl("#item-y").value, value.y / value464) * value464;
    ...
    if (value.type === "tv") {
      const value408 = tvMountStyles.has(value.tvMountStyle) ? value.tvMountStyle : "standard";
      const tvMountStyle = tvMountStyles.has(selectEl("#tv-mount-style").value) ? selectEl("#tv-mount-style").value : "standard";
      if (value408 !== tvMountStyle && tvMountStyle === "mobile") {
        ...
        value.elevation = 0;
      } else if (value408 === "mobile" && tvMountStyle !== "mobile" && Math.abs(value.height - modelLoadStatusTimer.height) < 0.001 && Math.abs(value.depth - modelLoadStatusTimer.depth) < 0.001) {
...
async function clearInspectorHover() {
  ...
  try {
    ...
    const value690 = await studioFetch(isStageEmbed ? "/modules/interaction3d/scenes/" + encodeURIComponent(get5.get("sceneId") || "") + "/current?projectId=" + encodeURIComponent(get5.get("projectId") || "") : "/studio3d");
    floorRenameInput.textContent = "户型图绘制";
    document.title = "户型图绘制";
    await loadProjectDocument(value690);
    if (isStageEmbed) {
      await new Promise(requestAnimationFrame);
      const {
        mountStage: value540
      } = await import("/api/v1/modules/interaction3d/stage.js?v=20260910-curtain-cache-v1");
      value540(ww());
      return;
    }
    setSaveStateLabel("已自动保存", "saved");
...
alignFloor.addEventListener("click", _0);
...
for (const e of r0) {
  e.addEventListener("click", () => nh(e.dataset.previewFloor));
}
...
function md(arg210) {
  if (exportPresetEditorOpen && exportPresetEditorOpen !== arg210) {
    commitGlobalWallThickness();
  }
  if (!exportPresetEditorOpen) {
    pushHistory();
    exportPresetEditorOpen = arg210;
    hi();
  }
}
...
function scheduleClearInspectorHover2(arg211 = 80) {
  window.clearTimeout(exportUiDebounceTimer);
  exportUiDebounceTimer = window.setTimeout(commitGlobalWallThickness, arg211);
}
function dw() {
...
function hw() {
  ...
  if (!(Math.abs(wallOpacity - floorScene2.settings.wallOpacity) < 1e-8)) {
    md(globalWallOpacity);
    floorScene2.settings.wallOpacity = wallOpacity;
    scheduleSave();
  }
}
for (const [e, o] of [[globalWallHeight, dw], [globalWallThickness, uw], [globalWallOpacity, hw]]) {
...
function closeLightGroupRenameDialog() {
  ...
  for (const dataset5 of w0) {
    const value691 = dataset5.dataset.itemType;
    const value692 = lightItemTypes2.has(value691);
    const value693 = RESERVED_TEXTURE_UNITS.has(value691);
    const value694 = assetCategory === "light" ? value692 : assetCategory === "appliance" ? value693 : !value693 && !value692;
    dataset5.hidden = !value694;
  }
}
function Ks(arg212) {
  const value881 = ["home", "appliance", "light"].includes(arg212) ? arg212 : "home";
  const value882 = activeSelectionLightGroupFilter();
  hideLightGroupContextMenu();
  assetCategory = value881;
  for (const dataset6 of m0) {
    const value695 = dataset6.dataset.assetCategory === value881;
    dataset6.classList.toggle("active", value695);
    dataset6.setAttribute("aria-pressed", String(value695));
  }
  closeLightGroupRenameDialog();
  lightItemTypes.hidden = value881 === "light";
  stairLikeTypes.hidden = value881 !== "light";
  const type22 = selectedEntity();
  const value883 = selection?.kind === "item" && type22 && lightItemTypes2.has(type22.type);
  if (selection && value881 === "light" != !!value883) {
    clearSelection();
  }
  if (multiSelection.length) {
    clearSelection();
  ...
  updateSelectionInspector();
  drawPlan();
  if (value882 !== activeSelectionLightGroupFilter()) {
    rebuildPreviewForAssetFilters(value882);
  }
}
for (const e of m0) {
  e.addEventListener("click", () => Ks(e.dataset.assetCategory));
}
Ks("home");
Tg.addEventListener("click", () => {
  pushHistory();
  const has10 = new Set(floorScene2.lightGroups.map(name2 => name2.name));
  let value710 = floorScene2.lightGroups.length + 1;
  while (has10.has("灯组 " + value710)) {
    value710 += 1;
  }
  const id2 = {
    id: makeId("light-group"),
...
toolEls.addEventListener("submit", preventDefault => {
...
Tf.addEventListener("submit", camera2 => {
  ...
  const querySelectorAll = normalizeLabelText(lightGroupRenameInput.value, isClosest.name, 24);
  if (querySelectorAll !== isClosest.name) {
    pushHistory();
    isClosest.name = querySelectorAll;
    refreshViews("none");
    scheduleSave();
  }
  syncLightPropertySelectAll();
...
function buildStageReferenceScene(querySelectorAll = wallFields) {
  return [...querySelectorAll.querySelectorAll("[data-light-target-item-id]")];
}
function el() {
  const length32 = buildStageReferenceScene();
  const value884 = length32.filter(checked6 => checked6.checked).length;
  windowFields.textContent = value884 + "/" + length32.length + " 灯";
  ka.disabled = !length32.length;
  ka.textContent = length32.length && value884 === length32.length ? "取消全选" : "全选";
  for (const querySelector of wallFields.querySelectorAll("[data-light-target-group-id]")) {
    const length16 = buildStageReferenceScene(querySelector);
    const value696 = length16.filter(checked3 => checked3.checked).length;
    querySelector.querySelector("[data-light-target-group-count]").textContent = value696 + "/" + length16.length + " 灯";
    querySelector.querySelector("[data-light-target-group-toggle]").textContent = length16.length && value696 === length16.length ? "取消全选" : "全选";
  }
}
function renderLightPropertyTargetList(arg213) {
  ...
  for (const id2 of floorScene2.lightGroups) {
    ...
    for (const type13 of length17) {
      ...
      const value541 = (get7.get(type13.type) || 0) + 1;
      get7.set(type13.type, value541);
      ...
      textContent.textContent = get6.get(type13.type) > 1 ? name3.name + " " + value541 : name3.name;
      const textContent2 = document.createElement("small");
      const value542 = clampLightPropertyValue(arg213, type13[arg213], type13.type);
      textContent2.textContent = (type13.id === selection?.id ? "当前灯 · " : "") + "当前 " + formatLightPropertyValue(arg213, value542);
      append.append(textContent, textContent2);
      className.append(type10, append);
...
wallFields.addEventListener("click", target4 => {
  const closest = target4.target.closest("[data-light-target-group-toggle]");
  if (!closest) {
    return;
  }
  const value711 = closest.closest("[data-light-target-group-id]");
  const every2 = buildStageReferenceScene(value711);
...
wallFields.addEventListener("change", el);
...
Rf.addEventListener("submit", event => {
  ...
  const has11 = new Set(buildStageReferenceScene().filter(checked => checked.checked).map(dataset => dataset.dataset.lightTargetItemId));
  ...
  const length22 = length21.map(item => ({
    item,
    value: clampLightPropertyValue(value712, value714, item.type)
  })).filter(item3 => Math.abs(finite(item3.item[value712]) - item3.value) > 0.000001);
  if (length22.length) {
    pushHistory();
    for (const item5 of length22) {
      item5.item[value712] = item5.value;
    }
    refreshViews("lights");
    scheduleSave();
  }
  postAutoDiagramBusy();
  showToast("已将" + value713 + "应用到 " + length21.length + " 盏灯。", "success");
});
for (const e of w0) {
  ...
  e.addEventListener("click", () => {
    ...
    Cs(e.dataset.itemType, planPoint);
  });
}
...
planStage.addEventListener("drop", preventDefault4 => {
  ...
  planStage.classList.remove("dragging-item");
  const value715 = preventDefault4.dataTransfer.getData("application/x-ha-bridge-3d-item");
  if (value715) {
    Cs(value715, screenToPlanWithView(pointerEventToCanvasPoint(preventDefault4)));
  }
});
planCanvas2.addEventListener("pointerdown", nw);
planCanvas2.addEventListener("pointermove", iw);
...
f0.addEventListener("click", hm);
g0.addEventListener("click", fm);
p0.addEventListener("click", gm);
...
detailsPanelEl.addEventListener("submit", preventDefault10 => {
  ...
  const value716 = normalizeActiveExportPresetSlot(projectDoc2?.activeExportPresetSlot, length23.length);
  const name7 = length23[value716];
  if (!name7) {
    closeExportPresetRenameDialog();
    return;
  }
  const value717 = defaultExportPresetLabel(name7, value716);
  const name8 = Iu(exportPresetRenameInput.value, value716);
  name7.name = name8;
  ...
  if (!!command && command.componentId === autoDiagramComponentId) {
    if (command.type === "ha-bridge-floorplan-auto-diagram-floor") {
      if (command.command === "set-floor") {
        ...
        const value381 = command.value === "all" && projectDoc2.floors.length > 1 ? "all" : id2?.id || activeFloor()?.id || activeFloorId;
        setExportFloorScope(value381);
        pw();
      }
      return;
    }
    ...
    if (command.type === "ha-bridge-floorplan-auto-diagram-camera") {
      ...
      if (command.command === "restore") {
        ...
        if (snapshot.snapshot) {
          pi(snapshot.snapshot, snapshot.snapshot.viewportAspect);
        } else {
...
const endDetailsPanelResize2 = (pointerId3, arg150 = true) => {
  ...
  const value720 = Math.abs(floorScene2.settings.previewPanelRatio - pointerId4.startPreviewRatio) > 0.0001 || Math.abs(floorScene2.settings.detailsPanelWidthRatio - pointerId4.startWidthRatio) > 0.0001;
...
detailsResizer.addEventListener("lostpointercapture", arg151 => endDetailsPanelResize2(arg151, false));
...
detailsResizer.addEventListener("keydown", signal => {
  const value721 = signal.key === "ArrowUp" ? -0.03 : signal.key === "ArrowDown" ? 0.03 : 0;
  const value722 = signal.key === "ArrowLeft" ? 0.03 : signal.key === "ArrowRight" ? -0.03 : 0;
  if (!value721 && !value722) {
    return;
  }
  signal.preventDefault();
  const minimumHeightRatio = studioLayoutMetrics();
  floorScene2.settings.previewPanelRatio = clamp(floorScene2.settings.previewPanelRatio + value721, minimumHeightRatio.minimumHeightRatio, minimumHeightRatio.maximumHeightRatio);
  floorScene2.settings.detailsPanelWidthRatio = clamp(floorScene2.settings.detailsPanelWidthRatio + value722, minimumHeightRatio.minimumWidthRatio, minimumHeightRatio.maximumWidthRatio);
  applyPreviewPaneWidth();
  applyDetailsPaneWidth();
  scheduleSave();
});
for (const e of cameraModeEls) {
  e.addEventListener("click", () => {
    const arg0 = e.dataset.cameraMode === "perspective" ? "perspective" : "orthographic";
    if (arg0 !== getCameraProjectionMode()) {
      ...
      if (stageSession) {
        exportStatus.textContent = arg0 === "perspective" ? "已切换为透视构图" : "已切换为正交构图";
      } else {
        scheduleSave();
      }
    }
...
for (const e of cameraViewEls) {
  e.addEventListener("click", () => {
    ...
    if (arg0 !== cameraViewMode()) {
      ...
      nudgeCamera(arg0);
      if (stageSession) {
        exportStatus.textContent = arg0 === "top" ? "已切换为顶视构图" : "已切换为自由构图";
      } else {
        scheduleSave();
      }
    }
...
baseLightControlsHeader?.addEventListener("pointermove", pointerId6 => {
  ...
  const value723 = pointerId6.clientX - lightCacheTileMap.startX;
  const value724 = pointerId6.clientY - lightCacheTileMap.startY;
  if (!lightCacheTileMap.moved && Math.hypot(value723, value724) < 4) {
    return;
  }
  lightCacheTileMap.moved = true;
  pointerId6.preventDefault();
  const width11 = baseLightControls.getBoundingClientRect();
  const value725 = Math.max(8, window.innerWidth - width11.width - 8);
  const value726 = Math.max(8, window.innerHeight - width11.height - 8);
  baseLightControls.style.right = "auto";
  baseLightControls.style.left = clamp(lightCacheTileMap.startLeft + value723, 8, value725) + "px";
  baseLightControls.style.top = clamp(lightCacheTileMap.startTop + value724, 8, value726) + "px";
});
const fh = pointerId7 => {
...
Qd.addEventListener("click", stopPropagation => {
...
lightPropertyApplyValue.addEventListener("submit", preventDefault11 => {
  ...
  const value727 = distance(shiftKeyHeld.start, shiftKeyHeld.end);
  if (meters <= 0 || value727 <= 0) {
    showToast("请输入有效的真实长度。", "error");
    return;
  }
  pushHistory();
  floorScene2.calibration = {
    pixelsPerMeter: value727 / meters,
    reference: {
      ...shiftKeyHeld,
      meters
    }
...
window.addEventListener("keydown", key2 => {
  ...
  const value728 = key2.target instanceof HTMLInputElement || key2.target instanceof HTMLTextAreaElement || lightPropertyApplyTitle.open;
  if (key2.code === "Space" && !value728) {
    saveConflictState = true;
    key2.preventDefault();
  }
  if (key2.key.toLowerCase() === "s" && !value728) {
    Or = true;
    Oo();
    drawPlan();
  }
  ...
  const value729 = key2.metaKey || key2.ctrlKey;
  if (value729 && key2.key.toLowerCase() === "z") {
    key2.preventDefault();
    if (key2.shiftKey) {
      redoEdit();
    } else {
  ...
  if (value729 && key2.key.toLowerCase() === "d") {
    key2.preventDefault();
    selectedItemIds();
    return;
  }
  if (value729 && key2.key.toLowerCase() === "c") {
    key2.preventDefault();
    copySelectedItems();
    return;
  }
  if (value729 && key2.key.toLowerCase() === "v") {
    key2.preventDefault();
    pasteClipboardItems();
    return;
  }
  ...
  if (x42 && !value729) {
    ...
    if (length7.length) {
      ...
      const value447 = (key2.altKey ? 0.01 : key2.shiftKey ? 0.25 : 0.05) * (pixelsPerMeter() || 1);
      const has3 = new Set(length7);
      for (const id2 of floorScene2.items) {
        if (has3.has(id2.id)) {
          id2.x += x42.x * value447;
          id2.y += x42.y * value447;
        }
      }
  ...
  if (key2.key === "Escape") {
    ...
    hideLightGroupContextMenu();
    schedulePlanRedraw();
    const value543 = activeSelectionLightGroupFilter();
    clearSelection();
    updateSelectionInspector();
    drawPlan();
    rebuildPreviewForAssetFilters(value543);
  }
});
...
async function mw() {
  ...
  scheduleAdaptiveQuality(0);
  const value886 = performance.now() + 1800;
  while (isPreviewQualityReady() && (!lightCacheReady || previewQualityJustBecameReady || previewLightCache.hidden) && performance.now() < value886) {
    await new Promise(requestAnimationFrame);
  }
}
function ww() {
  ...
  let value888 = "";
  let cacheEpoch = 0;
  const value889 = node3 => {
    releaseRoot?.releaseRoot?.(node3.node);
    studioReady?.releaseRoot?.(node3.node);
    disposeObject3dResources(node3.node);
  };
  const value890 = (has7 = null) => {
    if (!has7) {
      cacheEpoch++;
    }
    for (const [value465, value466] of entryMap) {
      if (!has7 || !!has7.has(value465)) {
        value889(value466);
        entryMap.delete(value465);
      }
    }
  };
  function release(node6) {
    ...
    const value730 = node6.id;
    const node7 = entryMap.get(value730);
    ...
    entryMap.delete(value730);
    entryMap.set(value730, node6);
    studioReady?.retainRoot?.(node6.node);
    releaseRoot?.retainRoot?.(node6.node);
    while (entryMap.size > 8) {
      const value544 = entryMap.keys().next().value;
      value889(entryMap.get(value544));
      entryMap.delete(value544);
    }
    return true;
  }
  ...
  function fn3() {
    ...
    const value731 = enabled6 === enabled7 || enabled6 && enabled7 && enabled6.enabled === enabled7.enabled && enabled6.fullWidth === enabled7.fullWidth && enabled6.fullHeight === enabled7.fullHeight && enabled6.offsetX === enabled7.offsetX && enabled6.offsetY === enabled7.offsetY && enabled6.width === enabled7.width && enabled6.height === enabled7.height;
    if (motion.motion === height10 && motion.camera === camera2 && motion.distance === distance2 && motion.height === height8 && motion.aspect === aspect && motion.weight === weight && motion.near === camera2.near && motion.far === camera2.far && value731 && motion.matrix.equals(camera2.projectionMatrix)) {
      return;
    }
    Object.assign(view3, {
    ...
    const value732 = view3.projectionMatrix.elements;
    const value733 = aspect2.projectionMatrix.elements;
    for (let value581 = 0; value581 < 16; value581++) {
      camera2.projectionMatrix.elements[value581] = value732[value581] * (1 - weight) + value733[value581] / distance2 * weight;
    }
  ...
  let get11 = new Map();
  const value920 = {
    restore: () => fn8(performance.now(), true)
  };
  function fn4(arg152) {
    if (value916 !== null) {
      window.clearTimeout(value916);
    }
    value916 = null;
    if (arg152) {
      if (isStageWarmup) {
  ...
  function fn5(type18, arg153) {
    const value734 = clamp(finite(arg153, 0), 0, 100) / 100;
    if (yt) {
      return value734;
    } else if (type18.type === "striplight") {
      return Math.pow(value734, 0.82);
    } else {
      return spotLightBrightnessResponse(type18.type, value734);
    }
  }
  function fn6(type19) {
    const range = defaultLightPresets[type19.type] || defaultLightPresets.downlight;
    const value735 = deferExternalModels[type19.type] || 1.1;
    if (type19.type !== "striplight") {
      return (type19.type === "ceilinglight" ? 680 : 520) * value735;
    }
    const value736 = clamp(finite(type19.lightRange, range.range), 0.5, 10);
    const value737 = Math.max(finite(type19.elevation, 2.7), 0.4);
    return clamp(value736 / range.range, 0.45, 1.65) * 48 * clamp(Math.max(1, Math.pow(value737 / 2.7, 2)), 1, 4) * value735;
  }
  function fn7(intensity4, intensity5) {
  ...
  function fn8(arg154, arg155 = false) {
    if (isCapturingFrame === value920) {
      if (value917 !== worldGroup || value918 !== worldGroup?.children[0]) {
        value917 = worldGroup;
        value918 = worldGroup?.children[0];
        get10 = collectWorldItemKeys();
        get11 = new Map(collectVisibleLights().map(itemKey => [itemKey.itemKey, itemKey]));
        value919 = true;
      }
      if (arg155 || value919) {
        for (const [value409, value410] of get10) {
          const item2 = get11.get(value409);
          if (!item2) {
            continue;
          }
          const visible3 = item2.group?.enabled !== false && item2.item.lightBrightness > 0;
          for (const intensity of value410) {
    ...
    for (const [value582, value583] of size) {
      fn7(value582, sampleLightTransition(value583, arg154));
    }
  }
  function fn9(duration, arg156) {
    const value738 = duration.duration ? clamp((arg156 - duration.started) / duration.duration, 0, 1) : 1;
    return duration.from + (duration.to - duration.from) * value738 * value738 * (3 - value738 * 2);
  }
  function fn10() {
    if (value911) {
      cancelAnimationFrame(value911);
    }
    value911 = 0;
    size2.clear();
    rs = false;
  }
  function fn11(arg157) {
    value911 = 0;
    if (!lightCacheReady || previewQualityJustBecameReady || previewLightCache.hidden || isCapturingFrame) {
      fn10();
      return;
    }
    for (const [value584, started] of size2) {
      pendingModelLoads.set(value584, fn9(started, arg157));
      if (arg157 >= started.started + started.duration) {
        size2.delete(value584);
      }
    }
    rs = size2.size > 0;
    blitLightCacheToOverlay();
    if (size2.size) {
      value911 = requestAnimationFrame(fn11);
    }
  }
  function fn12(values, immediate, arg158) {
    if (!lightCacheReady || previewQualityJustBecameReady || isBakingLightCache || previewLightCache.hidden || isCapturingFrame || value907 || value908) {
      return false;
    }
    const every3 = [...values.values()].filter(floorId6 => getPreviewFloorMode2() === "all" || floorId6.floorId === activeFloorId);
    ...
    for (const wasOn2 of every3) {
      const value545 = previewScopedItemKey(wasOn2.floorId, wasOn2.item.lightGroupId);
      const value546 = size2.get(value545);
      const from3 = value546 ? fn9(value546, started3) : finite(pendingModelLoads.get(value545), wasOn2.wasOn ? 1 : 0);
      size2.set(value545, {
        from: from3,
        to: wasOn2.isOn ? 1 : 0,
        started: started3,
  ...
  function fn13() {
    ...
    fn10();
    isCapturingFrame = value920;
    if (value913 !== null) {
      window.clearTimeout(value913);
    }
    value913 = null;
    window.clearTimeout(stageSessionEndTimer);
    stageSessionEndTimer = null;
    ...
    value918 = worldGroup?.children[0];
    value919 = true;
    const value739 = performance.now();
    for (const groupKey of map7) {
      const to2 = get8.get(groupKey.groupKey);
      if (!to2) {
        continue;
      }
      const value547 = fn9(to2, value739);
      for (const userData16 of get10.get(groupKey.itemKey) || []) {
        const value448 = finite(userData16.userData.lightOnIntensity, 0);
        const color2 = userData16.color.toArray();
        size.set(userData16, createLightTransition({
          intensity: value448 * value547,
          color: color2
        }, {
          intensity: value448 * to2.to,
          color: color2
        }, value739, Math.max(0, to2.started + to2.duration - value739)));
      }
    }
    if (size.size && !value912) {
      value912 = requestAnimationFrame(fn15);
    }
    syncOrbitControls();
    return get10;
  }
  function fn14() {
    value913 = null;
    if (!floorSelectionQuery && !Wo && !$o && !value907 && !value908 && !size.size && isCapturingFrame === value920) {
      if (isBakingLightCache || isStageWarmup) {
        value913 = window.setTimeout(fn14, 60);
        return;
      }
  ...
  function fn15(arg159) {
    value912 = 0;
    fn8(arg159);
    let value740 = false;
    for (const [visible8, started2] of size) {
      if (!(arg159 - started2.started < started2.duration)) {
    ...
    if (size.size) {
      ...
      fn4(false);
      if (isCapturingFrame === value920) {
        value913 = window.setTimeout(fn14, 180);
      }
    }
  }
  function fn16() {
    ...
    value916 = null;
    isStageWarmup = false;
    for (const [value585, to4] of size) {
      fn7(value585, {
        ...to4.to,
        complete: true
      });
  ...
  function setLightStates(filter5, editor = {}) {
    ...
    if (!value915) {
      value915 = true;
      window.addEventListener("pagehide", fn16, {
        once: true
      });
    }
    const immediate2 = editor.immediate === true || !value914 || window.matchMedia?.("(prefers-reduced-motion: reduce)").matches;
    const map = new Map();
    for (const floorId7 of filter5) {
      ...
      for (const lightBrightness3 of scene5.scene.items) {
        ...
        if (wasOn !== isOn || brightness !== lightBrightness3.lightBrightness || kelvin !== lightBrightness3.lightTemperature) {
          map.set(layerScopedKey(scene5.id, lightBrightness3.id), {
    ...
    const value741 = isPreviewQualityReady();
    if (value741 && fn12(map, immediate2, editor)) {
      return;
    }
    ...
    if (value741) {
      has12 = fn13();
    } else if ([...map.keys()].some(arg57 => !has12.has(arg57))) {
      has12 = ensureWorldItemsCached(collectVisibleLights().filter(itemKey2 => map.has(itemKey2.itemKey)));
    }
    let value742 = false;
    const value743 = performance.now();
    for (const [value586, item6] of map) {
      if (getPreviewFloorMode2() !== "all" && item6.floorId !== activeFloorId) {
        continue;
      }
      const length8 = has12.get(value586);
      if (length8?.length) {
        for (const userData11 of length8) {
          const value382 = size.get(userData11);
          const value383 = fn5(item6.item, item6.previousBrightness);
          const value384 = get9.get(value586)?.includes(userData11) && value383 > 1e-7 && userData11.userData.lightOnIntensity > 0 ? userData11.userData.lightOnIntensity / value383 : fn6(item6.item);
          const lightOnIntensity = value384 * fn5(item6.item, item6.item.lightBrightness);
          const color = {
            intensity: item6.isOn ? lightOnIntensity : 0,
            color: new THREE.Color(lightEffectColorHex(item6.item.lightTemperature)).toArray()
          };
          const intensity2 = value382 ? sampleLightTransition(value382, value743) : null;
          const value385 = !item6.wasOn && item6.isOn && (!intensity2 || intensity2.intensity <= 0.000001) ? {
            intensity: 0,
            color: color.color
          } : intensity2 || {
            intensity: value741 ? item6.wasOn ? value384 * value383 : 0 : userData11.intensity,
            color: value741 ? new THREE.Color(lightEffectColorHex(item6.previousKelvin)).toArray() : userData11.color.toArray()
          };
          userData11.userData.lightOnIntensity = lightOnIntensity;
          userData11.userData.lightBrightness = item6.item.lightBrightness;
          const value386 = lightTransitionDurationMs(item6.wasOn, item6.isOn, item6.fadeDuration, {
            ...editor,
            immediate: immediate2
          });
          const value387 = createLightTransition(value385, color, value743, value386);
          const value388 = userData11.visible;
          fn7(userData11, sampleLightTransition(value387, value743));
          value742 ||= value388 !== userData11.visible;
          if (value386) {
            size.set(userData11, value387);
          } else {
    ...
    if (!value912) {
      if (size.size) {
        ...
        value913 = window.setTimeout(fn14, 100);
      }
    }
  }
  function fn17() {
    renderer.getSize(x54);
    const value744 = Math.max(x54.x || 1, 1);
    const value745 = Math.max(x54.y || 1, 1);
    const value746 = value744 + "/" + value745 + "/" + value909 + "/" + camera2.uuid;
    if (value746 === value910) {
      fn3();
      return;
    }
    value910 = value746;
    if (value909) {
      camera2.setViewOffset(value744, value745, value744 * value909 / 2, 0, value744, value745);
    } else {
  ...
  function fn18() {
    ...
    if (value906 === "horizontal") {
      orbitControls.minPolarAngle = orbitControls.maxPolarAngle = orbitControls.getPolarAngle();
    }
    if (value906 === "vertical") {
      orbitControls.minAzimuthAngle = orbitControls.maxAzimuthAngle = orbitControls.getAzimuthalAngle();
    }
  }
  function worldPoint(arg160, x43, y4, arg161 = 0.1) {
    const scene9 = projectDoc2.floors.find(id2 => id2.id === arg160);
    if (!scene9) {
      return null;
    }
    const value747 = scene9.scene.calibration?.pixelsPerMeter || 1;
    if (getPreviewFloorMode2() === "all") {
      const indexOf = [...projectDoc2.floors].sort((elevation5, elevation6) => elevation5.elevation - elevation6.elevation);
      const x27 = planPointToWorldXZ(scene9, {
        x: x43,
        y: y4
      });
      return new THREE.Vector3(x27.x, indexOf.indexOf(scene9) * projectDoc2.previewFloorGap + arg161, x27.z);
    }
    const value748 = floorScene2;
    floorScene2 = scene9.scene;
    const minX3 = getPreviewFloorMode();
    floorScene2 = value748;
    return new THREE.Vector3((x43 - (minX3.minX + minX3.maxX) / 2) / value747, arg161, (y4 - (minX3.minY + minX3.maxY) / 2) / value747);
  }
  function fn19(arg162) {
    if (!projectDoc2.floors.some(id2 => id2.id === arg162)) {
      return null;
    }
    const value749 = projectDoc2.floors.find(id2 => id2.id === arg162)?.scene.calibration?.pixelsPerMeter || 1;
    const value750 = worldPoint(arg162, 0, 0, 0);
    return new THREE.Matrix4().makeBasis(worldPoint(arg162, value749, 0, 0).sub(value750), new THREE.Vector3(0, 1, 0), worldPoint(arg162, 0, value749, 0).sub(value750)).setPosition(value750);
  }
  function fn20() {
    ...
    for (const scene6 of projectDoc2.floors) {
      for (const start2 of scene6.scene.walls || []) {
        ...
        value751 = Math.max(value751, finite(start2.height, scene6.scene.settings?.wallHeight || 2.8));
      }
    }
    ...
    if (getPreviewFloorMode2() === "all") {
      y5.y = (projectDoc2.floors.length - 1) * projectDoc2.previewFloorGap / 2 + value751 / 2;
      return y5;
    }
    const id2 = projectDoc2.floors.find(id2 => id2.id === activeFloorId);
    const x44 = rotatePlanPointByFloor(id2, y5);
    return worldPoint(id2.id, x44.x, x44.y, value751 / 2);
  }
  function fn21(arg163) {
    if (value903 !== worldGroup || value904 !== planCanvas) {
      const value548 = fn20();
      const isEmpty = value548 ? null : isWallCloseSnap({
        excludeModelLayers
      });
      value903 = worldGroup;
      value904 = planCanvas;
      value905 = value548 || (isEmpty.isEmpty() ? null : isEmpty.getCenter(new THREE.Vector3()));
    }
    return (value905 || arg163).clone();
  }
  function fn22() {
    clone3 ||= fn21(orbitControls.target);
    if (value901 === orbitControls) {
      return;
    }
    const addEventListener3 = orbitControls;
    const quaternion = camera2;
    const value752 = addEventListener3.update.bind(addEventListener3);
    value901 = addEventListener3;
    const value753 = () => {
      if (isCapturingFrame === value920) {
        if (value913 !== null) {
          window.clearTimeout(value913);
    ...
    addEventListener3.addEventListener("change", () => {
      if (!value907) {
        if (value908 && isPreviewQualityReady() && isCapturingFrame !== value920) {
          fn13();
          fn8(performance.now());
          Ht(worldGroup, {
            rebuildAtlas: false
    ...
    let value754 = addEventListener3.enabled;
    Object.defineProperty(addEventListener3, "enabled", {
      configurable: true,
      get: () => enabled8 && !value907 && value754,
      set: arg50 => {
        value754 = arg50 === true;
      }
    });
    ...
    addEventListener3.update = arg65 => {
      if (value907) {
        return false;
      }
      const angleTo = quaternion.quaternion.clone();
      const value467 = value752(arg65);
      if (enabled8 && angleTo.angleTo(quaternion.quaternion) > 1e-7) {
        const value389 = quaternion.quaternion.clone().multiply(angleTo.invert());
        const clone = clone3.clone().sub(addEventListener3.target);
        const value390 = clone.clone().sub(clone.applyQuaternion(value389));
        quaternion.position.add(value390);
        addEventListener3.target.add(value390);
  ...
  function fn23() {
    if (value898 !== worldGroup || value899 !== worldGroup?.children[0]) {
      value898 = worldGroup;
      value899 = worldGroup?.children[0];
      push24 = [];
    ...
    for (const visible9 of push24) {
      visible9.visible = value897 && !visible9.userData.floorBackgroundHidden;
    }
  }
  ...
  const value921 = new URLSearchParams(globalThis.window?.location?.search || "").get("performance-diagnostics") === "1" && new URLSearchParams(window.location.search).get("reflection-work") === "baseline";
  const changed = createGroundReflections({
    ...
    floorLighting: yt,
    cull: !value921,
    syncLighting: arg58 => value921 ? studioReady?.sync(arg58, true) : studioReady?.syncCamera(arg58),
    requestFrame: () => updateLightPreview(),
    getStateKey: () => [planCanvas, floorSelectionQuery, yt ? "" : lightCacheEpoch, lightCacheReady, renderer.toneMappingExposure].join("|")
  });
  const value922 = new URLSearchParams(globalThis.window?.location?.search || "").get("floorEffects");
  let value923 = false;
  let value924 = false;
  function setEditorEffects(arg164, arg165) {
    const value755 = arg164 && !arg165;
    renderer.domElement.dataset.editorEffects = arg164 ? arg165 ? "light-preview" : "paused" : "runtime";
    if (value923 !== value755) {
      value923 = value755;
      changed.setSuspended?.(value923 || value924);
      updateLightPreview();
    }
  }
  const value925 = (yt || value922 === "follow") && value922 !== "deferred";
  let value926 = false;
  let value927 = null;
  let autoUpdate = true;
  const has20 = new Map();
  function fn24(arg166) {
    if (value926 !== arg166) {
      value926 = arg166;
      if (value925) {
        if (arg166) {
        ...
        renderer.shadowMap.autoUpdate = arg166 ? true : autoUpdate;
        renderer.shadowMap.needsUpdate = true;
        studioReady?.setMotion?.(arg166, true);
        return;
      }
      if (arg166) {
        value927 = null;
        ...
        renderer.shadowMap.needsUpdate = true;
        value927 = performance.now();
      }
      studioReady?.setMotion?.(arg166);
    }
  }
  function fn25() {
    if (value926 || value927 === null) {
      return;
    }
    const value756 = Math.min(1, (performance.now() - value927) / 280);
    for (const [intensity3, value587] of has20) {
      intensity3.intensity = value587 * value756;
    }
    if (value756 < 1) {
      updateLightPreview();
    } else {
  ...
  const profileFrameWork = (arg100, arg101) => typeof fitCameraToSelection2 == "function" ? fitCameraToSelection2(arg100, arg101) : arg101();
  const finish = createFloorTransition({
    THREE,
    getRoot: () => worldGroup,
    dispose: disposeObject3dResources,
    release,
    suspendReflections: arg59 => {
      value924 = arg59;
      changed.setSuspended?.(value923 || value924);
      renderCache?.setMotion?.(arg59);
      fn24(arg59);
    },
    invalidate: arg60 => {
      if (arg60) {
  ...
  renderCache?.setFrameProvider?.(arg102 => {
    const node4 = finish.records.find(id2 => id2.id === arg102);
    if (node4) {
  ...
  studioReady?.setMotionTransformProvider?.(arg103 => {
    const node5 = finish.records.find(id2 => id2.id === arg103);
    if (node5) {
  ...
  const apply = previewScene2.onBeforeRender;
  ...
  name9.onBeforeRender = () => {
    if (value892 && !value926) {
      value892 = shadowAtlas ? !shadowAtlas.refreshGeometry(worldGroup, push23) : false;
    }
  };
  ...
  previewScene2.onBeforeRender = function (...arg104) {
    if (changed.stats.inCapture) {
      if (value921) {
        studioReady?.sync(arg104[2], true);
      }
      return;
    }
    fn25();
    if (!value926) {
      profileFrameWork("curtains", () => value891?.());
    }
    if (value892 && !value926) {
      if (value893 !== worldGroup || value894 !== planCanvas) {
        value893 = worldGroup;
        value894 = planCanvas;
        push22 = [];
        push23 = [];
      ...
      renderCache?.invalidate(value896, true);
    }
    profileFrameWork("lighting-and-contact", () => apply?.apply(this, arg104));
    if (!value926) {
    ...
    if (!renderer.getRenderTarget()) {
      profileFrameWork("reflections", () => changed.render(arg104[2]));
  ...
  function fn26(arg167 = orbitControls.target.clone()) {
    const distanceTo = camera2.position.clone();
    orbitControls.dispose();
    orbitControls = createOrbitControls(camera2);
    camera2.position.copy(distanceTo);
    orbitControls.target.copy(arg167);
    const value757 = Math.max(distanceTo.distanceTo(arg167), 0.001);
    const value758 = distanceTo.clone().sub(arg167).normalize().angleTo(camera2.up);
    orbitControls.minDistance = Math.min(2, value757);
    orbitControls.maxDistance = Math.max(100, value757 * 2);
    orbitControls.minZoom = Math.min(0.35, camera2.zoom);
    orbitControls.maxZoom = Math.max(6, camera2.zoom);
    orbitControls.maxPolarAngle = Math.max(Math.PI * 0.49, value758 + 0.00001);
  ...
  return {
    onCameraChange(arg105) {
      const addEventListener = renderer.domElement;
      addEventListener.addEventListener("hb-i3d-camera-frame", arg105);
      return () => addEventListener.removeEventListener("hb-i3d-camera-frame", arg105);
    },
    createFrameLoop: arg76 => createDemandFrameLoop(arg76),
    setPresentedVisible(arg106) {
    ...
    invalidateReflections(arg107) {
      changed.changed(arg107);
    },
    get modelRoot() {
      return worldGroup;
    },
    ...
    setEnvironmentScene(arg108) {
      releaseRoot = arg108;
    },
    setEnvironmentAirflow(arg109) {
      setRoot = arg109;
    },
    setCurtainSync(arg110) {
      value891 = arg110;
    },
    curtainFrame({
      key: arg111,
      ...
      const value588 = arg111 !== saveState;
      const value589 = Wo;
      if (!!value588 || value589 !== arg113) {
        if (value588) {
          try {
            const value356 = arg25 => {
              const map = new Map();
              for (const value297 of JSON.parse(arg25)) {
                const value277 = value297[1];
                if (!map.has(value277)) {
                  map.set(value277, []);
                }
                map.get(value277).push(value297);
              }
              return map;
            };
            const keys = value356(saveState);
            const keys2 = value356(arg111);
            changed.changed([...new Set([...keys.keys(), ...keys2.keys()])].filter(arg22 => JSON.stringify(keys.get(arg22)) !== JSON.stringify(keys2.get(arg22))));
          } catch {
            changed.changed(arg114);
          }
        }
        if (arg112 !== importPlan) {
          shadowAtlas?.prepareRoot(worldGroup);
          studioReady?.invalidate();
        }
        value896 = [...new Set([...value895, ...arg114])];
        value895 = arg114;
        if (value588 || arg113) {
          if (!value589) {
            fn13();
            fn8(performance.now(), true);
          }
          if (value588) {
            value892 = true;
          }
    ...
    setVacuumMoving(arg115) {
      const value590 = arg115 === true;
      if (value590 !== $o) {
        $o = value590;
        canvasEmpty++;
        if (value590) {
    ...
    environmentModelPose(arg116, arg117) {
      let userData27;
      worldGroup?.traverse(userData13 => {
        if (userData13.userData?.environmentFloorId === arg116 && userData13.userData?.environmentModelId === arg117) {
    ...
    setEnvironmentActive(arg118) {
      const value591 = arg118 === true;
      if (value591 !== floorSelectionQuery) {
        if (value591) {
          fn13();
          fn8(performance.now(), true);
        }
        floorSelectionQuery = value591;
        if (!value591) {
          fn14();
        }
        requestRender();
    ...
    pickEnvironmentModel(arg119, arg120, length9 = [], arg121 = 0) {
      ...
      const value592 = push15.filter(userData8 => {
      ...
      const value593 = Math.max(0, Math.min(12, arg121));
      const value594 = [[0, 0], ...(value593 ? [[value593, 0], [-value593, 0], [0, value593], [0, -value593], [value593 * 0.7, value593 * 0.7], [-value593 * 0.7, value593 * 0.7], [value593 * 0.7, -value593 * 0.7], [-value593 * 0.7, -value593 * 0.7]] : [])];
      for (const [value468, value469] of value594) {
        setFromCamera.setFromCamera(new THREE.Vector2((arg119 + value468 - width6.left) / width6.width * 2 - 1, 1 - (arg120 + value469 - width6.top) / width6.height * 2), camera2);
        for (const object of setFromCamera.intersectObjects(value592, false)) {
    ...
    transformCamera(arg122, arg123, arg124 = false) {
      return transformSceneCamera(arg122, combinedFixedCameraView, projectDoc2, arg123, arg124);
    },
    async readSceneUpdate(arg125) {
      const get4 = new URLSearchParams(window.location.search);
      const value595 = new URLSearchParams({
        projectId: get4.get("projectId") || "",
        since: hasProjectLoaded2.syncKey || ""
      });
      const scene7 = await withRequestTimeout(15000, async signal => {
        const status = await fetch("/api/v1/modules/interaction3d/scenes/" + encodeURIComponent(get4.get("sceneId") || "") + "/current?" + value595, {
    ...
    async replaceScene(scene8) {
      ...
      finish.finish();
      value890(floors.full || floors.lighting ? null : new Set(floors.floors));
      fn16();
      value900 = null;
      clone3 = null;
      value914 = false;
    ...
    getOrbitCenter() {
      return (fn20() || fn21(orbitControls.target)).toArray();
    },
    setOrbitPivot(arg126) {
      clone3 = arg126 ? new THREE.Vector3().fromArray(arg126) : null;
      fn22();
    },
    orbitCameraPose(target, arg127) {
      const up2 = structuredClone(target);
      const value596 = finite(arg127, 0) % (Math.PI * 2);
      if (Math.abs(value596) < 1e-12 || Math.abs(Math.abs(value596) - Math.PI * 2) < 1e-12) {
        return up2;
      }
      clone3 ||= fn21(new THREE.Vector3().fromArray(target.target));
      const value597 = new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0, 1, 0), value596);
      for (const value470 of ["position", "target"]) {
        up2[value470] = new THREE.Vector3().fromArray(target[value470]).sub(clone3).applyQuaternion(value597).add(clone3).toArray();
      }
      let clone2 = new THREE.Vector3().fromArray(target.up || [0, 1, 0]);
      const value598 = new THREE.Vector3().fromArray(target.position).sub(new THREE.Vector3().fromArray(target.target));
      if (clone2.lengthSq() < 1e-12 || clone2.clone().cross(value598).lengthSq() < 1e-12) {
        const value449 = THREE.MathUtils.degToRad(finite(target.topRotation, 0));
        clone2 = new THREE.Vector3(Math.sin(value449), 0, -Math.cos(value449));
        if (clone2.clone().cross(value598).lengthSq() < 1e-12) {
          clone2.set(1, 0, 0);
        }
      }
      up2.up = clone2.applyQuaternion(value597).toArray();
      return up2;
    },
    setFocusViewport(arg128) {
      const value599 = clamp(finite(arg128, 0), 0, 0.7);
      if (value599 !== value909) {
        value909 = value599;
        fn17();
        requestRender({
          preserveLightCache: false
    ...
    beginCameraMotion(arg129, arg130) {
      const mode = this.cameraState();
      const value600 = arg130 && (height10 || mode.mode !== arg129);
      const fromHeight = value600 ? height10?.height ?? resetOrbitTarget(camera2, orbitControls.target) : 0;
      ...
      setCameraProjectionMode(arg129, {
        preserveView: true,
      ...
      if (value600) {
        height10 = {
          ...
          height: fromHeight,
          fromWeight,
          toWeight: arg129 === "perspective" ? 1 : 0,
          weight: fromWeight
        };
        return mode;
      } else {
    ...
    applyCameraFrame(arg131, arg132, arg133) {
      const value601 = clamp(finite(arg133, 0), 0, 0.7);
      const value602 = value601 === value909;
      value909 = value601;
      this.applyCameraPose(arg131, arg132, value602);
    },
    applyCameraPose(view2, arg134 = 1, preserveLightCache2 = true) {
      if (height10) {
        if (arg134 >= 1) {
          height10 = null;
        } else {
          height10.height = height10.fromHeight + (height10.toHeight - height10.fromHeight) * arg134;
          height10.weight = height10.fromWeight + (height10.toWeight - height10.fromWeight) * arg134;
        }
      }
      softLockPreviewOrbit();
    ...
    endCameraMotion() {
      ...
      unlockPreviewOrbit();
      requestRender();
      if (typeof isCapturingFrame !== "undefined" && isCapturingFrame === value920 && !size.size) {
        if (value913 !== null) {
          window.clearTimeout(value913);
        }
        value913 = window.setTimeout(fn14, 180);
    ...
    setFloorGap(arg135) {
      const value603 = Number.isFinite(arg135) ? clamp(arg135, 0, 20) : null;
      if (value603 !== null || value887 !== null) {
        const previewFloorGap = value603 ?? normalizeProjectDocument(hasProjectLoaded2.scene).previewFloorGap;
        if (Math.abs(projectDoc2.previewFloorGap - previewFloorGap) > 0.000001) {
          finish.finish();
          const value391 = getPreviewFloorMode2() === "all";
          const value392 = value391 ? finish.take(projectDoc2.floors.map(id2 => id2.id), fn19, true) : [];
          projectDoc2.previewFloorGap = previewFloorGap;
          for (const id2 of value392) {
            finish.reuse(id2, fn19(id2.id));
          }
          if (value391) {
            clone3 = null;
            value903 = null;
    ...
    appearance(baseLighting2) {
      const value604 = JSON.stringify([baseLighting2.baseLighting, baseLighting2.lightingMode, baseLighting2.lightRegionOverrides]);
      if (value604 !== value888) {
        value890();
        value888 = value604;
        changed.changed();
      }
      ...
      value902 = !!baseLighting2.baseLighting;
      const value605 = clamp(finite(baseLighting2.renderScale, 1), 0.25, 2);
      const value606 = typeof baseLighting2.motionRenderScale == "number" && Number.isFinite(baseLighting2.motionRenderScale) ? clamp(baseLighting2.motionRenderScale, 0.25, 1) : null;
      if (value605 !== ur || value606 !== isAutoDiagramEmbed) {
        ur = value605;
        isAutoDiagramEmbed = value606;
        renderer.setPixelRatio(computeStudioPixelRatio(previewOrbitLocked));
        onPreviewContainerResize();
        requestRender();
      }
      const value607 = value897 !== (baseLighting2.backgroundVisible !== false);
      value897 = baseLighting2.backgroundVisible !== false;
      document.body.classList.toggle("is-background-hidden", !value897);
      fn23();
    ...
    floorDefaultCamera(arg136) {
      const position7 = arg136 === "all" ? combinedFixedCameraView?.combinedFixedCameraView : combinedFixedCameraView?.floors.find(id2 => id2.id === arg136)?.scene.settings?.fixedCameraView;
      if (!position7) {
        return null;
      }
      const value608 = {
      ...
      return transformSceneCamera(value608, combinedFixedCameraView, projectDoc2, arg136);
    },
    get floorTransitionActive() {
      return finish.active;
    },
    advanceFloorTransition(arg137, arg138) {
      profileFrameWork("motion-and-settle", () => finish.sample(arg137, arg138));
    },
    setFloorSlideCameras(arg139, arg140) {
      finish.setSlideCameras(arg139, arg140);
    },
    ...
    profileFrameWork,
    recordFloorFrame(arg141, arg142) {
      if (typeof setPerfMotionActive == "function") {
        setPerfMotionActive(arg141, arg142);
      }
    },
    ...
    transitionFloor(target2) {
      ...
      const value609 = [...projectDoc2.floors].sort((elevation3, elevation4) => elevation3.elevation - elevation4.elevation).map(id2 => id2.id);
      const value610 = arg51 => {
        const scene4 = projectDoc2.floors.find(id2 => id2.id === arg51);
        const value411 = scene4.scene.calibration?.pixelsPerMeter || 1;
        const value412 = this.worldPoint(arg51, 0, 0, 0);
        return new THREE.Matrix4().makeBasis(this.worldPoint(arg51, value411, 0, 0).sub(value412), new THREE.Vector3(0, 1, 0), this.worldPoint(arg51, 0, value411, 0).sub(value412)).setPosition(value412);
      };
      const value611 = getPreviewFloorMode2() === "all";
      const cacheKey3 = value611 ? "all" : activeFloorId;
      const find = profileFrameWork("detach-source", () => finish.take(value611 ? value609 : [activeFloorId], value610, value611));
      for (const cacheKey of find) {
        cacheKey.cacheKey ||= cacheKey3;
        cacheKey.cacheEpoch ??= cacheEpoch;
      }
      const expandByObject = new THREE.Box3();
      for (const node2 of find) {
        expandByObject.expandByObject(node2.node);
      }
      const x32 = expandByObject.getSize(new THREE.Vector3());
      const value612 = !value611 && target2 !== "all";
      const value613 = value612 ? new THREE.Vector3(0, 1, 0).applyQuaternion(camera2.quaternion).normalize() : null;
      const value614 = value612 ? resetOrbitTarget(camera2, orbitControls.target) * 1.2 : Math.max(20, x32.x, x32.z) * 1.5;
      const map5 = target2 === "all" ? value609 : [target2];
      const every = map5.map(arg42 => find.find(id2 => id2.id === arg42 && id2.cacheEpoch === cacheEpoch) || entryMap.get(arg42));
      const length10 = every.every(Boolean) ? every : null;
      if (length10) {
      ...
      if (length10) {
        ...
        value890(new Set(map5));
      }
      profileFrameWork("set-destination", () => this.setFloor(target2, length10, value610));
      const value615 = this.getOrbitCenter();
      const value616 = profileFrameWork("capture-destination", () => finish.capture(map5, value610, target2 === "all"));
      for (const cacheKey2 of value616) {
        cacheKey2.cacheKey = target2;
        cacheKey2.cacheEpoch = cacheEpoch;
      }
      profileFrameWork("begin-motion", () => finish.begin(find, value616, value609, value614, value612, value613));
      value891?.();
      return value615;
    },
    setFloor(arg143, arg144 = null, arg145 = fn19) {
      const id2 = projectDoc2.floors.find(id2 => id2.id === arg143) || projectDoc2.floors[0];
      const previewFloorMode = arg143 === "all" && projectDoc2.floors.length > 1 ? "all" : "active";
      if (!!arg144 || activeFloorId !== id2.id || getPreviewFloorMode2() !== previewFloorMode) {
        if (size.size || typeof isCapturingFrame !== "undefined" && isCapturingFrame === value920) {
          fn16();
        }
        value914 = false;
        activeFloorId = id2.id;
        ...
        if (arg144) {
          ...
          syncPreviewFloorButtons();
          syncFloorCameraChrome();
          const value393 = [...projectDoc2.floors].sort((elevation, elevation2) => elevation.elevation - elevation2.elevation)[0].id;
          for (const id2 of arg144) {
            finish.reuse(id2, arg145(id2.id)).traverse(userData => {
              if (["background", "grid"].includes(userData.userData?.exportRole)) {
                userData.userData.floorBackgroundHidden = previewFloorMode === "all" && id2.id !== value393;
                userData.visible = value897 && !userData.userData.floorBackgroundHidden;
              }
            });
          }
          ...
          if (studioReady) {
            worldGroup.traverse(userData2 => {
              ...
              const value315 = projectDoc2.floors.find(id2 => id2.id === userData2.userData.lightFloorId)?.scene.items.find(id2 => id2.id === userData2.userData.lightItemId);
              if (value315) {
                studioReady.register(userData2, value315);
              }
            });
          }
          Ht(worldGroup);
    ...
    setLightStates,
    setEditorEffects,
    mapLightEffectState: arg77 => mapLightEffectState(arg77),
    lightEffectColorHex: arg78 => lightEffectColorHex(arg78),
    cameraState(arg146 = false) {
      if (arg146) {
        fn26();
      }
      return {
    ...
    setCameraProjection(arg147) {
      activeCameraSettings().cameraMode = arg147 === "perspective" ? "perspective" : "orthographic";
    ...
    setCameraFocalLength(arg148) {
      activeCameraSettings().cameraFocalLength = clamp(finite(arg148, 50), 18, 120);
      applyCameraFocalLength();
      requestRender();
    },
    setCameraInteraction(rotationMode = {}) {
      const value617 = ["horizontal", "vertical"].includes(rotationMode.rotationMode) ? rotationMode.rotationMode : "free";
      const enabled4 = rotationMode.enabled === true;
      const value618 = rotationMode.panEnabled !== false;
      const value619 = rotationMode.zoomEnabled !== false;
      const value620 = enabled8 !== enabled4 || value906 !== value617 || enablePan !== value618 || enableZoom !== value619;
      enabled8 = enabled4;
      value906 = value617;
      enablePan = value618;
      enableZoom = value619;
    ...
    whenPresented() {
      value900 ||= (async () => {
        const value394 = loadVisibleExternalModels();
        let value395;
        try {
          await Promise.race([Promise.allSettled(value394), new Promise(arg20 => {
            value395 = window.setTimeout(arg20, 8000);
          })]);
        } finally {
          window.clearTimeout(value395);
        }
        refreshStudioChrome();
        await new Promise(requestAnimationFrame);
        await new Promise(requestAnimationFrame);
    ...
    setCameraView(arg149) {
      activeCameraSettings().cameraView = arg149 === "top" ? "top" : "free";
    ...
    restoreCamera(mode2) {
      ...
      getCameraPose(camera2, orbitControls.target);
      onPreviewContainerResize();
      fn26(orbitControls.target.clone());
      requestRender();
    },
    topView() {
      nudgeCamera("top", {
