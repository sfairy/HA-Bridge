...
export function normalizeFixedCameraView(view) {
  ...
  return {
    ...
    topRotation:
      (((Math.round(finite(view.topRotation, 0) / 90) * 90) % 360) + 360) % 360,
    position,
    target,
    visibleHeight: clamp(finite(view.visibleHeight, 10), 1, 100),
    fov: clamp(finite(view.fov, 36), 20, 80),
    focalLength:
      view.focalLength !== null &&
      view.focalLength !== undefined &&
      Number.isFinite(Number(view.focalLength))
        ? clamp(finite(view.focalLength, 50), 18, 120)
        : null,
  };
}
...
export function normalizeCameraSettings(settings) {
  return {
    ...
    cameraTopRotation:
      (((Math.round(finite(settings?.cameraTopRotation, 0) / 90) * 90) % 360) +
        360) %
      360,
    cameraMode:
      settings?.cameraMode === "orthographic" ? "orthographic" : "perspective",
...
export function normalizeBaseLighting(lighting) {
  ...
  return {
    exposure: clamp(
      finite(source.exposure, DEFAULT_BASE_LIGHTING.exposure),
      0.5,
      2,
    ),
    hemisphereIntensity: clamp(
      finite(
        source.hemisphereIntensity,
    ...
    ambientIntensity: clamp(
      finite(source.ambientIntensity, DEFAULT_BASE_LIGHTING.ambientIntensity),
      0,
      2,
    ),
    mainIntensity: clamp(
      finite(source.mainIntensity, DEFAULT_BASE_LIGHTING.mainIntensity),
      0,
      5,
    ),
    mainAzimuth: clamp(
      finite(source.mainAzimuth, DEFAULT_BASE_LIGHTING.mainAzimuth),
      -180,
      180,
    ),
    mainElevation: clamp(
      finite(source.mainElevation, DEFAULT_BASE_LIGHTING.mainElevation),
      5,
      89,
    ),
    mainShadowIntensity: clamp(
      finite(
        source.mainShadowIntensity,
    ...
    fillIntensity: clamp(
      finite(source.fillIntensity, DEFAULT_BASE_LIGHTING.fillIntensity),
      0,
      3,
    ),
    fillAzimuth: clamp(
      finite(source.fillAzimuth, DEFAULT_BASE_LIGHTING.fillAzimuth),
      -180,
      180,
    ),
    fillElevation: clamp(
      finite(source.fillElevation, DEFAULT_BASE_LIGHTING.fillElevation),
      0,
      89,
    ),
    topIntensity: clamp(
      finite(source.topIntensity, DEFAULT_BASE_LIGHTING.topIntensity),
      0,
      3,
    ),
    topAzimuth: clamp(
      finite(source.topAzimuth, DEFAULT_BASE_LIGHTING.topAzimuth),
      -180,
      180,
    ),
    topElevation: clamp(
      finite(source.topElevation, DEFAULT_BASE_LIGHTING.topElevation),
      0,
      89,
    ),
  };
