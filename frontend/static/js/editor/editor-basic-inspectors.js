import { clampNumber, roundField } from "./editor-utils.js?v=20260831-editor-utils-v1";
...
export function fitInspectorComponentToDimensions(component, template, measure) {
  if (!component) {
    return;
  }
...
export function inspectorComponentMetrics(component, document) {
  ...
  return {
    ...
    width,
    height,
    left: roundField(clampNumber((Number(position.x || 0) + width / 2) / canvasWidth * 100, 0, 100)),
    top: roundField(clampNumber((Number(position.y || 0) + height / 2) / canvasHeight * 100, 0, 100)),
    ...
    scale: roundField(clampNumber(Number(component.style?.scale || 1) * 100, 1, 500)),
