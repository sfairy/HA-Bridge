...
export function setupGlobalLog({
  ...
  if (!globalLogOpen || !globalLogDialog || globalLogOpen.dataset.globalLogReady === "true") {
    return null;
  }
  globalLogOpen.dataset.globalLogReady = "true";
  const globalLogClose = globalLogDialog.querySelector("#global-log-close");
  const globalLogCloseFooter = globalLogDialog.querySelector("#global-log-close-footer");
  ...
  const globalLogCategory = globalLogDialog.querySelector("#global-log-category");
  ...
  function renderLogList(entries) {
    ...
    for (const entry of entries) {
      ...
      const timeLabel = entry.clientTimestamp ? "客户端发生 " + formatLogTime(entry.clientTimestamp) + " · 接收 " + formatLogTime(entry.timestamp) : formatLogTime(entry.timestamp);
      meta.textContent = timeLabel + " · " + (entry.source || "系统后台") + " · " + (entry.category || "系统");
      ...
      if (entry.details || Object.keys(entry.context || {}).length) {
        ...
        pre.textContent = [...Object.entries(entry.context || {}).map(([key, contextValue]) => key + ": " + contextValue), entry.details || ""].filter(Boolean).join("\n");
        details.append(summary, pre);
        body.append(details);
      }
      if (Number(entry.repeatCount || 1) > 1) {
        const repeat = document.createElement("span");
        repeat.textContent = "重复 " + entry.repeatCount + " 次 · 最近" + (entry.lastClientTimestamp ? "发生" : "接收") + " " + formatLogTime(entry.lastClientTimestamp || entry.lastTimestamp || entry.timestamp);
        body.append(repeat);
      }
  ...
  function syncCategoryOptions(categories) {
    ...
    globalLogCategory.value = [...globalLogCategory.options].some(option => option.value === previous) ? previous : "";
  }
  ...
  async function refresh({
    ...
    try {
      ...
      logEntries = append ? [...new Map([...logEntries, ...items].map(entry => [entry.id, entry])).values()] : items;
      nextOffset = payload?.hasMore ? payload.nextOffset : null;
      syncCategoryOptions(payload?.categories || []);
      renderLogList(logEntries);
      const storage = payload?.storage || {};
      const maxBytesLabel = storage.maxBytes ? " / " + (storage.maxBytes / 1024 / 1024).toFixed(0) + " MB 上限" : "";
      const historyPauseLabel = logEntries.length > pageLimit ? " · 查看历史时暂停自动刷新" : "";
      const storageErrorLabel = storage.healthy === false ? " · 日志存储异常：" + (storage.lastError || "写入失败") + "（待写 " + (storage.pendingEvents || 0) + "，丢弃 " + (storage.droppedEvents || 0) + "）" : "";
      const writeFailureLabel = storage.healthy !== false && (storage.writeFailures > 0 || storage.droppedEvents > 0) ? " · 历史写入失败 " + (storage.writeFailures || 0) + " 次，丢弃 " + (storage.droppedEvents || 0) + " 条" : "";
      globalLogStatus.textContent = "已显示 " + logEntries.length + " / " + (payload?.total ?? logEntries.length) + " 条 · 自动保留最近 " + (storage.retentionDays || payload?.retentionDays || 7) + " 天" + maxBytesLabel + historyPauseLabel + storageErrorLabel + writeFailureLabel;
  ...
  async function open() {
    ...
    autoRefreshTimer = window.setInterval(() => {
      if (logEntries.length <= pageLimit && !globalLogList.querySelector("details[open]")) {
        refresh();
      }
    }, 10000);
  }
  ...
  globalLogMore.addEventListener("click", () => refresh({
    append: true
  }));
  ...
  globalLogExport.addEventListener("click", () => {
    ...
    link.download = "ha-bridge-global-log-" + new Date().toISOString().slice(0, 10) + ".txt";
