...
export function createInteraction3dEditorPickers({
  ...
  return {
    ...
    icon({
      ...
      currentIcon ||= iconDeviceKind === "vacuum" ? "mdi:robot-vacuum" : iconDeviceKind === "television" ? "mdi:television" : iconDeviceKind === "nas" ? "mdi:nas" : iconDeviceKind === "cover" ? "mdi:curtains" : iconDeviceKind === "climate" ? "mdi:air-conditioner" : DEFAULT_LIGHT_ICON;
      return openPicker({
        kind: "icon",
        title: iconDeviceKind === "vacuum" ? "选择扫地机按钮图标" : iconDeviceKind === "television" ? "选择电视按钮图标" : iconDeviceKind === "nas" ? "选择NAS按钮图标" : iconDeviceKind === "cover" ? "选择窗帘按钮图标" : iconDeviceKind === "climate" ? "选择空调按钮图标" : "选择灯光按钮图标",
    ...
    async entity({
      ...
      const test = startsWith === "presence" ? /^(binary_sensor|event)\.[a-z0-9_]+$/ : startsWith === "vacuum" ? /^vacuum\.[a-z0-9_]+$/ : startsWith === "vacuum-map" ? /^(camera|image)\.[a-z0-9_]+$/ : startsWith === "vacuum-room" ? /^[a-z_]+\.[a-z0-9_]+$/ : isTelevision ? /^media_player\.[a-z0-9_]+$/ : isTelevisionPower ? /^(media_player|switch|binary_sensor|input_boolean)\.[a-z0-9_]+$/ : isNas ? /^(binary_sensor|switch|input_boolean)\.[a-z0-9_]+$/ : isCover ? /^cover\.[a-z0-9_]+$/ : isClimate ? /^climate\.[a-z0-9_]+$/ : /^(light|switch)\.[a-z0-9_]+$/;
      const {
        editorEntityMatches: matchEntities
      } = createEditorPickerQueries({
        entityPickerConfig: () => ({
          recommended: entityId5 => startsWith === "presence" ? ["occupancy", "motion", "presence"].includes(entityId5.deviceClass || entityId5.device_class || entityId5.attributes?.device_class || getEntityState(entityId5.entityId)?.attributes?.device_class) : entityId5.entityId.startsWith(isTelevision || isTelevisionPower ? "media_player." : isNas || startsWith === "presence" ? "binary_sensor." : isCover ? "cover." : isClimate ? "climate." : "light.")
        }),
      ...
      return openPicker({
        kind: "entity",
        title: startsWith === "presence" ? "选择人在传感器" : startsWith === "vacuum" ? "选择扫地机实体" : startsWith === "vacuum-map" ? "选择扫地机地图" : startsWith === "vacuum-room" ? "选择房间快捷指令" : isTelevision ? "选择电视媒体实体（Apple TV）" : isTelevisionPower ? "选择电视电源状态" : isNas ? "选择NAS开启实体" : isCover ? "选择窗帘实体" : isClimate ? "选择空调实体" : "选择灯光实体",
        ...
        emptyText: startsWith === "presence" ? "没有匹配的人在传感器或移动事件，请先在 Home Assistant 接入设备" : startsWith.startsWith("vacuum") ? "没有匹配的实体，请先在 Home Assistant 中接入" : isTelevision ? "没有匹配的媒体播放器，请先在 Home Assistant 接入 Apple TV" : isTelevisionPower ? "没有匹配的电源状态实体" : isNas ? "没有匹配的开关或二元传感器" : isCover ? "没有匹配的窗帘" : isClimate ? "没有匹配的空调" : "没有匹配的灯光或开关",
        itemClass: "entity-list",
        getPage: ({
          query: searchQuery,
          page: pageNumber
