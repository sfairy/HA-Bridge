...
export function sha(input) {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let round = 0; round < 64; round++) {
      const value = h + (rotateRight(e, 6) ^ rotateRight(e, 11) ^ rotateRight(e, 25)) + (e & f ^ ~e & g) + roundConstants[round] + schedule[round] >>> 0;
      const item = (rotateRight(a, 2) ^ rotateRight(a, 13) ^ rotateRight(a, 22)) + (a & b ^ a & c ^ b & c) >>> 0;
      h = g;
      g = f;
      f = e;
      e = d + value >>> 0;
