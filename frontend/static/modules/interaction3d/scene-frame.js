function getFloorFrame(config, floorId, allFloorId) {
  ...
  if (floorId === "all" && floors.length > 1) {
    ...
    return {
      ...
      c: cos,
      s: sin,
      x: (floor.offsetX || 0) - (cos * (floor.originX || 0) + sin * (floor.originY || 0)) / ppm,
      z: (floor.offsetZ || 0) - (-sin * (floor.originX || 0) + cos * (floor.originY || 0)) / ppm,
      y: [...floors].sort((a, b) => a.elevation - b.elevation).indexOf(floor) * config.previewFloorGap
    };
  }
  let points = scene.walls?.length ? scene.walls.flatMap(wall => [wall.start, wall.end]) : scene.items?.length ? scene.items : scene.background?.width && scene.background?.height ? [{
    x: 0,
    y: 0
  }, {
...
export function transformSceneCamera(camera, fromConfig, toConfig, floorId, reverse = false) {
  ...
  const sharedFloorId = fromConfig.floors?.find(fromFloor => toConfig.floors?.some(toFloor => toFloor.id === fromFloor.id))?.id;
  ...
  const rotateVector = ([x, y, z]) => [cos * x + sin * z, y, -sin * x + cos * z];
  const transformPoint = ([x, y, z]) => {
    const rotated = rotateVector([x - fromFrame.x, y - fromFrame.y, z - fromFrame.z]);
    return [rotated[0] * scale + toFrame.x, rotated[1] * scale + toFrame.y, rotated[2] * scale + toFrame.z];
  };
  return {
