...
export function runtimeEntityStateIsActive(entityOrEvent) {
  const state = String(entityOrEvent?.newState?.state ?? entityOrEvent?.state ?? "")
    .trim()
    .toLowerCase();
...
export function coverPositionReachedTarget(
  currentPosition,
  targetPosition,
  direction,
) {
...
export function runtimeCoverStateIsActive(entityOrEvent) {
  const stateEntry = entityOrEvent?.newState || entityOrEvent || {};
  const state = String(stateEntry.state || "")
    .trim()
    .toLowerCase();
  if (state === "opening") {
    return true;
...
export function relatedDeviceEntity(
  ...
  const matches = [...entityMetadata.values()].filter(
    (candidate) =>
...
export function relatedDeviceDomainEntity(entityMetadata, entityId, domain) {
  ...
  matches.sort((left, right) => {
    const leftLightScore = /灯|照明|light/i.test(
      (left.name || "") + " " + (left.entityId || ""),
    )
      ? 0
      : 1;
    const rightLightScore = /灯|照明|light/i.test(
      (right.name || "") + " " + (right.entityId || ""),
    )
      ? 0
      : 1;
    return (
      leftLightScore - rightLightScore ||
      String(left.entityId || "").length -
        String(right.entityId || "").length ||
...
function extractPointIndices(entityId) {
  return new Set(
    [...String(entityId || "").matchAll(/_(?:s|p)_(\d+)(?:_|$)/gi)].map(
      (match) => match[1],
    ),
  );
}
    ...
    [...entityMetadata.values()]
      .filter(
        (candidate) =>
          ...
          ["light", "switch"].includes(String(candidate.domain || "")) &&
          entityMetadataIsAvailable(candidate),
      )
      .map((candidate) => {
        const searchText =
        ...
        const sharesPointIndex = [...sourcePointIndices].some((pointIndex) =>
          candidatePointIndices.has(pointIndex),
        );
...
function findRelatedAirerEntity(entityMetadata, entityId, domain, pattern) {
  const metadata = entityMetadata.get(entityId);
  if (!metadata?.deviceId) {
    const hydMatch = String(entityId || "").match(
...
export function relatedAirerMotorActionEntities(entityMetadata, entityId) {
  ...
  const buttons = [...entityMetadata.values()].filter(
    (candidate) =>
      candidate.entityId !== entityId &&
      candidate.deviceId === metadata.deviceId &&
      candidate.domain === "button" &&
      entityMetadataIsAvailable(candidate),
  );
  return Object.fromEntries(
    Object.entries(MOTOR_CONTROL_PATTERNS).map(([action, pattern]) => {
...
export function airerVisualDrop(position, coverState = "", calibration = {}) {
  ...
  const raised =
    calibration.raised === null || calibration.raised === undefined
      ? Number.NaN
      : Number(calibration.raised);
  const lowered =
    calibration.lowered === null || calibration.lowered === undefined
      ? Number.NaN
      : Number(calibration.lowered);
  const raisedBaseline = Number.isFinite(raised)
    ? raised
    : Number.isFinite(lowered) && lowered >= 50
      ? 0
      : 100;
  const travel =
    (Number.isFinite(lowered) ? lowered : raisedBaseline < 50 ? 100 : 0) -
    raisedBaseline;
  return (
    2 +
    (Math.abs(travel) < 0.5
      ? 0
      : Math.max(0, Math.min(1, (clamped - raisedBaseline) / travel))) *
      38
  );
}
...
export function learnAirerPositionCalibration(
  ...
  const commandRaised =
    calibration.commandRaised === null ||
    calibration.commandRaised === undefined
      ? Number.NaN
      : Number(calibration.commandRaised);
  const commandLowered =
    calibration.commandLowered === null ||
    calibration.commandLowered === undefined
      ? Number.NaN
      : Number(calibration.commandLowered);
  if (
    !!Number.isFinite(reported) &&
    !!Number.isFinite(commanded) &&
    ...
    if (
      ...
      Math.abs(commanded - commandLowered) <= 0.5
    ) {
...
export function airerPresentationPosition(position, calibration = {}) {
  ...
  if (Math.abs(loweredBaseline - raisedBaseline) < 0.5) {
    ...
    return Math.max(
      ...
      Math.min(
        100,
        ((loweredBaseline - clamped) / (loweredBaseline - raisedBaseline)) *
          100,
      ),
    );
  }
...
export function airerReportedPosition(sensorState, coverState, calibration = {}) {
  ...
  const hasRaised =
    calibration.raised !== null &&
    calibration.raised !== undefined &&
    Number.isFinite(Number(calibration.raised));
  const hasLowered =
    calibration.lowered !== null &&
    calibration.lowered !== undefined &&
    Number.isFinite(Number(calibration.lowered));
  if (
    hasRaised &&
    hasLowered &&
    Math.abs(Number(calibration.lowered) - Number(calibration.raised)) >= 0.5 &&
    Number.isFinite(sensorNumeric)
  ) {
...
export function airerDevicePosition(presentationPosition, calibration = {}) {
  ...
  const loweredCommand =
    ...
    calibration.commandLowered === undefined
      ...
      : Number(calibration.commandLowered);
  if (
    !Number.isFinite(raisedCommand) ||
    !Number.isFinite(loweredCommand) ||
    Math.abs(loweredCommand - raisedCommand) < 0.5
  ) {
    return clamped;
  }
...
const WATER_HEATER_RELATED_DOMAINS = new Set([
  "switch",
  "select",
  "number",
  "button",
  ...
  return [...entityMetadata.values()]
    .filter(
      (candidate) =>
        candidate.entityId !== entityId &&
        candidate.deviceId === metadata.deviceId &&
        WATER_HEATER_RELATED_DOMAINS.has(String(candidate.domain || "")) &&
        entityMetadataIsAvailable(candidate),
    )
    .sort(
      (left, right) =>
        (DOMAIN_ORDER.get(left.domain) ?? 99) -
          (DOMAIN_ORDER.get(right.domain) ?? 99) ||
        String(left.entityId || "").localeCompare(String(right.entityId || "")),
    );
}
export function waterHeaterRelatedEntityLabel(sourceEntity, relatedEntity) {
  let label = String(relatedEntity?.name || relatedEntity?.originalName || "")
    .replace(/\s+/g, " ")
    .trim();
  const prefixes = [
    ...new Set(
      [sourceEntity?.originalName, sourceEntity?.name]
        .map((name) =>
          String(name || "")
            .replace(/\s+/g, " ")
            .trim(),
  ...
  return (
    label ||
    (String(relatedEntity?.entityId || "").split(".", 2)[1] || "扩展功能").replace(
      /_/g,
      " ",
    )
  );
...
export function relatedCoverMotorReverseEntity(entityMetadata, entityId) {
  ...
  return (
    (metadata?.deviceId &&
      [...entityMetadata.values()].find(
        (candidate) =>
          candidate.deviceId === metadata.deviceId &&
          ["switch", "select"].includes(String(candidate.domain || "")) &&
...
function isCoverMotorReversed(entityMetadata, states, entityId) {
  ...
  return (
    !!reverseEntity?.entityId &&
    !!stateLooksEnabled(states.get(reverseEntity.entityId))
  );
}
export function coverMotorIsReversedForComponent(
  ...
  return isCoverMotorReversed(entityMetadata, states, entityId);
}
...
export function coverPresentationState(entityOrEvent, motorReversed = false) {
  ...
  if (currentPosition === null) {
    ...
    (motorReversed ? 100 - currentPosition : currentPosition) <=
...
export function dreamCurtainStatusText(
  ...
  return (
    ...
    " · 叶片：" +
    dreamCurtainBladeLabel(bladePosition)
  );
}
...
export function coverToggleServiceForComponent(
  ...
  const presentationState = coverComponentIsDream(
    ...
    ? coverPhysicalPresentationState(stateEntry, motorReversed)
    : coverPresentationState(stateEntry, motorReversed);
