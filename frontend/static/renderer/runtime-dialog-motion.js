export function runtimeDialogUsesStableMotion({
  ...
  return /AppleWebKit/i.test(text) && /Safari/i.test(text) && !/Chrome|Chromium|CriOS|Edg|OPR|OPiOS|FxiOS/i.test(text);
}
export function playStableRuntimeDialogEntrance(overlay, dialog) {
  if (!runtimeDialogUsesStableMotion()) {
    return [];
  }
  const targets = overlay.classList.contains("hb-runtime-simplified-motion") ? [overlay, dialog, ...dialog.querySelectorAll("*")] : [overlay, dialog];
  ...
  if (firstElementChild?.animate) {
    animations.push(firstElementChild.animate([{
      opacity: 0
    }, {
      opacity: 1
    }], {
...
export function playFixedDeviceDropEntrance(element, {
  distance = 150,
  delay = 90
} = {}) {
  if (!element || window.matchMedia?.("(prefers-reduced-motion: reduce)").matches) {
