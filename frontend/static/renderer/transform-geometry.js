...
export function airflowLayerGeometry(
  ...
  const layerWidth =
    (width * Math.max(0.01, Number(properties.airflowWidth ?? 64))) / 100;
  const layerHeight =
    (height * Math.max(0.01, Number(properties.airflowHeight ?? 125))) / 100;
  const componentRotation = Number(position.rotation || 0);
  const airflowRotation = Number(properties.airflowRotation || 0);
  const airflowScale = Math.max(
    0.01,
    Math.min(5, Number(properties.airflowScale || 1)),
  );
  if (!grouped) {
    return {
