...
def _remap_popup_references(value, replacements: dict[str, str]) -> None:
    if isinstance(value, dict):
        if (
            value.get("popupSource") == "custom"
            and value.get("popupId") in replacements
        ):
...
def upgrade() -> None:
    ...
    rows = (
        connection.execute(
            sa.text(
                "SELECT project_id, document_json FROM project_drafts "
                "ORDER BY project_id"
            )
        )
        .mappings()
    ...
    for row in rows:
        ...
        for popup in document.get("customPopups") or []:
            if not (
                isinstance(popup, dict) and isinstance(popup.get("id"), str)
            ):
                continue
            popup_id = popup["id"]
            ...
            if (
                ...
                and _canonical(existing) != _canonical(popup)
            ):
        ...
        connection.execute(
            sa.text(
                "UPDATE project_drafts SET document_json = :document "
    ...
    connection.execute(
        sa.text(
            "INSERT INTO global_custom_popup_state "
            "(id, revision, popups_json, updated_by, updated_at) "
...
def downgrade() -> None:
    ...
    row = (
        connection.execute(
            sa.text(
                "SELECT popups_json FROM global_custom_popup_state WHERE id = 1"
            )
        )
        .mappings()
        .first()
    ...
    op.drop_index(
        "ix_global_custom_popup_state_updated_by",
        table_name="global_custom_popup_state",
    )
